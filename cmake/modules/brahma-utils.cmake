...
function(brahma_get_space_string OUTPUT_VAR LENGTH)
  set(_curr_length 0)
  set(_out_str "")
...
macro(brahma_get_max_str_length OUTPUT_VAR)
  set(${OUTPUT_VAR} 0)
  foreach(var ${ARGN})
...
if (__GIT_EXECUTABLE)
  ...
  if (__BUILDING_FROM_GIT_SOURCES)
    ...
    set(BRAHMA_GIT_VERSION "${__GIT_DESCRIBE_VERSION}"
      CACHE STRING "BRAHMA's version string as told by git.")
...
macro(append_str_tf STRING_VAR)
  brahma_get_max_str_length(_max_length ${ARGN})
  math(EXPR _max_length "${_max_length} + 2")

  foreach(var ${ARGN})
    string(LENGTH "${var}" _var_length)
    math(EXPR _num_spaces "${_max_length} - ${_var_length}")
    brahma_get_space_string(_spaces ${_num_spaces})
    if (${var})
      set(${var} "TRUE")
