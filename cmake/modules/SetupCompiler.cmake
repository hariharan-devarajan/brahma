...
macro(brahma_add_cxx_flags MY_FLAGS)
  foreach(flag ${ARGN})
...
macro(brahma_add_c_flags MY_FLAGS)
  foreach(flag ${ARGN})
    string(FIND "${${MY_FLAGS}}" "${flag}" flag_already_set)
    if(flag_already_set EQUAL -1)
      string(REPLACE "-" "_" _CLEAN_FLAG "${flag}")
...
if (BRAHMA_WARNINGS_AS_ERRORS)
  brahma_add_cxx_flags(_WERROR_FLAGS -Werror)
  separate_arguments(_WERROR_FLAGS NATIVE_COMMAND "${_WERROR_FLAGS}")
  if (NOT TARGET BRAHMA_CXX_FLAGS_werror)
    add_library(BRAHMA_CXX_FLAGS_werror INTERFACE)
    set_property(TARGET BRAHMA_CXX_FLAGS_werror PROPERTY
      INTERFACE_COMPILE_OPTIONS $<$<COMPILE_LANGUAGE:CXX>:${_WERROR_FLAGS}>)

    add_library(BRAHMA_C_FLAGS_werror INTERFACE)
    set_property(TARGET BRAHMA_C_FLAGS_werror PROPERTY
      INTERFACE_COMPILE_OPTIONS $<$<COMPILE_LANGUAGE:C>:${_WERROR_FLAGS}>)

    # Add the "library" to the export
    install(TARGETS BRAHMA_C_FLAGS_werror EXPORT ${BRAHMA_EXPORTED_TARGETS})
    install(TARGETS BRAHMA_CXX_FLAGS_werror EXPORT ${BRAHMA_EXPORTED_TARGETS})
  endif ()
endif ()
...
if (CMAKE_CXX_COMPILER_ID MATCHES "Clang")
  if (USE_CLANG_LIBCXX)
    brahma_add_cxx_flags(CMAKE_CXX_FLAGS "--stdlib=libc++")
  else (USE_CLANG_LIBCXX)
    if (GCC_TOOLCHAIN)
      brahma_add_cxx_flags(CMAKE_CXX_FLAGS "--gcc-toolchain=${GCC_TOOLCHAIN}")
    endif (GCC_TOOLCHAIN)
  ...
  if (CMAKE_BUILD_TYPE MATCHES Debug)
    ...
    brahma_add_cxx_flags(CMAKE_CXX_FLAGS -fno-omit-frame-pointer)
    brahma_add_c_flags(CMAKE_C_FLAGS -fno-omit-frame-pointer)
  endif ()
endif ()
...
if (CMAKE_CXX_COMPILER_ID MATCHES "Intel")
  ...
  # -openmp_profile
  brahma_add_cxx_flags(CMAKE_CXX_FLAGS -diag-disable=2196 -wd1011 -wd1875 -diag-disable=11074 -diag-disable=11076 ${GCC_INTEROP})
  brahma_add_cxx_flags(CMAKE_C_FLAGS -diag-disable=2196 -wd1011 -wd1875 -diag-disable=11074 -diag-disable=11076 ${GCC_INTEROP})
endif ()
...
set(CMAKE_INSTALL_RPATH "${CMAKE_INSTALL_PREFIX}/${BRAHMA_LIBDIR}")

list(FIND CMAKE_PLATFORM_IMPLICIT_LINK_DIRECTORIES
    "${CMAKE_INSTALL_PREFIX}/${BRAHMA_LIBDIR}" _IS_SYSTEM_DIR)

if (${_IS_SYSTEM_DIR} STREQUAL "-1")
    # Set the install RPATH correctly
    list(APPEND CMAKE_INSTALL_RPATH
      "${CMAKE_INSTALL_PREFIX}/${BRAHMA_LIBDIR}")
endif ()
...
try_compile(BRAHMA_HAS_STD_FILESYSTEM "${CMAKE_BINARY_DIR}/temp"
            "${CMAKE_SOURCE_DIR}/cmake/tests/has_filesystem.cpp"
            CMAKE_FLAGS ${CMAKE_CXX_FLAGS}
            LINK_LIBRARIES stdc++fs)
if (BRAHMA_HAS_STD_FILESYSTEM)
...
endif (BRAHMA_HAS_STD_FILESYSTEM)

try_compile(BRAHMA_HAS_STD_FSTREAM_FD "${CMAKE_BINARY_DIR}/temp"
            "${CMAKE_SOURCE_DIR}/cmake/tests/has_fd.cpp"
            CMAKE_FLAGS ${CMAKE_CXX_FLAGS}
            LINK_LIBRARIES stdc++fs)
if (BRAHMA_HAS_STD_FSTREAM_FD)
...
endif (BRAHMA_HAS_STD_FSTREAM_FD)