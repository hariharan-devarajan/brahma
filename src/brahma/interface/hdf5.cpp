
///
/// This file is generated by tools/generate_interfaces.py
/// Generated on: 2024-09-04 21:42:50
///

#include <brahma/interface/hdf5.h>

#include <stdexcept>

#ifdef BRAHMA_ENABLE_HDF5

GOTCHA_MACRO(H5open, herr_t, (), (), brahma::HDF5);

GOTCHA_MACRO(H5close, herr_t, (), (), brahma::HDF5);

GOTCHA_MACRO(H5dont_atexit, herr_t, (), (), brahma::HDF5);

GOTCHA_MACRO(H5garbage_collect, herr_t, (), (), brahma::HDF5);

GOTCHA_MACRO(H5set_free_list_limits, herr_t,
             (int reg_global_lim, int reg_list_lim, int arr_global_lim,
              int arr_list_lim, int blk_global_lim, int blk_list_lim),
             (reg_global_lim, reg_list_lim, arr_global_lim, arr_list_lim,
              blk_global_lim, blk_list_lim),
             brahma::HDF5);

GOTCHA_MACRO(H5get_free_list_sizes, herr_t,
             (size_t * reg_size, size_t *arr_size, size_t *blk_size,
              size_t *fac_size),
             (reg_size, arr_size, blk_size, fac_size), brahma::HDF5);

GOTCHA_MACRO(H5get_alloc_stats, herr_t, (H5_alloc_stats_t * stats), (stats),
             brahma::HDF5);

GOTCHA_MACRO(H5get_libversion, herr_t,
             (unsigned int *majnum, unsigned int *minnum, unsigned int *relnum),
             (majnum, minnum, relnum), brahma::HDF5);

GOTCHA_MACRO(H5check_version, herr_t,
             (unsigned int majnum, unsigned int minnum, unsigned int relnum),
             (majnum, minnum, relnum), brahma::HDF5);

GOTCHA_MACRO(H5is_library_threadsafe, herr_t, (hbool_t * is_ts), (is_ts),
             brahma::HDF5);

GOTCHA_MACRO(H5free_memory, herr_t, (void *mem), (mem), brahma::HDF5);

GOTCHA_MACRO(H5allocate_memory, void *, (size_t size, hbool_t clear),
             (size, clear), brahma::HDF5);

GOTCHA_MACRO(H5resize_memory, void *, (void *mem, size_t size), (mem, size),
             brahma::HDF5);

GOTCHA_MACRO(H5Iregister, hid_t, (H5I_type_t type, const void *object),
             (type, object), brahma::HDF5);

GOTCHA_MACRO(H5Iobject_verify, void *, (hid_t id, H5I_type_t id_type),
             (id, id_type), brahma::HDF5);

GOTCHA_MACRO(H5Iremove_verify, void *, (hid_t id, H5I_type_t id_type),
             (id, id_type), brahma::HDF5);

GOTCHA_MACRO(H5Iget_type, H5I_type_t, (hid_t id), (id), brahma::HDF5);

GOTCHA_MACRO(H5Iget_file_id, hid_t, (hid_t id), (id), brahma::HDF5);

GOTCHA_MACRO(H5Iget_name, ssize_t, (hid_t id, char *name, size_t size),
             (id, name, size), brahma::HDF5);

GOTCHA_MACRO(H5Iinc_ref, int, (hid_t id), (id), brahma::HDF5);

GOTCHA_MACRO(H5Idec_ref, int, (hid_t id), (id), brahma::HDF5);

GOTCHA_MACRO(H5Iget_ref, int, (hid_t id), (id), brahma::HDF5);

GOTCHA_MACRO(H5Iregister_type, H5I_type_t,
             (size_t hash_size, unsigned int reserved, H5I_free_t free_func),
             (hash_size, reserved, free_func), brahma::HDF5);

GOTCHA_MACRO(H5Iclear_type, herr_t, (H5I_type_t type, hbool_t force),
             (type, force), brahma::HDF5);

GOTCHA_MACRO(H5Idestroy_type, herr_t, (H5I_type_t type), (type), brahma::HDF5);

GOTCHA_MACRO(H5Iinc_type_ref, int, (H5I_type_t type), (type), brahma::HDF5);

GOTCHA_MACRO(H5Idec_type_ref, int, (H5I_type_t type), (type), brahma::HDF5);

GOTCHA_MACRO(H5Iget_type_ref, int, (H5I_type_t type), (type), brahma::HDF5);

GOTCHA_MACRO(H5Isearch, void *,
             (H5I_type_t type, H5I_search_func_t func, void *key),
             (type, func, key), brahma::HDF5);

GOTCHA_MACRO(H5Inmembers, herr_t, (H5I_type_t type, hsize_t *num_members),
             (type, num_members), brahma::HDF5);

GOTCHA_MACRO(H5Itype_exists, htri_t, (H5I_type_t type), (type), brahma::HDF5);

GOTCHA_MACRO(H5Iis_valid, htri_t, (hid_t id), (id), brahma::HDF5);

GOTCHA_MACRO(H5Tcreate, hid_t, (H5T_class_t type, size_t size), (type, size),
             brahma::HDF5);

GOTCHA_MACRO(H5Tcopy, hid_t, (hid_t type_id), (type_id), brahma::HDF5);

GOTCHA_MACRO(H5Tclose, herr_t, (hid_t type_id), (type_id), brahma::HDF5);

GOTCHA_MACRO(H5Tequal, htri_t, (hid_t type1_id, hid_t type2_id),
             (type1_id, type2_id), brahma::HDF5);

GOTCHA_MACRO(H5Tlock, herr_t, (hid_t type_id), (type_id), brahma::HDF5);

GOTCHA_MACRO(H5Tcommit2, herr_t,
             (hid_t loc_id, const char *name, hid_t type_id, hid_t lcpl_id,
              hid_t tcpl_id, hid_t tapl_id),
             (loc_id, name, type_id, lcpl_id, tcpl_id, tapl_id), brahma::HDF5);

GOTCHA_MACRO(H5Topen2, hid_t, (hid_t loc_id, const char *name, hid_t tapl_id),
             (loc_id, name, tapl_id), brahma::HDF5);

GOTCHA_MACRO(H5Tcommit_anon, herr_t,
             (hid_t loc_id, hid_t type_id, hid_t tcpl_id, hid_t tapl_id),
             (loc_id, type_id, tcpl_id, tapl_id), brahma::HDF5);

GOTCHA_MACRO(H5Tget_create_plist, hid_t, (hid_t type_id), (type_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Tcommitted, htri_t, (hid_t type_id), (type_id), brahma::HDF5);

GOTCHA_MACRO(H5Tencode, herr_t, (hid_t obj_id, void *buf, size_t *nalloc),
             (obj_id, buf, nalloc), brahma::HDF5);

GOTCHA_MACRO(H5Tdecode, hid_t, (const void *buf), (buf), brahma::HDF5);

GOTCHA_MACRO(H5Tflush, herr_t, (hid_t type_id), (type_id), brahma::HDF5);

GOTCHA_MACRO(H5Trefresh, herr_t, (hid_t type_id), (type_id), brahma::HDF5);

GOTCHA_MACRO(H5Tinsert, herr_t,
             (hid_t parent_id, const char *name, size_t offset,
              hid_t member_id),
             (parent_id, name, offset, member_id), brahma::HDF5);

GOTCHA_MACRO(H5Tpack, herr_t, (hid_t type_id), (type_id), brahma::HDF5);

GOTCHA_MACRO(H5Tenum_create, hid_t, (hid_t base_id), (base_id), brahma::HDF5);

GOTCHA_MACRO(H5Tenum_insert, herr_t,
             (hid_t type, const char *name, const void *value),
             (type, name, value), brahma::HDF5);

GOTCHA_MACRO(H5Tenum_nameof, herr_t,
             (hid_t type, const void *value, char *name, size_t size),
             (type, value, name, size), brahma::HDF5);

GOTCHA_MACRO(H5Tenum_valueof, herr_t,
             (hid_t type, const char *name, void *value), (type, name, value),
             brahma::HDF5);

GOTCHA_MACRO(H5Tvlen_create, hid_t, (hid_t base_id), (base_id), brahma::HDF5);

GOTCHA_MACRO(H5Tarray_create2, hid_t,
             (hid_t base_id, unsigned int ndims, const hsize_t dim[]),
             (base_id, ndims, dim), brahma::HDF5);

GOTCHA_MACRO(H5Tget_array_ndims, int, (hid_t type_id), (type_id), brahma::HDF5);

GOTCHA_MACRO(H5Tget_array_dims2, int, (hid_t type_id, hsize_t dims[]),
             (type_id, dims), brahma::HDF5);

GOTCHA_MACRO(H5Tset_tag, herr_t, (hid_t type, const char *tag), (type, tag),
             brahma::HDF5);

GOTCHA_MACRO(H5Tget_tag, char *, (hid_t type), (type), brahma::HDF5);

GOTCHA_MACRO(H5Tget_super, hid_t, (hid_t type), (type), brahma::HDF5);

GOTCHA_MACRO(H5Tget_class, H5T_class_t, (hid_t type_id), (type_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Tdetect_class, htri_t, (hid_t type_id, H5T_class_t cls),
             (type_id, cls), brahma::HDF5);

GOTCHA_MACRO(H5Tget_size, size_t, (hid_t type_id), (type_id), brahma::HDF5);

GOTCHA_MACRO(H5Tget_order, H5T_order_t, (hid_t type_id), (type_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Tget_precision, size_t, (hid_t type_id), (type_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Tget_offset, int, (hid_t type_id), (type_id), brahma::HDF5);

GOTCHA_MACRO(H5Tget_pad, herr_t,
             (hid_t type_id, H5T_pad_t *lsb, H5T_pad_t *msb),
             (type_id, lsb, msb), brahma::HDF5);

GOTCHA_MACRO(H5Tget_sign, H5T_sign_t, (hid_t type_id), (type_id), brahma::HDF5);

GOTCHA_MACRO(H5Tget_fields, herr_t,
             (hid_t type_id, size_t *spos, size_t *epos, size_t *esize,
              size_t *mpos, size_t *msize),
             (type_id, spos, epos, esize, mpos, msize), brahma::HDF5);

GOTCHA_MACRO(H5Tget_ebias, size_t, (hid_t type_id), (type_id), brahma::HDF5);

GOTCHA_MACRO(H5Tget_norm, H5T_norm_t, (hid_t type_id), (type_id), brahma::HDF5);

GOTCHA_MACRO(H5Tget_inpad, H5T_pad_t, (hid_t type_id), (type_id), brahma::HDF5);

GOTCHA_MACRO(H5Tget_strpad, H5T_str_t, (hid_t type_id), (type_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Tget_nmembers, int, (hid_t type_id), (type_id), brahma::HDF5);

GOTCHA_MACRO(H5Tget_member_name, char *, (hid_t type_id, unsigned int membno),
             (type_id, membno), brahma::HDF5);

GOTCHA_MACRO(H5Tget_member_index, int, (hid_t type_id, const char *name),
             (type_id, name), brahma::HDF5);

GOTCHA_MACRO(H5Tget_member_offset, size_t, (hid_t type_id, unsigned int membno),
             (type_id, membno), brahma::HDF5);

GOTCHA_MACRO(H5Tget_member_class, H5T_class_t,
             (hid_t type_id, unsigned int membno), (type_id, membno),
             brahma::HDF5);

GOTCHA_MACRO(H5Tget_member_type, hid_t, (hid_t type_id, unsigned int membno),
             (type_id, membno), brahma::HDF5);

GOTCHA_MACRO(H5Tget_member_value, herr_t,
             (hid_t type_id, unsigned int membno, void *value),
             (type_id, membno, value), brahma::HDF5);

GOTCHA_MACRO(H5Tget_cset, H5T_cset_t, (hid_t type_id), (type_id), brahma::HDF5);

GOTCHA_MACRO(H5Tis_variable_str, htri_t, (hid_t type_id), (type_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Tget_native_type, hid_t,
             (hid_t type_id, H5T_direction_t direction), (type_id, direction),
             brahma::HDF5);

GOTCHA_MACRO(H5Tset_size, herr_t, (hid_t type_id, size_t size), (type_id, size),
             brahma::HDF5);

GOTCHA_MACRO(H5Tset_order, herr_t, (hid_t type_id, H5T_order_t order),
             (type_id, order), brahma::HDF5);

GOTCHA_MACRO(H5Tset_precision, herr_t, (hid_t type_id, size_t prec),
             (type_id, prec), brahma::HDF5);

GOTCHA_MACRO(H5Tset_offset, herr_t, (hid_t type_id, size_t offset),
             (type_id, offset), brahma::HDF5);

GOTCHA_MACRO(H5Tset_pad, herr_t, (hid_t type_id, H5T_pad_t lsb, H5T_pad_t msb),
             (type_id, lsb, msb), brahma::HDF5);

GOTCHA_MACRO(H5Tset_sign, herr_t, (hid_t type_id, H5T_sign_t sign),
             (type_id, sign), brahma::HDF5);

GOTCHA_MACRO(H5Tset_fields, herr_t,
             (hid_t type_id, size_t spos, size_t epos, size_t esize,
              size_t mpos, size_t msize),
             (type_id, spos, epos, esize, mpos, msize), brahma::HDF5);

GOTCHA_MACRO(H5Tset_ebias, herr_t, (hid_t type_id, size_t ebias),
             (type_id, ebias), brahma::HDF5);

GOTCHA_MACRO(H5Tset_norm, herr_t, (hid_t type_id, H5T_norm_t norm),
             (type_id, norm), brahma::HDF5);

GOTCHA_MACRO(H5Tset_inpad, herr_t, (hid_t type_id, H5T_pad_t pad),
             (type_id, pad), brahma::HDF5);

GOTCHA_MACRO(H5Tset_cset, herr_t, (hid_t type_id, H5T_cset_t cset),
             (type_id, cset), brahma::HDF5);

GOTCHA_MACRO(H5Tset_strpad, herr_t, (hid_t type_id, H5T_str_t strpad),
             (type_id, strpad), brahma::HDF5);

GOTCHA_MACRO(H5Tregister, herr_t,
             (H5T_pers_t pers, const char *name, hid_t src_id, hid_t dst_id,
              H5T_conv_t func),
             (pers, name, src_id, dst_id, func), brahma::HDF5);

GOTCHA_MACRO(H5Tunregister, herr_t,
             (H5T_pers_t pers, const char *name, hid_t src_id, hid_t dst_id,
              H5T_conv_t func),
             (pers, name, src_id, dst_id, func), brahma::HDF5);

GOTCHA_MACRO(H5Tfind, H5T_conv_t,
             (hid_t src_id, hid_t dst_id, H5T_cdata_t **pcdata),
             (src_id, dst_id, pcdata), brahma::HDF5);

GOTCHA_MACRO(H5Tcompiler_conv, htri_t, (hid_t src_id, hid_t dst_id),
             (src_id, dst_id), brahma::HDF5);

GOTCHA_MACRO(H5Tconvert, herr_t,
             (hid_t src_id, hid_t dst_id, size_t nelmts, void *buf,
              void *background, hid_t plist_id),
             (src_id, dst_id, nelmts, buf, background, plist_id), brahma::HDF5);

GOTCHA_MACRO(H5Tcommit1, herr_t,
             (hid_t loc_id, const char *name, hid_t type_id),
             (loc_id, name, type_id), brahma::HDF5);

GOTCHA_MACRO(H5Topen1, hid_t, (hid_t loc_id, const char *name), (loc_id, name),
             brahma::HDF5);

GOTCHA_MACRO(H5Tarray_create1, hid_t,
             (hid_t base_id, int ndims, const hsize_t dim[], const int perm[]),
             (base_id, ndims, dim, perm), brahma::HDF5);

GOTCHA_MACRO(H5Tget_array_dims1, int,
             (hid_t type_id, hsize_t dims[], int perm[]), (type_id, dims, perm),
             brahma::HDF5);

GOTCHA_MACRO(H5Lmove, herr_t,
             (hid_t src_loc, const char *src_name, hid_t dst_loc,
              const char *dst_name, hid_t lcpl_id, hid_t lapl_id),
             (src_loc, src_name, dst_loc, dst_name, lcpl_id, lapl_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Lcopy, herr_t,
             (hid_t src_loc, const char *src_name, hid_t dst_loc,
              const char *dst_name, hid_t lcpl_id, hid_t lapl_id),
             (src_loc, src_name, dst_loc, dst_name, lcpl_id, lapl_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Lcreate_hard, herr_t,
             (hid_t cur_loc, const char *cur_name, hid_t dst_loc,
              const char *dst_name, hid_t lcpl_id, hid_t lapl_id),
             (cur_loc, cur_name, dst_loc, dst_name, lcpl_id, lapl_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Lcreate_soft, herr_t,
             (const char *link_target, hid_t link_loc_id, const char *link_name,
              hid_t lcpl_id, hid_t lapl_id),
             (link_target, link_loc_id, link_name, lcpl_id, lapl_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Ldelete, herr_t, (hid_t loc_id, const char *name, hid_t lapl_id),
             (loc_id, name, lapl_id), brahma::HDF5);

GOTCHA_MACRO(H5Ldelete_by_idx, herr_t,
             (hid_t loc_id, const char *group_name, H5_index_t idx_type,
              H5_iter_order_t order, hsize_t n, hid_t lapl_id),
             (loc_id, group_name, idx_type, order, n, lapl_id), brahma::HDF5);

GOTCHA_MACRO(H5Lget_val, herr_t,
             (hid_t loc_id, const char *name, void *buf, size_t size,
              hid_t lapl_id),
             (loc_id, name, buf, size, lapl_id), brahma::HDF5);

GOTCHA_MACRO(H5Lget_val_by_idx, herr_t,
             (hid_t loc_id, const char *group_name, H5_index_t idx_type,
              H5_iter_order_t order, hsize_t n, void *buf, size_t size,
              hid_t lapl_id),
             (loc_id, group_name, idx_type, order, n, buf, size, lapl_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Lexists, htri_t, (hid_t loc_id, const char *name, hid_t lapl_id),
             (loc_id, name, lapl_id), brahma::HDF5);

GOTCHA_MACRO(H5Lget_info, herr_t,
             (hid_t loc_id, const char *name, H5L_info_t *linfo, hid_t lapl_id),
             (loc_id, name, linfo, lapl_id), brahma::HDF5);

GOTCHA_MACRO(H5Lget_info_by_idx, herr_t,
             (hid_t loc_id, const char *group_name, H5_index_t idx_type,
              H5_iter_order_t order, hsize_t n, H5L_info_t *linfo,
              hid_t lapl_id),
             (loc_id, group_name, idx_type, order, n, linfo, lapl_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Lget_name_by_idx, ssize_t,
             (hid_t loc_id, const char *group_name, H5_index_t idx_type,
              H5_iter_order_t order, hsize_t n, char *name, size_t size,
              hid_t lapl_id),
             (loc_id, group_name, idx_type, order, n, name, size, lapl_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Literate, herr_t,
             (hid_t grp_id, H5_index_t idx_type, H5_iter_order_t order,
              hsize_t *idx, H5L_iterate_t op, void *op_data),
             (grp_id, idx_type, order, idx, op, op_data), brahma::HDF5);

GOTCHA_MACRO(H5Literate_by_name, herr_t,
             (hid_t loc_id, const char *group_name, H5_index_t idx_type,
              H5_iter_order_t order, hsize_t *idx, H5L_iterate_t op,
              void *op_data, hid_t lapl_id),
             (loc_id, group_name, idx_type, order, idx, op, op_data, lapl_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Lvisit, herr_t,
             (hid_t grp_id, H5_index_t idx_type, H5_iter_order_t order,
              H5L_iterate_t op, void *op_data),
             (grp_id, idx_type, order, op, op_data), brahma::HDF5);

GOTCHA_MACRO(H5Lvisit_by_name, herr_t,
             (hid_t loc_id, const char *group_name, H5_index_t idx_type,
              H5_iter_order_t order, H5L_iterate_t op, void *op_data,
              hid_t lapl_id),
             (loc_id, group_name, idx_type, order, op, op_data, lapl_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Lcreate_ud, herr_t,
             (hid_t link_loc_id, const char *link_name, H5L_type_t link_type,
              const void *udata, size_t udata_size, hid_t lcpl_id,
              hid_t lapl_id),
             (link_loc_id, link_name, link_type, udata, udata_size, lcpl_id,
              lapl_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Lregister, herr_t, (const H5L_class_t *cls), (cls),
             brahma::HDF5);

GOTCHA_MACRO(H5Lunregister, herr_t, (H5L_type_t id), (id), brahma::HDF5);

GOTCHA_MACRO(H5Lis_registered, htri_t, (H5L_type_t id), (id), brahma::HDF5);

GOTCHA_MACRO(H5Lunpack_elink_val, herr_t,
             (const void *ext_linkval, size_t link_size, unsigned int *flags,
              const char **filename, const char **obj_path),
             (ext_linkval, link_size, flags, filename, obj_path), brahma::HDF5);

GOTCHA_MACRO(H5Lcreate_external, herr_t,
             (const char *file_name, const char *obj_name, hid_t link_loc_id,
              const char *link_name, hid_t lcpl_id, hid_t lapl_id),
             (file_name, obj_name, link_loc_id, link_name, lcpl_id, lapl_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Oopen, hid_t, (hid_t loc_id, const char *name, hid_t lapl_id),
             (loc_id, name, lapl_id), brahma::HDF5);

GOTCHA_MACRO(H5Oopen_by_addr, hid_t, (hid_t loc_id, haddr_t addr),
             (loc_id, addr), brahma::HDF5);

GOTCHA_MACRO(H5Oopen_by_idx, hid_t,
             (hid_t loc_id, const char *group_name, H5_index_t idx_type,
              H5_iter_order_t order, hsize_t n, hid_t lapl_id),
             (loc_id, group_name, idx_type, order, n, lapl_id), brahma::HDF5);

GOTCHA_MACRO(H5Oexists_by_name, htri_t,
             (hid_t loc_id, const char *name, hid_t lapl_id),
             (loc_id, name, lapl_id), brahma::HDF5);

GOTCHA_MACRO(H5Oget_info2, herr_t,
             (hid_t loc_id, H5O_info_t *oinfo, unsigned int fields),
             (loc_id, oinfo, fields), brahma::HDF5);

GOTCHA_MACRO(H5Oget_info_by_name2, herr_t,
             (hid_t loc_id, const char *name, H5O_info_t *oinfo,
              unsigned int fields, hid_t lapl_id),
             (loc_id, name, oinfo, fields, lapl_id), brahma::HDF5);

GOTCHA_MACRO(H5Oget_info_by_idx2, herr_t,
             (hid_t loc_id, const char *group_name, H5_index_t idx_type,
              H5_iter_order_t order, hsize_t n, H5O_info_t *oinfo,
              unsigned int fields, hid_t lapl_id),
             (loc_id, group_name, idx_type, order, n, oinfo, fields, lapl_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Olink, herr_t,
             (hid_t obj_id, hid_t new_loc_id, const char *new_name,
              hid_t lcpl_id, hid_t lapl_id),
             (obj_id, new_loc_id, new_name, lcpl_id, lapl_id), brahma::HDF5);

GOTCHA_MACRO(H5Oincr_refcount, herr_t, (hid_t object_id), (object_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Odecr_refcount, herr_t, (hid_t object_id), (object_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Ocopy, herr_t,
             (hid_t src_loc_id, const char *src_name, hid_t dst_loc_id,
              const char *dst_name, hid_t ocpypl_id, hid_t lcpl_id),
             (src_loc_id, src_name, dst_loc_id, dst_name, ocpypl_id, lcpl_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Oset_comment, herr_t, (hid_t obj_id, const char *comment),
             (obj_id, comment), brahma::HDF5);

GOTCHA_MACRO(H5Oset_comment_by_name, herr_t,
             (hid_t loc_id, const char *name, const char *comment,
              hid_t lapl_id),
             (loc_id, name, comment, lapl_id), brahma::HDF5);

GOTCHA_MACRO(H5Oget_comment, ssize_t,
             (hid_t obj_id, char *comment, size_t bufsize),
             (obj_id, comment, bufsize), brahma::HDF5);

GOTCHA_MACRO(H5Oget_comment_by_name, ssize_t,
             (hid_t loc_id, const char *name, char *comment, size_t bufsize,
              hid_t lapl_id),
             (loc_id, name, comment, bufsize, lapl_id), brahma::HDF5);

GOTCHA_MACRO(H5Ovisit2, herr_t,
             (hid_t obj_id, H5_index_t idx_type, H5_iter_order_t order,
              H5O_iterate_t op, void *op_data, unsigned int fields),
             (obj_id, idx_type, order, op, op_data, fields), brahma::HDF5);

GOTCHA_MACRO(H5Ovisit_by_name2, herr_t,
             (hid_t loc_id, const char *obj_name, H5_index_t idx_type,
              H5_iter_order_t order, H5O_iterate_t op, void *op_data,
              unsigned int fields, hid_t lapl_id),
             (loc_id, obj_name, idx_type, order, op, op_data, fields, lapl_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Oclose, herr_t, (hid_t object_id), (object_id), brahma::HDF5);

GOTCHA_MACRO(H5Oflush, herr_t, (hid_t obj_id), (obj_id), brahma::HDF5);

GOTCHA_MACRO(H5Orefresh, herr_t, (hid_t oid), (oid), brahma::HDF5);

GOTCHA_MACRO(H5Odisable_mdc_flushes, herr_t, (hid_t object_id), (object_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Oenable_mdc_flushes, herr_t, (hid_t object_id), (object_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Oare_mdc_flushes_disabled, herr_t,
             (hid_t object_id, hbool_t *are_disabled),
             (object_id, are_disabled), brahma::HDF5);

GOTCHA_MACRO(H5Oget_info, herr_t, (hid_t loc_id, H5O_info_t *oinfo),
             (loc_id, oinfo), brahma::HDF5);

GOTCHA_MACRO(H5Oget_info_by_name, herr_t,
             (hid_t loc_id, const char *name, H5O_info_t *oinfo, hid_t lapl_id),
             (loc_id, name, oinfo, lapl_id), brahma::HDF5);

GOTCHA_MACRO(H5Oget_info_by_idx, herr_t,
             (hid_t loc_id, const char *group_name, H5_index_t idx_type,
              H5_iter_order_t order, hsize_t n, H5O_info_t *oinfo,
              hid_t lapl_id),
             (loc_id, group_name, idx_type, order, n, oinfo, lapl_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Ovisit, herr_t,
             (hid_t obj_id, H5_index_t idx_type, H5_iter_order_t order,
              H5O_iterate_t op, void *op_data),
             (obj_id, idx_type, order, op, op_data), brahma::HDF5);

GOTCHA_MACRO(H5Ovisit_by_name, herr_t,
             (hid_t loc_id, const char *obj_name, H5_index_t idx_type,
              H5_iter_order_t order, H5O_iterate_t op, void *op_data,
              hid_t lapl_id),
             (loc_id, obj_name, idx_type, order, op, op_data, lapl_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Oget_info1, herr_t, (hid_t loc_id, H5O_info_t *oinfo),
             (loc_id, oinfo), brahma::HDF5);

GOTCHA_MACRO(H5Oget_info_by_name1, herr_t,
             (hid_t loc_id, const char *name, H5O_info_t *oinfo, hid_t lapl_id),
             (loc_id, name, oinfo, lapl_id), brahma::HDF5);

GOTCHA_MACRO(H5Oget_info_by_idx1, herr_t,
             (hid_t loc_id, const char *group_name, H5_index_t idx_type,
              H5_iter_order_t order, hsize_t n, H5O_info_t *oinfo,
              hid_t lapl_id),
             (loc_id, group_name, idx_type, order, n, oinfo, lapl_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Ovisit1, herr_t,
             (hid_t obj_id, H5_index_t idx_type, H5_iter_order_t order,
              H5O_iterate_t op, void *op_data),
             (obj_id, idx_type, order, op, op_data), brahma::HDF5);

GOTCHA_MACRO(H5Ovisit_by_name1, herr_t,
             (hid_t loc_id, const char *obj_name, H5_index_t idx_type,
              H5_iter_order_t order, H5O_iterate_t op, void *op_data,
              hid_t lapl_id),
             (loc_id, obj_name, idx_type, order, op, op_data, lapl_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Acreate2, hid_t,
             (hid_t loc_id, const char *attr_name, hid_t type_id,
              hid_t space_id, hid_t acpl_id, hid_t aapl_id),
             (loc_id, attr_name, type_id, space_id, acpl_id, aapl_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Acreate_by_name, hid_t,
             (hid_t loc_id, const char *obj_name, const char *attr_name,
              hid_t type_id, hid_t space_id, hid_t acpl_id, hid_t aapl_id,
              hid_t lapl_id),
             (loc_id, obj_name, attr_name, type_id, space_id, acpl_id, aapl_id,
              lapl_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Aopen, hid_t,
             (hid_t obj_id, const char *attr_name, hid_t aapl_id),
             (obj_id, attr_name, aapl_id), brahma::HDF5);

GOTCHA_MACRO(H5Aopen_by_name, hid_t,
             (hid_t loc_id, const char *obj_name, const char *attr_name,
              hid_t aapl_id, hid_t lapl_id),
             (loc_id, obj_name, attr_name, aapl_id, lapl_id), brahma::HDF5);

GOTCHA_MACRO(H5Aopen_by_idx, hid_t,
             (hid_t loc_id, const char *obj_name, H5_index_t idx_type,
              H5_iter_order_t order, hsize_t n, hid_t aapl_id, hid_t lapl_id),
             (loc_id, obj_name, idx_type, order, n, aapl_id, lapl_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Awrite, herr_t, (hid_t attr_id, hid_t type_id, const void *buf),
             (attr_id, type_id, buf), brahma::HDF5);

GOTCHA_MACRO(H5Aread, herr_t, (hid_t attr_id, hid_t type_id, void *buf),
             (attr_id, type_id, buf), brahma::HDF5);

GOTCHA_MACRO(H5Aclose, herr_t, (hid_t attr_id), (attr_id), brahma::HDF5);

GOTCHA_MACRO(H5Aget_space, hid_t, (hid_t attr_id), (attr_id), brahma::HDF5);

GOTCHA_MACRO(H5Aget_type, hid_t, (hid_t attr_id), (attr_id), brahma::HDF5);

GOTCHA_MACRO(H5Aget_create_plist, hid_t, (hid_t attr_id), (attr_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Aget_name, ssize_t, (hid_t attr_id, size_t buf_size, char *buf),
             (attr_id, buf_size, buf), brahma::HDF5);

GOTCHA_MACRO(H5Aget_name_by_idx, ssize_t,
             (hid_t loc_id, const char *obj_name, H5_index_t idx_type,
              H5_iter_order_t order, hsize_t n, char *name, size_t size,
              hid_t lapl_id),
             (loc_id, obj_name, idx_type, order, n, name, size, lapl_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Aget_storage_size, hsize_t, (hid_t attr_id), (attr_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Aget_info, herr_t, (hid_t attr_id, H5A_info_t *ainfo),
             (attr_id, ainfo), brahma::HDF5);

GOTCHA_MACRO(H5Aget_info_by_name, herr_t,
             (hid_t loc_id, const char *obj_name, const char *attr_name,
              H5A_info_t *ainfo, hid_t lapl_id),
             (loc_id, obj_name, attr_name, ainfo, lapl_id), brahma::HDF5);

GOTCHA_MACRO(H5Aget_info_by_idx, herr_t,
             (hid_t loc_id, const char *obj_name, H5_index_t idx_type,
              H5_iter_order_t order, hsize_t n, H5A_info_t *ainfo,
              hid_t lapl_id),
             (loc_id, obj_name, idx_type, order, n, ainfo, lapl_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Arename, herr_t,
             (hid_t loc_id, const char *old_name, const char *new_name),
             (loc_id, old_name, new_name), brahma::HDF5);

GOTCHA_MACRO(H5Arename_by_name, herr_t,
             (hid_t loc_id, const char *obj_name, const char *old_attr_name,
              const char *new_attr_name, hid_t lapl_id),
             (loc_id, obj_name, old_attr_name, new_attr_name, lapl_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Aiterate2, herr_t,
             (hid_t loc_id, H5_index_t idx_type, H5_iter_order_t order,
              hsize_t *idx, H5A_operator2_t op, void *op_data),
             (loc_id, idx_type, order, idx, op, op_data), brahma::HDF5);

GOTCHA_MACRO(H5Aiterate_by_name, herr_t,
             (hid_t loc_id, const char *obj_name, H5_index_t idx_type,
              H5_iter_order_t order, hsize_t *idx, H5A_operator2_t op,
              void *op_data, hid_t lapd_id),
             (loc_id, obj_name, idx_type, order, idx, op, op_data, lapd_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Adelete, herr_t, (hid_t loc_id, const char *name),
             (loc_id, name), brahma::HDF5);

GOTCHA_MACRO(H5Adelete_by_name, herr_t,
             (hid_t loc_id, const char *obj_name, const char *attr_name,
              hid_t lapl_id),
             (loc_id, obj_name, attr_name, lapl_id), brahma::HDF5);

GOTCHA_MACRO(H5Adelete_by_idx, herr_t,
             (hid_t loc_id, const char *obj_name, H5_index_t idx_type,
              H5_iter_order_t order, hsize_t n, hid_t lapl_id),
             (loc_id, obj_name, idx_type, order, n, lapl_id), brahma::HDF5);

GOTCHA_MACRO(H5Aexists, htri_t, (hid_t obj_id, const char *attr_name),
             (obj_id, attr_name), brahma::HDF5);

GOTCHA_MACRO(H5Aexists_by_name, htri_t,
             (hid_t obj_id, const char *obj_name, const char *attr_name,
              hid_t lapl_id),
             (obj_id, obj_name, attr_name, lapl_id), brahma::HDF5);

GOTCHA_MACRO(H5Acreate1, hid_t,
             (hid_t loc_id, const char *name, hid_t type_id, hid_t space_id,
              hid_t acpl_id),
             (loc_id, name, type_id, space_id, acpl_id), brahma::HDF5);

GOTCHA_MACRO(H5Aopen_name, hid_t, (hid_t loc_id, const char *name),
             (loc_id, name), brahma::HDF5);

GOTCHA_MACRO(H5Aopen_idx, hid_t, (hid_t loc_id, unsigned int idx),
             (loc_id, idx), brahma::HDF5);

GOTCHA_MACRO(H5Aget_num_attrs, int, (hid_t loc_id), (loc_id), brahma::HDF5);

GOTCHA_MACRO(H5Aiterate1, herr_t,
             (hid_t loc_id, unsigned int *attr_num, H5A_operator1_t op,
              void *op_data),
             (loc_id, attr_num, op, op_data), brahma::HDF5);

GOTCHA_MACRO(H5Dcreate2, hid_t,
             (hid_t loc_id, const char *name, hid_t type_id, hid_t space_id,
              hid_t lcpl_id, hid_t dcpl_id, hid_t dapl_id),
             (loc_id, name, type_id, space_id, lcpl_id, dcpl_id, dapl_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Dcreate_anon, hid_t,
             (hid_t file_id, hid_t type_id, hid_t space_id, hid_t plist_id,
              hid_t dapl_id),
             (file_id, type_id, space_id, plist_id, dapl_id), brahma::HDF5);

GOTCHA_MACRO(H5Dopen2, hid_t, (hid_t file_id, const char *name, hid_t dapl_id),
             (file_id, name, dapl_id), brahma::HDF5);

GOTCHA_MACRO(H5Dclose, herr_t, (hid_t dset_id), (dset_id), brahma::HDF5);

GOTCHA_MACRO(H5Dget_space, hid_t, (hid_t dset_id), (dset_id), brahma::HDF5);

GOTCHA_MACRO(H5Dget_space_status, herr_t,
             (hid_t dset_id, H5D_space_status_t *allocation),
             (dset_id, allocation), brahma::HDF5);

GOTCHA_MACRO(H5Dget_type, hid_t, (hid_t dset_id), (dset_id), brahma::HDF5);

GOTCHA_MACRO(H5Dget_create_plist, hid_t, (hid_t dset_id), (dset_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Dget_access_plist, hid_t, (hid_t dset_id), (dset_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Dget_storage_size, hsize_t, (hid_t dset_id), (dset_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Dget_chunk_storage_size, herr_t,
             (hid_t dset_id, const hsize_t *offset, hsize_t *chunk_bytes),
             (dset_id, offset, chunk_bytes), brahma::HDF5);

GOTCHA_MACRO(H5Dget_num_chunks, herr_t,
             (hid_t dset_id, hid_t fspace_id, hsize_t *nchunks),
             (dset_id, fspace_id, nchunks), brahma::HDF5);

GOTCHA_MACRO(H5Dget_chunk_info_by_coord, herr_t,
             (hid_t dset_id, const hsize_t *coord, unsigned int *filter_mask,
              haddr_t *addr, hsize_t *size),
             (dset_id, coord, filter_mask, addr, size), brahma::HDF5);

GOTCHA_MACRO(H5Dget_chunk_info, herr_t,
             (hid_t dset_id, hid_t fspace_id, hsize_t chk_idx, hsize_t *coord,
              unsigned int *filter_mask, haddr_t *addr, hsize_t *size),
             (dset_id, fspace_id, chk_idx, coord, filter_mask, addr, size),
             brahma::HDF5);

GOTCHA_MACRO(H5Dget_offset, haddr_t, (hid_t dset_id), (dset_id), brahma::HDF5);

GOTCHA_MACRO(H5Dread, herr_t,
             (hid_t dset_id, hid_t mem_type_id, hid_t mem_space_id,
              hid_t file_space_id, hid_t plist_id, void *buf),
             (dset_id, mem_type_id, mem_space_id, file_space_id, plist_id, buf),
             brahma::HDF5);

GOTCHA_MACRO(H5Dwrite, herr_t,
             (hid_t dset_id, hid_t mem_type_id, hid_t mem_space_id,
              hid_t file_space_id, hid_t plist_id, const void *buf),
             (dset_id, mem_type_id, mem_space_id, file_space_id, plist_id, buf),
             brahma::HDF5);

GOTCHA_MACRO(H5Dwrite_chunk, herr_t,
             (hid_t dset_id, hid_t dxpl_id, uint32_t filters,
              const hsize_t *offset, size_t data_size, const void *buf),
             (dset_id, dxpl_id, filters, offset, data_size, buf), brahma::HDF5);

GOTCHA_MACRO(H5Dread_chunk, herr_t,
             (hid_t dset_id, hid_t dxpl_id, const hsize_t *offset,
              uint32_t *filters, void *buf),
             (dset_id, dxpl_id, offset, filters, buf), brahma::HDF5);

GOTCHA_MACRO(H5Diterate, herr_t,
             (void *buf, hid_t type_id, hid_t space_id, H5D_operator_t op,
              void *operator_data),
             (buf, type_id, space_id, op, operator_data), brahma::HDF5);

GOTCHA_MACRO(H5Dvlen_reclaim, herr_t,
             (hid_t type_id, hid_t space_id, hid_t plist_id, void *buf),
             (type_id, space_id, plist_id, buf), brahma::HDF5);

GOTCHA_MACRO(H5Dvlen_get_buf_size, herr_t,
             (hid_t dataset_id, hid_t type_id, hid_t space_id, hsize_t *size),
             (dataset_id, type_id, space_id, size), brahma::HDF5);

GOTCHA_MACRO(H5Dfill, herr_t,
             (const void *fill, hid_t fill_type, void *buf, hid_t buf_type,
              hid_t space),
             (fill, fill_type, buf, buf_type, space), brahma::HDF5);

GOTCHA_MACRO(H5Dset_extent, herr_t, (hid_t dset_id, const hsize_t size[]),
             (dset_id, size), brahma::HDF5);

GOTCHA_MACRO(H5Dflush, herr_t, (hid_t dset_id), (dset_id), brahma::HDF5);

GOTCHA_MACRO(H5Drefresh, herr_t, (hid_t dset_id), (dset_id), brahma::HDF5);

GOTCHA_MACRO(H5Dscatter, herr_t,
             (H5D_scatter_func_t op, void *op_data, hid_t type_id,
              hid_t dst_space_id, void *dst_buf),
             (op, op_data, type_id, dst_space_id, dst_buf), brahma::HDF5);

GOTCHA_MACRO(H5Dgather, herr_t,
             (hid_t src_space_id, const void *src_buf, hid_t type_id,
              size_t dst_buf_size, void *dst_buf, H5D_gather_func_t op,
              void *op_data),
             (src_space_id, src_buf, type_id, dst_buf_size, dst_buf, op,
              op_data),
             brahma::HDF5);

GOTCHA_MACRO(H5Ddebug, herr_t, (hid_t dset_id), (dset_id), brahma::HDF5);

GOTCHA_MACRO(H5Dformat_convert, herr_t, (hid_t dset_id), (dset_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Dget_chunk_index_type, herr_t,
             (hid_t did, H5D_chunk_index_t *idx_type), (did, idx_type),
             brahma::HDF5);

GOTCHA_MACRO(H5Dcreate1, hid_t,
             (hid_t file_id, const char *name, hid_t type_id, hid_t space_id,
              hid_t dcpl_id),
             (file_id, name, type_id, space_id, dcpl_id), brahma::HDF5);

GOTCHA_MACRO(H5Dopen1, hid_t, (hid_t file_id, const char *name),
             (file_id, name), brahma::HDF5);

GOTCHA_MACRO(H5Dextend, herr_t, (hid_t dset_id, const hsize_t size[]),
             (dset_id, size), brahma::HDF5);

GOTCHA_MACRO(H5Eregister_class, hid_t,
             (const char *cls_name, const char *lib_name, const char *version),
             (cls_name, lib_name, version), brahma::HDF5);

GOTCHA_MACRO(H5Eunregister_class, herr_t, (hid_t class_id), (class_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Eclose_msg, herr_t, (hid_t err_id), (err_id), brahma::HDF5);

GOTCHA_MACRO(H5Ecreate_msg, hid_t,
             (hid_t cls, H5E_type_t msg_type, const char *msg),
             (cls, msg_type, msg), brahma::HDF5);

GOTCHA_MACRO(H5Ecreate_stack, hid_t, (), (), brahma::HDF5);

GOTCHA_MACRO(H5Eget_current_stack, hid_t, (), (), brahma::HDF5);

GOTCHA_MACRO(H5Eclose_stack, herr_t, (hid_t stack_id), (stack_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Eget_class_name, ssize_t,
             (hid_t class_id, char *name, size_t size), (class_id, name, size),
             brahma::HDF5);

GOTCHA_MACRO(H5Eset_current_stack, herr_t, (hid_t err_stack_id), (err_stack_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Epush2, herr_t,
             (hid_t err_stack, const char *file, const char *func,
              unsigned int line, hid_t cls_id, hid_t maj_id, hid_t min_id,
              const char *msg),
             (err_stack, file, func, line, cls_id, maj_id, min_id, msg),
             brahma::HDF5);

GOTCHA_MACRO(H5Epop, herr_t, (hid_t err_stack, size_t count),
             (err_stack, count), brahma::HDF5);

GOTCHA_MACRO(H5Eprint2, herr_t, (hid_t err_stack, FILE *stream),
             (err_stack, stream), brahma::HDF5);

GOTCHA_MACRO(H5Ewalk2, herr_t,
             (hid_t err_stack, H5E_direction_t direction, H5E_walk2_t func,
              void *client_data),
             (err_stack, direction, func, client_data), brahma::HDF5);

GOTCHA_MACRO(H5Eget_auto2, herr_t,
             (hid_t estack_id, H5E_auto2_t *func, void **client_data),
             (estack_id, func, client_data), brahma::HDF5);

GOTCHA_MACRO(H5Eset_auto2, herr_t,
             (hid_t estack_id, H5E_auto2_t func, void *client_data),
             (estack_id, func, client_data), brahma::HDF5);

GOTCHA_MACRO(H5Eclear2, herr_t, (hid_t err_stack), (err_stack), brahma::HDF5);

GOTCHA_MACRO(H5Eauto_is_v2, herr_t, (hid_t err_stack, unsigned int *is_stack),
             (err_stack, is_stack), brahma::HDF5);

GOTCHA_MACRO(H5Eget_msg, ssize_t,
             (hid_t msg_id, H5E_type_t *type, char *msg, size_t size),
             (msg_id, type, msg, size), brahma::HDF5);

GOTCHA_MACRO(H5Eget_num, ssize_t, (hid_t error_stack_id), (error_stack_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Eclear1, herr_t, (), (), brahma::HDF5);

GOTCHA_MACRO(H5Eget_auto1, herr_t, (H5E_auto1_t * func, void **client_data),
             (func, client_data), brahma::HDF5);

GOTCHA_MACRO(H5Epush1, herr_t,
             (const char *file, const char *func, unsigned int line,
              H5E_major_t maj, H5E_minor_t min, const char *str),
             (file, func, line, maj, min, str), brahma::HDF5);

GOTCHA_MACRO(H5Eprint1, herr_t, (FILE * stream), (stream), brahma::HDF5);

GOTCHA_MACRO(H5Eset_auto1, herr_t, (H5E_auto1_t func, void *client_data),
             (func, client_data), brahma::HDF5);

GOTCHA_MACRO(H5Ewalk1, herr_t,
             (H5E_direction_t direction, H5E_walk1_t func, void *client_data),
             (direction, func, client_data), brahma::HDF5);

GOTCHA_MACRO(H5Eget_major, char *, (H5E_major_t maj), (maj), brahma::HDF5);

GOTCHA_MACRO(H5Eget_minor, char *, (H5E_minor_t min), (min), brahma::HDF5);

GOTCHA_MACRO(H5Fis_hdf5, htri_t, (const char *filename), (filename),
             brahma::HDF5);

GOTCHA_MACRO(H5Fcreate, hid_t,
             (const char *filename, unsigned int flags, hid_t create_plist,
              hid_t access_plist),
             (filename, flags, create_plist, access_plist), brahma::HDF5);

GOTCHA_MACRO(H5Fopen, hid_t,
             (const char *filename, unsigned int flags, hid_t access_plist),
             (filename, flags, access_plist), brahma::HDF5);

GOTCHA_MACRO(H5Freopen, hid_t, (hid_t file_id), (file_id), brahma::HDF5);

GOTCHA_MACRO(H5Fflush, herr_t, (hid_t object_id, H5F_scope_t scope),
             (object_id, scope), brahma::HDF5);

GOTCHA_MACRO(H5Fclose, herr_t, (hid_t file_id), (file_id), brahma::HDF5);

GOTCHA_MACRO(H5Fget_create_plist, hid_t, (hid_t file_id), (file_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Fget_access_plist, hid_t, (hid_t file_id), (file_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Fget_intent, herr_t, (hid_t file_id, unsigned int *intent),
             (file_id, intent), brahma::HDF5);

GOTCHA_MACRO(H5Fget_obj_count, ssize_t, (hid_t file_id, unsigned int types),
             (file_id, types), brahma::HDF5);

GOTCHA_MACRO(H5Fget_obj_ids, ssize_t,
             (hid_t file_id, unsigned int types, size_t max_objs,
              hid_t *obj_id_list),
             (file_id, types, max_objs, obj_id_list), brahma::HDF5);

GOTCHA_MACRO(H5Fget_vfd_handle, herr_t,
             (hid_t file_id, hid_t fapl, void **file_handle),
             (file_id, fapl, file_handle), brahma::HDF5);

GOTCHA_MACRO(H5Fmount, herr_t,
             (hid_t loc, const char *name, hid_t child, hid_t plist),
             (loc, name, child, plist), brahma::HDF5);

GOTCHA_MACRO(H5Funmount, herr_t, (hid_t loc, const char *name), (loc, name),
             brahma::HDF5);

GOTCHA_MACRO(H5Fget_freespace, hssize_t, (hid_t file_id), (file_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Fget_filesize, herr_t, (hid_t file_id, hsize_t *size),
             (file_id, size), brahma::HDF5);

GOTCHA_MACRO(H5Fget_eoa, herr_t, (hid_t file_id, haddr_t *eoa), (file_id, eoa),
             brahma::HDF5);

GOTCHA_MACRO(H5Fincrement_filesize, herr_t, (hid_t file_id, hsize_t increment),
             (file_id, increment), brahma::HDF5);

GOTCHA_MACRO(H5Fget_file_image, ssize_t,
             (hid_t file_id, void *buf_ptr, size_t buf_len),
             (file_id, buf_ptr, buf_len), brahma::HDF5);

GOTCHA_MACRO(H5Fget_mdc_config, herr_t,
             (hid_t file_id, H5AC_cache_config_t *config_ptr),
             (file_id, config_ptr), brahma::HDF5);

GOTCHA_MACRO(H5Fset_mdc_config, herr_t,
             (hid_t file_id, H5AC_cache_config_t *config_ptr),
             (file_id, config_ptr), brahma::HDF5);

GOTCHA_MACRO(H5Fget_mdc_hit_rate, herr_t, (hid_t file_id, double *hit_rate_ptr),
             (file_id, hit_rate_ptr), brahma::HDF5);

GOTCHA_MACRO(H5Fget_mdc_size, herr_t,
             (hid_t file_id, size_t *max_size_ptr, size_t *min_clean_size_ptr,
              size_t *cur_size_ptr, int *cur_num_entries_ptr),
             (file_id, max_size_ptr, min_clean_size_ptr, cur_size_ptr,
              cur_num_entries_ptr),
             brahma::HDF5);

GOTCHA_MACRO(H5Freset_mdc_hit_rate_stats, herr_t, (hid_t file_id), (file_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Fget_name, ssize_t, (hid_t obj_id, char *name, size_t size),
             (obj_id, name, size), brahma::HDF5);

GOTCHA_MACRO(H5Fget_info2, herr_t, (hid_t obj_id, H5F_info2_t *finfo),
             (obj_id, finfo), brahma::HDF5);

GOTCHA_MACRO(H5Fget_metadata_read_retry_info, herr_t,
             (hid_t file_id, H5F_retry_info_t *info), (file_id, info),
             brahma::HDF5);

GOTCHA_MACRO(H5Fstart_swmr_write, herr_t, (hid_t file_id), (file_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Fget_free_sections, ssize_t,
             (hid_t file_id, H5F_mem_t type, size_t nsects,
              H5F_sect_info_t *sect_info),
             (file_id, type, nsects, sect_info), brahma::HDF5);

GOTCHA_MACRO(H5Fclear_elink_file_cache, herr_t, (hid_t file_id), (file_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Fset_libver_bounds, herr_t,
             (hid_t file_id, H5F_libver_t low, H5F_libver_t high),
             (file_id, low, high), brahma::HDF5);

GOTCHA_MACRO(H5Fstart_mdc_logging, herr_t, (hid_t file_id), (file_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Fstop_mdc_logging, herr_t, (hid_t file_id), (file_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Fget_mdc_logging_status, herr_t,
             (hid_t file_id, hbool_t *is_enabled,
              hbool_t *is_currently_logging),
             (file_id, is_enabled, is_currently_logging), brahma::HDF5);

GOTCHA_MACRO(H5Fformat_convert, herr_t, (hid_t fid), (fid), brahma::HDF5);

GOTCHA_MACRO(H5Freset_page_buffering_stats, herr_t, (hid_t file_id), (file_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Fget_page_buffering_stats, herr_t,
             (hid_t file_id, unsigned int accesses[2], unsigned int hits[2],
              unsigned int misses[2], unsigned int evictions[2],
              unsigned int bypasses[2]),
             (file_id, accesses, hits, misses, evictions, bypasses),
             brahma::HDF5);

GOTCHA_MACRO(H5Fget_mdc_image_info, herr_t,
             (hid_t file_id, haddr_t *image_addr, hsize_t *image_size),
             (file_id, image_addr, image_size), brahma::HDF5);

GOTCHA_MACRO(H5Fget_dset_no_attrs_hint, herr_t,
             (hid_t file_id, hbool_t *minimize), (file_id, minimize),
             brahma::HDF5);

GOTCHA_MACRO(H5Fset_dset_no_attrs_hint, herr_t,
             (hid_t file_id, hbool_t minimize), (file_id, minimize),
             brahma::HDF5);

GOTCHA_MACRO(H5Fget_info1, herr_t, (hid_t obj_id, H5F_info1_t *finfo),
             (obj_id, finfo), brahma::HDF5);

GOTCHA_MACRO(H5Fset_latest_format, herr_t,
             (hid_t file_id, hbool_t latest_format), (file_id, latest_format),
             brahma::HDF5);

GOTCHA_MACRO(H5FDregister, hid_t, (const H5FD_class_t *cls), (cls),
             brahma::HDF5);

GOTCHA_MACRO(H5FDunregister, herr_t, (hid_t driver_id), (driver_id),
             brahma::HDF5);

GOTCHA_MACRO(H5FDopen, H5FD_t *,
             (const char *name, unsigned int flags, hid_t fapl_id,
              haddr_t maxaddr),
             (name, flags, fapl_id, maxaddr), brahma::HDF5);

GOTCHA_MACRO(H5FDclose, herr_t, (H5FD_t * file), (file), brahma::HDF5);

GOTCHA_MACRO(H5FDcmp, int, (const H5FD_t *f1, const H5FD_t *f2), (f1, f2),
             brahma::HDF5);

GOTCHA_MACRO(H5FDquery, int, (const H5FD_t *f, unsigned long *flags),
             (f, flags), brahma::HDF5);

GOTCHA_MACRO(H5FDalloc, haddr_t,
             (H5FD_t * file, H5FD_mem_t type, hid_t dxpl_id, hsize_t size),
             (file, type, dxpl_id, size), brahma::HDF5);

GOTCHA_MACRO(H5FDfree, herr_t,
             (H5FD_t * file, H5FD_mem_t type, hid_t dxpl_id, haddr_t addr,
              hsize_t size),
             (file, type, dxpl_id, addr, size), brahma::HDF5);

GOTCHA_MACRO(H5FDget_eoa, haddr_t, (H5FD_t * file, H5FD_mem_t type),
             (file, type), brahma::HDF5);

GOTCHA_MACRO(H5FDset_eoa, herr_t, (H5FD_t * file, H5FD_mem_t type, haddr_t eoa),
             (file, type, eoa), brahma::HDF5);

GOTCHA_MACRO(H5FDget_eof, haddr_t, (H5FD_t * file, H5FD_mem_t type),
             (file, type), brahma::HDF5);

GOTCHA_MACRO(H5FDget_vfd_handle, herr_t,
             (H5FD_t * file, hid_t fapl, void **file_handle),
             (file, fapl, file_handle), brahma::HDF5);

GOTCHA_MACRO(H5FDread, herr_t,
             (H5FD_t * file, H5FD_mem_t type, hid_t dxpl_id, haddr_t addr,
              size_t size, void *buf),
             (file, type, dxpl_id, addr, size, buf), brahma::HDF5);

GOTCHA_MACRO(H5FDwrite, herr_t,
             (H5FD_t * file, H5FD_mem_t type, hid_t dxpl_id, haddr_t addr,
              size_t size, const void *buf),
             (file, type, dxpl_id, addr, size, buf), brahma::HDF5);

GOTCHA_MACRO(H5FDflush, herr_t, (H5FD_t * file, hid_t dxpl_id, hbool_t closing),
             (file, dxpl_id, closing), brahma::HDF5);

GOTCHA_MACRO(H5FDtruncate, herr_t,
             (H5FD_t * file, hid_t dxpl_id, hbool_t closing),
             (file, dxpl_id, closing), brahma::HDF5);

GOTCHA_MACRO(H5FDlock, herr_t, (H5FD_t * file, hbool_t rw), (file, rw),
             brahma::HDF5);

GOTCHA_MACRO(H5FDunlock, herr_t, (H5FD_t * file), (file), brahma::HDF5);

GOTCHA_MACRO(H5FDdriver_query, herr_t, (hid_t driver_id, unsigned long *flags),
             (driver_id, flags), brahma::HDF5);

GOTCHA_MACRO(H5Gcreate2, hid_t,
             (hid_t loc_id, const char *name, hid_t lcpl_id, hid_t gcpl_id,
              hid_t gapl_id),
             (loc_id, name, lcpl_id, gcpl_id, gapl_id), brahma::HDF5);

GOTCHA_MACRO(H5Gcreate_anon, hid_t,
             (hid_t loc_id, hid_t gcpl_id, hid_t gapl_id),
             (loc_id, gcpl_id, gapl_id), brahma::HDF5);

GOTCHA_MACRO(H5Gopen2, hid_t, (hid_t loc_id, const char *name, hid_t gapl_id),
             (loc_id, name, gapl_id), brahma::HDF5);

GOTCHA_MACRO(H5Gget_create_plist, hid_t, (hid_t group_id), (group_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Gget_info, herr_t, (hid_t loc_id, H5G_info_t *ginfo),
             (loc_id, ginfo), brahma::HDF5);

GOTCHA_MACRO(H5Gget_info_by_name, herr_t,
             (hid_t loc_id, const char *name, H5G_info_t *ginfo, hid_t lapl_id),
             (loc_id, name, ginfo, lapl_id), brahma::HDF5);

GOTCHA_MACRO(H5Gget_info_by_idx, herr_t,
             (hid_t loc_id, const char *group_name, H5_index_t idx_type,
              H5_iter_order_t order, hsize_t n, H5G_info_t *ginfo,
              hid_t lapl_id),
             (loc_id, group_name, idx_type, order, n, ginfo, lapl_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Gclose, herr_t, (hid_t group_id), (group_id), brahma::HDF5);

GOTCHA_MACRO(H5Gflush, herr_t, (hid_t group_id), (group_id), brahma::HDF5);

GOTCHA_MACRO(H5Grefresh, herr_t, (hid_t group_id), (group_id), brahma::HDF5);

GOTCHA_MACRO(H5Gcreate1, hid_t,
             (hid_t loc_id, const char *name, size_t size_hint),
             (loc_id, name, size_hint), brahma::HDF5);

GOTCHA_MACRO(H5Gopen1, hid_t, (hid_t loc_id, const char *name), (loc_id, name),
             brahma::HDF5);

GOTCHA_MACRO(H5Glink, herr_t,
             (hid_t cur_loc_id, H5L_type_t type, const char *cur_name,
              const char *new_name),
             (cur_loc_id, type, cur_name, new_name), brahma::HDF5);

GOTCHA_MACRO(H5Glink2, herr_t,
             (hid_t cur_loc_id, const char *cur_name, H5L_type_t type,
              hid_t new_loc_id, const char *new_name),
             (cur_loc_id, cur_name, type, new_loc_id, new_name), brahma::HDF5);

GOTCHA_MACRO(H5Gmove, herr_t,
             (hid_t src_loc_id, const char *src_name, const char *dst_name),
             (src_loc_id, src_name, dst_name), brahma::HDF5);

GOTCHA_MACRO(H5Gmove2, herr_t,
             (hid_t src_loc_id, const char *src_name, hid_t dst_loc_id,
              const char *dst_name),
             (src_loc_id, src_name, dst_loc_id, dst_name), brahma::HDF5);

GOTCHA_MACRO(H5Gunlink, herr_t, (hid_t loc_id, const char *name),
             (loc_id, name), brahma::HDF5);

GOTCHA_MACRO(H5Gget_linkval, herr_t,
             (hid_t loc_id, const char *name, size_t size, char *buf),
             (loc_id, name, size, buf), brahma::HDF5);

GOTCHA_MACRO(H5Gset_comment, herr_t,
             (hid_t loc_id, const char *name, const char *comment),
             (loc_id, name, comment), brahma::HDF5);

GOTCHA_MACRO(H5Gget_comment, int,
             (hid_t loc_id, const char *name, size_t bufsize, char *buf),
             (loc_id, name, bufsize, buf), brahma::HDF5);

GOTCHA_MACRO(H5Giterate, herr_t,
             (hid_t loc_id, const char *name, int *idx, H5G_iterate_t op,
              void *op_data),
             (loc_id, name, idx, op, op_data), brahma::HDF5);

GOTCHA_MACRO(H5Gget_num_objs, herr_t, (hid_t loc_id, hsize_t *num_objs),
             (loc_id, num_objs), brahma::HDF5);

GOTCHA_MACRO(H5Gget_objinfo, herr_t,
             (hid_t loc_id, const char *name, hbool_t follow_link,
              H5G_stat_t *statbuf),
             (loc_id, name, follow_link, statbuf), brahma::HDF5);

GOTCHA_MACRO(H5Gget_objname_by_idx, ssize_t,
             (hid_t loc_id, hsize_t idx, char *name, size_t size),
             (loc_id, idx, name, size), brahma::HDF5);

GOTCHA_MACRO(H5Gget_objtype_by_idx, H5G_obj_t, (hid_t loc_id, hsize_t idx),
             (loc_id, idx), brahma::HDF5);

GOTCHA_MACRO(H5Zregister, herr_t, (const void *cls), (cls), brahma::HDF5);

GOTCHA_MACRO(H5Zunregister, herr_t, (H5Z_filter_t id), (id), brahma::HDF5);

GOTCHA_MACRO(H5Zfilter_avail, htri_t, (H5Z_filter_t id), (id), brahma::HDF5);

GOTCHA_MACRO(H5Zget_filter_info, herr_t,
             (H5Z_filter_t filter, unsigned int *filter_config_flags),
             (filter, filter_config_flags), brahma::HDF5);

GOTCHA_MACRO(H5Pcreate_class, hid_t,
             (hid_t parent, const char *name, H5P_cls_create_func_t cls_create,
              void *create_data, H5P_cls_copy_func_t cls_copy, void *copy_data,
              H5P_cls_close_func_t cls_close, void *close_data),
             (parent, name, cls_create, create_data, cls_copy, copy_data,
              cls_close, close_data),
             brahma::HDF5);

GOTCHA_MACRO(H5Pget_class_name, char *, (hid_t pclass_id), (pclass_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Pcreate, hid_t, (hid_t cls_id), (cls_id), brahma::HDF5);

GOTCHA_MACRO(H5Pregister2, herr_t,
             (hid_t cls_id, const char *name, size_t size, void *def_value,
              H5P_prp_create_func_t prp_create, H5P_prp_set_func_t prp_set,
              H5P_prp_get_func_t prp_get, H5P_prp_delete_func_t prp_del,
              H5P_prp_copy_func_t prp_copy, H5P_prp_compare_func_t prp_cmp,
              H5P_prp_close_func_t prp_close),
             (cls_id, name, size, def_value, prp_create, prp_set, prp_get,
              prp_del, prp_copy, prp_cmp, prp_close),
             brahma::HDF5);

GOTCHA_MACRO(H5Pinsert2, herr_t,
             (hid_t plist_id, const char *name, size_t size, void *value,
              H5P_prp_set_func_t prp_set, H5P_prp_get_func_t prp_get,
              H5P_prp_delete_func_t prp_delete, H5P_prp_copy_func_t prp_copy,
              H5P_prp_compare_func_t prp_cmp, H5P_prp_close_func_t prp_close),
             (plist_id, name, size, value, prp_set, prp_get, prp_delete,
              prp_copy, prp_cmp, prp_close),
             brahma::HDF5);

GOTCHA_MACRO(H5Pset, herr_t,
             (hid_t plist_id, const char *name, const void *value),
             (plist_id, name, value), brahma::HDF5);

GOTCHA_MACRO(H5Pexist, htri_t, (hid_t plist_id, const char *name),
             (plist_id, name), brahma::HDF5);

GOTCHA_MACRO(H5Pencode, herr_t, (hid_t plist_id, void *buf, size_t *nalloc),
             (plist_id, buf, nalloc), brahma::HDF5);

GOTCHA_MACRO(H5Pdecode, hid_t, (const void *buf), (buf), brahma::HDF5);

GOTCHA_MACRO(H5Pget_size, herr_t, (hid_t id, const char *name, size_t *size),
             (id, name, size), brahma::HDF5);

GOTCHA_MACRO(H5Pget_nprops, herr_t, (hid_t id, size_t *nprops), (id, nprops),
             brahma::HDF5);

GOTCHA_MACRO(H5Pget_class, hid_t, (hid_t plist_id), (plist_id), brahma::HDF5);

GOTCHA_MACRO(H5Pget_class_parent, hid_t, (hid_t pclass_id), (pclass_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Pget, herr_t, (hid_t plist_id, const char *name, void *value),
             (plist_id, name, value), brahma::HDF5);

GOTCHA_MACRO(H5Pequal, htri_t, (hid_t id1, hid_t id2), (id1, id2),
             brahma::HDF5);

GOTCHA_MACRO(H5Pisa_class, htri_t, (hid_t plist_id, hid_t pclass_id),
             (plist_id, pclass_id), brahma::HDF5);

GOTCHA_MACRO(H5Piterate, int,
             (hid_t id, int *idx, H5P_iterate_t iter_func, void *iter_data),
             (id, idx, iter_func, iter_data), brahma::HDF5);

GOTCHA_MACRO(H5Pcopy_prop, herr_t,
             (hid_t dst_id, hid_t src_id, const char *name),
             (dst_id, src_id, name), brahma::HDF5);

GOTCHA_MACRO(H5Premove, herr_t, (hid_t plist_id, const char *name),
             (plist_id, name), brahma::HDF5);

GOTCHA_MACRO(H5Punregister, herr_t, (hid_t pclass_id, const char *name),
             (pclass_id, name), brahma::HDF5);

GOTCHA_MACRO(H5Pclose_class, herr_t, (hid_t plist_id), (plist_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Pclose, herr_t, (hid_t plist_id), (plist_id), brahma::HDF5);

GOTCHA_MACRO(H5Pcopy, hid_t, (hid_t plist_id), (plist_id), brahma::HDF5);

GOTCHA_MACRO(H5Pset_attr_phase_change, herr_t,
             (hid_t plist_id, unsigned int max_compact, unsigned int min_dense),
             (plist_id, max_compact, min_dense), brahma::HDF5);

GOTCHA_MACRO(H5Pget_attr_phase_change, herr_t,
             (hid_t plist_id, unsigned int *max_compact,
              unsigned int *min_dense),
             (plist_id, max_compact, min_dense), brahma::HDF5);

GOTCHA_MACRO(H5Pset_attr_creation_order, herr_t,
             (hid_t plist_id, unsigned int crt_order_flags),
             (plist_id, crt_order_flags), brahma::HDF5);

GOTCHA_MACRO(H5Pget_attr_creation_order, herr_t,
             (hid_t plist_id, unsigned int *crt_order_flags),
             (plist_id, crt_order_flags), brahma::HDF5);

GOTCHA_MACRO(H5Pset_obj_track_times, herr_t,
             (hid_t plist_id, hbool_t track_times), (plist_id, track_times),
             brahma::HDF5);

GOTCHA_MACRO(H5Pget_obj_track_times, herr_t,
             (hid_t plist_id, hbool_t *track_times), (plist_id, track_times),
             brahma::HDF5);

GOTCHA_MACRO(H5Pmodify_filter, herr_t,
             (hid_t plist_id, H5Z_filter_t filter, unsigned int flags,
              size_t cd_nelmts, const unsigned int cd_values[]),
             (plist_id, filter, flags, cd_nelmts, cd_values), brahma::HDF5);

GOTCHA_MACRO(H5Pset_filter, herr_t,
             (hid_t plist_id, H5Z_filter_t filter, unsigned int flags,
              size_t cd_nelmts, const unsigned int c_values[]),
             (plist_id, filter, flags, cd_nelmts, c_values), brahma::HDF5);

GOTCHA_MACRO(H5Pget_nfilters, int, (hid_t plist_id), (plist_id), brahma::HDF5);

GOTCHA_MACRO(H5Pget_filter2, H5Z_filter_t,
             (hid_t plist_id, unsigned int filter, unsigned int *flags,
              size_t *cd_nelmts, unsigned int cd_values[], size_t namelen,
              char name[], unsigned int *filter_config),
             (plist_id, filter, flags, cd_nelmts, cd_values, namelen, name,
              filter_config),
             brahma::HDF5);

GOTCHA_MACRO(H5Pget_filter_by_id2, herr_t,
             (hid_t plist_id, H5Z_filter_t id, unsigned int *flags,
              size_t *cd_nelmts, unsigned int cd_values[], size_t namelen,
              char name[], unsigned int *filter_config),
             (plist_id, id, flags, cd_nelmts, cd_values, namelen, name,
              filter_config),
             brahma::HDF5);

GOTCHA_MACRO(H5Pall_filters_avail, htri_t, (hid_t plist_id), (plist_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Premove_filter, herr_t, (hid_t plist_id, H5Z_filter_t filter),
             (plist_id, filter), brahma::HDF5);

GOTCHA_MACRO(H5Pset_deflate, herr_t, (hid_t plist_id, unsigned int aggression),
             (plist_id, aggression), brahma::HDF5);

GOTCHA_MACRO(H5Pset_fletcher32, herr_t, (hid_t plist_id), (plist_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Pset_userblock, herr_t, (hid_t plist_id, hsize_t size),
             (plist_id, size), brahma::HDF5);

GOTCHA_MACRO(H5Pget_userblock, herr_t, (hid_t plist_id, hsize_t *size),
             (plist_id, size), brahma::HDF5);

GOTCHA_MACRO(H5Pset_sizes, herr_t,
             (hid_t plist_id, size_t sizeof_addr, size_t sizeof_size),
             (plist_id, sizeof_addr, sizeof_size), brahma::HDF5);

GOTCHA_MACRO(H5Pget_sizes, herr_t,
             (hid_t plist_id, size_t *sizeof_addr, size_t *sizeof_size),
             (plist_id, sizeof_addr, sizeof_size), brahma::HDF5);

GOTCHA_MACRO(H5Pset_sym_k, herr_t,
             (hid_t plist_id, unsigned int ik, unsigned int lk),
             (plist_id, ik, lk), brahma::HDF5);

GOTCHA_MACRO(H5Pget_sym_k, herr_t,
             (hid_t plist_id, unsigned int *ik, unsigned int *lk),
             (plist_id, ik, lk), brahma::HDF5);

GOTCHA_MACRO(H5Pset_istore_k, herr_t, (hid_t plist_id, unsigned int ik),
             (plist_id, ik), brahma::HDF5);

GOTCHA_MACRO(H5Pget_istore_k, herr_t, (hid_t plist_id, unsigned int *ik),
             (plist_id, ik), brahma::HDF5);

GOTCHA_MACRO(H5Pset_shared_mesg_nindexes, herr_t,
             (hid_t plist_id, unsigned int nindexes), (plist_id, nindexes),
             brahma::HDF5);

GOTCHA_MACRO(H5Pget_shared_mesg_nindexes, herr_t,
             (hid_t plist_id, unsigned int *nindexes), (plist_id, nindexes),
             brahma::HDF5);

GOTCHA_MACRO(H5Pset_shared_mesg_index, herr_t,
             (hid_t plist_id, unsigned int index_num,
              unsigned int mesg_type_flags, unsigned int min_mesg_size),
             (plist_id, index_num, mesg_type_flags, min_mesg_size),
             brahma::HDF5);

GOTCHA_MACRO(H5Pget_shared_mesg_index, herr_t,
             (hid_t plist_id, unsigned int index_num,
              unsigned int *mesg_type_flags, unsigned int *min_mesg_size),
             (plist_id, index_num, mesg_type_flags, min_mesg_size),
             brahma::HDF5);

GOTCHA_MACRO(H5Pset_shared_mesg_phase_change, herr_t,
             (hid_t plist_id, unsigned int max_list, unsigned int min_btree),
             (plist_id, max_list, min_btree), brahma::HDF5);

GOTCHA_MACRO(H5Pget_shared_mesg_phase_change, herr_t,
             (hid_t plist_id, unsigned int *max_list, unsigned int *min_btree),
             (plist_id, max_list, min_btree), brahma::HDF5);

GOTCHA_MACRO(H5Pset_file_space_strategy, herr_t,
             (hid_t plist_id, H5F_fspace_strategy_t strategy, hbool_t persist,
              hsize_t threshold),
             (plist_id, strategy, persist, threshold), brahma::HDF5);

GOTCHA_MACRO(H5Pget_file_space_strategy, herr_t,
             (hid_t plist_id, H5F_fspace_strategy_t *strategy, hbool_t *persist,
              hsize_t *threshold),
             (plist_id, strategy, persist, threshold), brahma::HDF5);

GOTCHA_MACRO(H5Pset_file_space_page_size, herr_t,
             (hid_t plist_id, hsize_t fsp_size), (plist_id, fsp_size),
             brahma::HDF5);

GOTCHA_MACRO(H5Pget_file_space_page_size, herr_t,
             (hid_t plist_id, hsize_t *fsp_size), (plist_id, fsp_size),
             brahma::HDF5);

GOTCHA_MACRO(H5Pset_alignment, herr_t,
             (hid_t fapl_id, hsize_t threshold, hsize_t alignment),
             (fapl_id, threshold, alignment), brahma::HDF5);

GOTCHA_MACRO(H5Pget_alignment, herr_t,
             (hid_t fapl_id, hsize_t *threshold, hsize_t *alignment),
             (fapl_id, threshold, alignment), brahma::HDF5);

GOTCHA_MACRO(H5Pset_driver, herr_t,
             (hid_t plist_id, hid_t driver_id, const void *driver_info),
             (plist_id, driver_id, driver_info), brahma::HDF5);

GOTCHA_MACRO(H5Pget_driver, hid_t, (hid_t plist_id), (plist_id), brahma::HDF5);

GOTCHA_MACRO(H5Pget_driver_info, const void *, (hid_t plist_id), (plist_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Pset_family_offset, herr_t, (hid_t fapl_id, hsize_t offset),
             (fapl_id, offset), brahma::HDF5);

GOTCHA_MACRO(H5Pget_family_offset, herr_t, (hid_t fapl_id, hsize_t *offset),
             (fapl_id, offset), brahma::HDF5);

GOTCHA_MACRO(H5Pset_multi_type, herr_t, (hid_t fapl_id, H5FD_mem_t type),
             (fapl_id, type), brahma::HDF5);

GOTCHA_MACRO(H5Pget_multi_type, herr_t, (hid_t fapl_id, H5FD_mem_t *type),
             (fapl_id, type), brahma::HDF5);

GOTCHA_MACRO(H5Pset_cache, herr_t,
             (hid_t plist_id, int mdc_nelmts, size_t rdcc_nslots,
              size_t rdcc_nbytes, double rdcc_w0),
             (plist_id, mdc_nelmts, rdcc_nslots, rdcc_nbytes, rdcc_w0),
             brahma::HDF5);

GOTCHA_MACRO(H5Pget_cache, herr_t,
             (hid_t plist_id, int *mdc_nelmts, size_t *rdcc_nslots,
              size_t *rdcc_nbytes, double *rdcc_w0),
             (plist_id, mdc_nelmts, rdcc_nslots, rdcc_nbytes, rdcc_w0),
             brahma::HDF5);

GOTCHA_MACRO(H5Pset_mdc_config, herr_t,
             (hid_t plist_id, H5AC_cache_config_t *config_ptr),
             (plist_id, config_ptr), brahma::HDF5);

GOTCHA_MACRO(H5Pget_mdc_config, herr_t,
             (hid_t plist_id, H5AC_cache_config_t *config_ptr),
             (plist_id, config_ptr), brahma::HDF5);

GOTCHA_MACRO(H5Pset_gc_references, herr_t, (hid_t fapl_id, unsigned int gc_ref),
             (fapl_id, gc_ref), brahma::HDF5);

GOTCHA_MACRO(H5Pget_gc_references, herr_t,
             (hid_t fapl_id, unsigned int *gc_ref), (fapl_id, gc_ref),
             brahma::HDF5);

GOTCHA_MACRO(H5Pset_fclose_degree, herr_t,
             (hid_t fapl_id, H5F_close_degree_t degree), (fapl_id, degree),
             brahma::HDF5);

GOTCHA_MACRO(H5Pget_fclose_degree, herr_t,
             (hid_t fapl_id, H5F_close_degree_t *degree), (fapl_id, degree),
             brahma::HDF5);

GOTCHA_MACRO(H5Pset_meta_block_size, herr_t, (hid_t fapl_id, hsize_t size),
             (fapl_id, size), brahma::HDF5);

GOTCHA_MACRO(H5Pget_meta_block_size, herr_t, (hid_t fapl_id, hsize_t *size),
             (fapl_id, size), brahma::HDF5);

GOTCHA_MACRO(H5Pset_sieve_buf_size, herr_t, (hid_t fapl_id, size_t size),
             (fapl_id, size), brahma::HDF5);

GOTCHA_MACRO(H5Pget_sieve_buf_size, herr_t, (hid_t fapl_id, size_t *size),
             (fapl_id, size), brahma::HDF5);

GOTCHA_MACRO(H5Pset_small_data_block_size, herr_t,
             (hid_t fapl_id, hsize_t size), (fapl_id, size), brahma::HDF5);

GOTCHA_MACRO(H5Pget_small_data_block_size, herr_t,
             (hid_t fapl_id, hsize_t *size), (fapl_id, size), brahma::HDF5);

GOTCHA_MACRO(H5Pset_libver_bounds, herr_t,
             (hid_t plist_id, H5F_libver_t low, H5F_libver_t high),
             (plist_id, low, high), brahma::HDF5);

GOTCHA_MACRO(H5Pget_libver_bounds, herr_t,
             (hid_t plist_id, H5F_libver_t *low, H5F_libver_t *high),
             (plist_id, low, high), brahma::HDF5);

GOTCHA_MACRO(H5Pset_elink_file_cache_size, herr_t,
             (hid_t plist_id, unsigned int efc_size), (plist_id, efc_size),
             brahma::HDF5);

GOTCHA_MACRO(H5Pget_elink_file_cache_size, herr_t,
             (hid_t plist_id, unsigned int *efc_size), (plist_id, efc_size),
             brahma::HDF5);

GOTCHA_MACRO(H5Pset_file_image, herr_t,
             (hid_t fapl_id, void *buf_ptr, size_t buf_len),
             (fapl_id, buf_ptr, buf_len), brahma::HDF5);

GOTCHA_MACRO(H5Pget_file_image, herr_t,
             (hid_t fapl_id, void **buf_ptr_ptr, size_t *buf_len_ptr),
             (fapl_id, buf_ptr_ptr, buf_len_ptr), brahma::HDF5);

GOTCHA_MACRO(H5Pset_file_image_callbacks, herr_t,
             (hid_t fapl_id, H5FD_file_image_callbacks_t *callbacks_ptr),
             (fapl_id, callbacks_ptr), brahma::HDF5);

GOTCHA_MACRO(H5Pget_file_image_callbacks, herr_t,
             (hid_t fapl_id, H5FD_file_image_callbacks_t *callbacks_ptr),
             (fapl_id, callbacks_ptr), brahma::HDF5);

GOTCHA_MACRO(H5Pset_core_write_tracking, herr_t,
             (hid_t fapl_id, hbool_t is_enabled, size_t page_size),
             (fapl_id, is_enabled, page_size), brahma::HDF5);

GOTCHA_MACRO(H5Pget_core_write_tracking, herr_t,
             (hid_t fapl_id, hbool_t *is_enabled, size_t *page_size),
             (fapl_id, is_enabled, page_size), brahma::HDF5);

GOTCHA_MACRO(H5Pset_metadata_read_attempts, herr_t,
             (hid_t plist_id, unsigned int attempts), (plist_id, attempts),
             brahma::HDF5);

GOTCHA_MACRO(H5Pget_metadata_read_attempts, herr_t,
             (hid_t plist_id, unsigned int *attempts), (plist_id, attempts),
             brahma::HDF5);

GOTCHA_MACRO(H5Pset_object_flush_cb, herr_t,
             (hid_t plist_id, H5F_flush_cb_t func, void *udata),
             (plist_id, func, udata), brahma::HDF5);

GOTCHA_MACRO(H5Pget_object_flush_cb, herr_t,
             (hid_t plist_id, H5F_flush_cb_t *func, void **udata),
             (plist_id, func, udata), brahma::HDF5);

GOTCHA_MACRO(H5Pset_mdc_log_options, herr_t,
             (hid_t plist_id, hbool_t is_enabled, const char *location,
              hbool_t start_on_access),
             (plist_id, is_enabled, location, start_on_access), brahma::HDF5);

GOTCHA_MACRO(H5Pget_mdc_log_options, herr_t,
             (hid_t plist_id, hbool_t *is_enabled, char *location,
              size_t *location_size, hbool_t *start_on_access),
             (plist_id, is_enabled, location, location_size, start_on_access),
             brahma::HDF5);

GOTCHA_MACRO(H5Pset_evict_on_close, herr_t,
             (hid_t fapl_id, hbool_t evict_on_close), (fapl_id, evict_on_close),
             brahma::HDF5);

GOTCHA_MACRO(H5Pget_evict_on_close, herr_t,
             (hid_t fapl_id, hbool_t *evict_on_close),
             (fapl_id, evict_on_close), brahma::HDF5);

GOTCHA_MACRO(H5Pset_file_locking, herr_t,
             (hid_t fapl_id, hbool_t use_file_locking,
              hbool_t ignore_when_disabled),
             (fapl_id, use_file_locking, ignore_when_disabled), brahma::HDF5);

GOTCHA_MACRO(H5Pget_file_locking, herr_t,
             (hid_t fapl_id, hbool_t *use_file_locking,
              hbool_t *ignore_when_disabled),
             (fapl_id, use_file_locking, ignore_when_disabled), brahma::HDF5);

GOTCHA_MACRO(H5Pset_mdc_image_config, herr_t,
             (hid_t plist_id, H5AC_cache_image_config_t *config_ptr),
             (plist_id, config_ptr), brahma::HDF5);

GOTCHA_MACRO(H5Pget_mdc_image_config, herr_t,
             (hid_t plist_id, H5AC_cache_image_config_t *config_ptr),
             (plist_id, config_ptr), brahma::HDF5);

GOTCHA_MACRO(H5Pset_page_buffer_size, herr_t,
             (hid_t plist_id, size_t buf_size, unsigned int min_meta_per,
              unsigned int min_raw_per),
             (plist_id, buf_size, min_meta_per, min_raw_per), brahma::HDF5);

GOTCHA_MACRO(H5Pget_page_buffer_size, herr_t,
             (hid_t plist_id, size_t *buf_size, unsigned int *min_meta_per,
              unsigned int *min_raw_per),
             (plist_id, buf_size, min_meta_per, min_raw_per), brahma::HDF5);

GOTCHA_MACRO(H5Pset_layout, herr_t, (hid_t plist_id, H5D_layout_t layout),
             (plist_id, layout), brahma::HDF5);

GOTCHA_MACRO(H5Pget_layout, H5D_layout_t, (hid_t plist_id), (plist_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Pset_chunk, herr_t,
             (hid_t plist_id, int ndims, const hsize_t dim[]),
             (plist_id, ndims, dim), brahma::HDF5);

GOTCHA_MACRO(H5Pget_chunk, int, (hid_t plist_id, int max_ndims, hsize_t dim[]),
             (plist_id, max_ndims, dim), brahma::HDF5);

GOTCHA_MACRO(H5Pset_virtual, herr_t,
             (hid_t dcpl_id, hid_t vspace_id, const char *src_file_name,
              const char *src_dset_name, hid_t src_space_id),
             (dcpl_id, vspace_id, src_file_name, src_dset_name, src_space_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Pget_virtual_count, herr_t, (hid_t dcpl_id, size_t *count),
             (dcpl_id, count), brahma::HDF5);

GOTCHA_MACRO(H5Pget_virtual_vspace, hid_t, (hid_t dcpl_id, size_t index),
             (dcpl_id, index), brahma::HDF5);

GOTCHA_MACRO(H5Pget_virtual_srcspace, hid_t, (hid_t dcpl_id, size_t index),
             (dcpl_id, index), brahma::HDF5);

GOTCHA_MACRO(H5Pget_virtual_filename, ssize_t,
             (hid_t dcpl_id, size_t index, char *name, size_t size),
             (dcpl_id, index, name, size), brahma::HDF5);

GOTCHA_MACRO(H5Pget_virtual_dsetname, ssize_t,
             (hid_t dcpl_id, size_t index, char *name, size_t size),
             (dcpl_id, index, name, size), brahma::HDF5);

GOTCHA_MACRO(H5Pset_external, herr_t,
             (hid_t plist_id, const char *name, off_t offset, hsize_t size),
             (plist_id, name, offset, size), brahma::HDF5);

GOTCHA_MACRO(H5Pset_chunk_opts, herr_t, (hid_t plist_id, unsigned int opts),
             (plist_id, opts), brahma::HDF5);

GOTCHA_MACRO(H5Pget_chunk_opts, herr_t, (hid_t plist_id, unsigned int *opts),
             (plist_id, opts), brahma::HDF5);

GOTCHA_MACRO(H5Pget_external_count, int, (hid_t plist_id), (plist_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Pget_external, herr_t,
             (hid_t plist_id, unsigned int idx, size_t name_size, char *name,
              off_t *offset, hsize_t *size),
             (plist_id, idx, name_size, name, offset, size), brahma::HDF5);

GOTCHA_MACRO(H5Pset_szip, herr_t,
             (hid_t plist_id, unsigned int options_mask,
              unsigned int pixels_per_block),
             (plist_id, options_mask, pixels_per_block), brahma::HDF5);

GOTCHA_MACRO(H5Pset_shuffle, herr_t, (hid_t plist_id), (plist_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Pset_nbit, herr_t, (hid_t plist_id), (plist_id), brahma::HDF5);

GOTCHA_MACRO(H5Pset_scaleoffset, herr_t,
             (hid_t plist_id, H5Z_SO_scale_type_t scale_type, int scale_factor),
             (plist_id, scale_type, scale_factor), brahma::HDF5);

GOTCHA_MACRO(H5Pset_fill_value, herr_t,
             (hid_t plist_id, hid_t type_id, const void *value),
             (plist_id, type_id, value), brahma::HDF5);

GOTCHA_MACRO(H5Pget_fill_value, herr_t,
             (hid_t plist_id, hid_t type_id, void *value),
             (plist_id, type_id, value), brahma::HDF5);

GOTCHA_MACRO(H5Pfill_value_defined, herr_t,
             (hid_t plist, H5D_fill_value_t *status), (plist, status),
             brahma::HDF5);

GOTCHA_MACRO(H5Pset_alloc_time, herr_t,
             (hid_t plist_id, H5D_alloc_time_t alloc_time),
             (plist_id, alloc_time), brahma::HDF5);

GOTCHA_MACRO(H5Pget_alloc_time, herr_t,
             (hid_t plist_id, H5D_alloc_time_t *alloc_time),
             (plist_id, alloc_time), brahma::HDF5);

GOTCHA_MACRO(H5Pset_fill_time, herr_t,
             (hid_t plist_id, H5D_fill_time_t fill_time), (plist_id, fill_time),
             brahma::HDF5);

GOTCHA_MACRO(H5Pget_fill_time, herr_t,
             (hid_t plist_id, H5D_fill_time_t *fill_time),
             (plist_id, fill_time), brahma::HDF5);

GOTCHA_MACRO(H5Pget_dset_no_attrs_hint, herr_t,
             (hid_t dcpl_id, hbool_t *minimize), (dcpl_id, minimize),
             brahma::HDF5);

GOTCHA_MACRO(H5Pset_dset_no_attrs_hint, herr_t,
             (hid_t dcpl_id, hbool_t minimize), (dcpl_id, minimize),
             brahma::HDF5);

GOTCHA_MACRO(H5Pset_chunk_cache, herr_t,
             (hid_t dapl_id, size_t rdcc_nslots, size_t rdcc_nbytes,
              double rdcc_w0),
             (dapl_id, rdcc_nslots, rdcc_nbytes, rdcc_w0), brahma::HDF5);

GOTCHA_MACRO(H5Pget_chunk_cache, herr_t,
             (hid_t dapl_id, size_t *rdcc_nslots, size_t *rdcc_nbytes,
              double *rdcc_w0),
             (dapl_id, rdcc_nslots, rdcc_nbytes, rdcc_w0), brahma::HDF5);

GOTCHA_MACRO(H5Pset_virtual_view, herr_t, (hid_t plist_id, H5D_vds_view_t view),
             (plist_id, view), brahma::HDF5);

GOTCHA_MACRO(H5Pget_virtual_view, herr_t,
             (hid_t plist_id, H5D_vds_view_t *view), (plist_id, view),
             brahma::HDF5);

GOTCHA_MACRO(H5Pset_virtual_printf_gap, herr_t,
             (hid_t plist_id, hsize_t gap_size), (plist_id, gap_size),
             brahma::HDF5);

GOTCHA_MACRO(H5Pget_virtual_printf_gap, herr_t,
             (hid_t plist_id, hsize_t *gap_size), (plist_id, gap_size),
             brahma::HDF5);

GOTCHA_MACRO(H5Pset_virtual_prefix, herr_t, (hid_t dapl_id, const char *prefix),
             (dapl_id, prefix), brahma::HDF5);

GOTCHA_MACRO(H5Pget_virtual_prefix, ssize_t,
             (hid_t dapl_id, char *prefix, size_t size),
             (dapl_id, prefix, size), brahma::HDF5);

GOTCHA_MACRO(H5Pset_append_flush, herr_t,
             (hid_t plist_id, unsigned int ndims, const hsize_t boundary[],
              H5D_append_cb_t func, void *udata),
             (plist_id, ndims, boundary, func, udata), brahma::HDF5);

GOTCHA_MACRO(H5Pget_append_flush, herr_t,
             (hid_t plist_id, unsigned int dims, hsize_t boundary[],
              H5D_append_cb_t *func, void **udata),
             (plist_id, dims, boundary, func, udata), brahma::HDF5);

GOTCHA_MACRO(H5Pset_efile_prefix, herr_t, (hid_t dapl_id, const char *prefix),
             (dapl_id, prefix), brahma::HDF5);

GOTCHA_MACRO(H5Pget_efile_prefix, ssize_t,
             (hid_t dapl_id, char *prefix, size_t size),
             (dapl_id, prefix, size), brahma::HDF5);

GOTCHA_MACRO(H5Pset_data_transform, herr_t,
             (hid_t plist_id, const char *expression), (plist_id, expression),
             brahma::HDF5);

GOTCHA_MACRO(H5Pget_data_transform, ssize_t,
             (hid_t plist_id, char *expression, size_t size),
             (plist_id, expression, size), brahma::HDF5);

GOTCHA_MACRO(H5Pset_buffer, herr_t,
             (hid_t plist_id, size_t size, void *tconv, void *bkg),
             (plist_id, size, tconv, bkg), brahma::HDF5);

GOTCHA_MACRO(H5Pget_buffer, size_t, (hid_t plist_id, void **tconv, void **bkg),
             (plist_id, tconv, bkg), brahma::HDF5);

GOTCHA_MACRO(H5Pset_preserve, herr_t, (hid_t plist_id, hbool_t status),
             (plist_id, status), brahma::HDF5);

GOTCHA_MACRO(H5Pget_preserve, int, (hid_t plist_id), (plist_id), brahma::HDF5);

GOTCHA_MACRO(H5Pset_edc_check, herr_t, (hid_t plist_id, H5Z_EDC_t check),
             (plist_id, check), brahma::HDF5);

GOTCHA_MACRO(H5Pget_edc_check, H5Z_EDC_t, (hid_t plist_id), (plist_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Pset_filter_callback, herr_t,
             (hid_t plist_id, H5Z_filter_func_t func, void *op_data),
             (plist_id, func, op_data), brahma::HDF5);

GOTCHA_MACRO(H5Pset_btree_ratios, herr_t,
             (hid_t plist_id, double left, double middle, double right),
             (plist_id, left, middle, right), brahma::HDF5);

GOTCHA_MACRO(H5Pget_btree_ratios, herr_t,
             (hid_t plist_id, double *left, double *middle, double *right),
             (plist_id, left, middle, right), brahma::HDF5);

GOTCHA_MACRO(H5Pset_vlen_mem_manager, herr_t,
             (hid_t plist_id, H5MM_allocate_t alloc_func, void *alloc_info,
              H5MM_free_t free_func, void *free_info),
             (plist_id, alloc_func, alloc_info, free_func, free_info),
             brahma::HDF5);

GOTCHA_MACRO(H5Pget_vlen_mem_manager, herr_t,
             (hid_t plist_id, H5MM_allocate_t *alloc_func, void **alloc_info,
              H5MM_free_t *free_func, void **free_info),
             (plist_id, alloc_func, alloc_info, free_func, free_info),
             brahma::HDF5);

GOTCHA_MACRO(H5Pset_hyper_vector_size, herr_t, (hid_t fapl_id, size_t size),
             (fapl_id, size), brahma::HDF5);

GOTCHA_MACRO(H5Pget_hyper_vector_size, herr_t, (hid_t fapl_id, size_t *size),
             (fapl_id, size), brahma::HDF5);

GOTCHA_MACRO(H5Pset_type_conv_cb, herr_t,
             (hid_t dxpl_id, H5T_conv_except_func_t op, void *operate_data),
             (dxpl_id, op, operate_data), brahma::HDF5);

GOTCHA_MACRO(H5Pget_type_conv_cb, herr_t,
             (hid_t dxpl_id, H5T_conv_except_func_t *op, void **operate_data),
             (dxpl_id, op, operate_data), brahma::HDF5);

GOTCHA_MACRO(H5Pset_create_intermediate_group, herr_t,
             (hid_t plist_id, unsigned int crt_intmd), (plist_id, crt_intmd),
             brahma::HDF5);

GOTCHA_MACRO(H5Pget_create_intermediate_group, herr_t,
             (hid_t plist_id, unsigned int *crt_intmd), (plist_id, crt_intmd),
             brahma::HDF5);

GOTCHA_MACRO(H5Pset_local_heap_size_hint, herr_t,
             (hid_t plist_id, size_t size_hint), (plist_id, size_hint),
             brahma::HDF5);

GOTCHA_MACRO(H5Pget_local_heap_size_hint, herr_t,
             (hid_t plist_id, size_t *size_hint), (plist_id, size_hint),
             brahma::HDF5);

GOTCHA_MACRO(H5Pset_link_phase_change, herr_t,
             (hid_t plist_id, unsigned int max_compact, unsigned int min_dense),
             (plist_id, max_compact, min_dense), brahma::HDF5);

GOTCHA_MACRO(H5Pget_link_phase_change, herr_t,
             (hid_t plist_id, unsigned int *max_compact,
              unsigned int *min_dense),
             (plist_id, max_compact, min_dense), brahma::HDF5);

GOTCHA_MACRO(H5Pset_est_link_info, herr_t,
             (hid_t plist_id, unsigned int est_num_entries,
              unsigned int est_name_len),
             (plist_id, est_num_entries, est_name_len), brahma::HDF5);

GOTCHA_MACRO(H5Pget_est_link_info, herr_t,
             (hid_t plist_id, unsigned int *est_num_entries,
              unsigned int *est_name_len),
             (plist_id, est_num_entries, est_name_len), brahma::HDF5);

GOTCHA_MACRO(H5Pset_link_creation_order, herr_t,
             (hid_t plist_id, unsigned int crt_order_flags),
             (plist_id, crt_order_flags), brahma::HDF5);

GOTCHA_MACRO(H5Pget_link_creation_order, herr_t,
             (hid_t plist_id, unsigned int *crt_order_flags),
             (plist_id, crt_order_flags), brahma::HDF5);

GOTCHA_MACRO(H5Pset_char_encoding, herr_t,
             (hid_t plist_id, H5T_cset_t encoding), (plist_id, encoding),
             brahma::HDF5);

GOTCHA_MACRO(H5Pget_char_encoding, herr_t,
             (hid_t plist_id, H5T_cset_t *encoding), (plist_id, encoding),
             brahma::HDF5);

GOTCHA_MACRO(H5Pset_nlinks, herr_t, (hid_t plist_id, size_t nlinks),
             (plist_id, nlinks), brahma::HDF5);

GOTCHA_MACRO(H5Pget_nlinks, herr_t, (hid_t plist_id, size_t *nlinks),
             (plist_id, nlinks), brahma::HDF5);

GOTCHA_MACRO(H5Pset_elink_prefix, herr_t, (hid_t plist_id, const char *prefix),
             (plist_id, prefix), brahma::HDF5);

GOTCHA_MACRO(H5Pget_elink_prefix, ssize_t,
             (hid_t plist_id, char *prefix, size_t size),
             (plist_id, prefix, size), brahma::HDF5);

GOTCHA_MACRO(H5Pget_elink_fapl, hid_t, (hid_t lapl_id), (lapl_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Pset_elink_fapl, herr_t, (hid_t lapl_id, hid_t fapl_id),
             (lapl_id, fapl_id), brahma::HDF5);

GOTCHA_MACRO(H5Pset_elink_acc_flags, herr_t,
             (hid_t lapl_id, unsigned int flags), (lapl_id, flags),
             brahma::HDF5);

GOTCHA_MACRO(H5Pget_elink_acc_flags, herr_t,
             (hid_t lapl_id, unsigned int *flags), (lapl_id, flags),
             brahma::HDF5);

GOTCHA_MACRO(H5Pset_elink_cb, herr_t,
             (hid_t lapl_id, H5L_elink_traverse_t func, void *op_data),
             (lapl_id, func, op_data), brahma::HDF5);

GOTCHA_MACRO(H5Pget_elink_cb, herr_t,
             (hid_t lapl_id, H5L_elink_traverse_t *func, void **op_data),
             (lapl_id, func, op_data), brahma::HDF5);

GOTCHA_MACRO(H5Pset_copy_object, herr_t,
             (hid_t plist_id, unsigned int crt_intmd), (plist_id, crt_intmd),
             brahma::HDF5);

GOTCHA_MACRO(H5Pget_copy_object, herr_t,
             (hid_t plist_id, unsigned int *crt_intmd), (plist_id, crt_intmd),
             brahma::HDF5);

GOTCHA_MACRO(H5Padd_merge_committed_dtype_path, herr_t,
             (hid_t plist_id, const char *path), (plist_id, path),
             brahma::HDF5);

GOTCHA_MACRO(H5Pfree_merge_committed_dtype_paths, herr_t, (hid_t plist_id),
             (plist_id), brahma::HDF5);

GOTCHA_MACRO(H5Pset_mcdt_search_cb, herr_t,
             (hid_t plist_id, H5O_mcdt_search_cb_t func, void *op_data),
             (plist_id, func, op_data), brahma::HDF5);

GOTCHA_MACRO(H5Pget_mcdt_search_cb, herr_t,
             (hid_t plist_id, H5O_mcdt_search_cb_t *func, void **op_data),
             (plist_id, func, op_data), brahma::HDF5);

GOTCHA_MACRO(H5Pregister1, herr_t,
             (hid_t cls_id, const char *name, size_t size, void *def_value,
              H5P_prp_create_func_t prp_create, H5P_prp_set_func_t prp_set,
              H5P_prp_get_func_t prp_get, H5P_prp_delete_func_t prp_del,
              H5P_prp_copy_func_t prp_copy, H5P_prp_close_func_t prp_close),
             (cls_id, name, size, def_value, prp_create, prp_set, prp_get,
              prp_del, prp_copy, prp_close),
             brahma::HDF5);

GOTCHA_MACRO(H5Pinsert1, herr_t,
             (hid_t plist_id, const char *name, size_t size, void *value,
              H5P_prp_set_func_t prp_set, H5P_prp_get_func_t prp_get,
              H5P_prp_delete_func_t prp_delete, H5P_prp_copy_func_t prp_copy,
              H5P_prp_close_func_t prp_close),
             (plist_id, name, size, value, prp_set, prp_get, prp_delete,
              prp_copy, prp_close),
             brahma::HDF5);

GOTCHA_MACRO(H5Pget_filter1, H5Z_filter_t,
             (hid_t plist_id, unsigned int filter, unsigned int *flags,
              size_t *cd_nelmts, unsigned int cd_values[], size_t namelen,
              char name[]),
             (plist_id, filter, flags, cd_nelmts, cd_values, namelen, name),
             brahma::HDF5);

GOTCHA_MACRO(H5Pget_filter_by_id1, herr_t,
             (hid_t plist_id, H5Z_filter_t id, unsigned int *flags,
              size_t *cd_nelmts, unsigned int cd_values[], size_t namelen,
              char name[]),
             (plist_id, id, flags, cd_nelmts, cd_values, namelen, name),
             brahma::HDF5);

GOTCHA_MACRO(H5Pget_version, herr_t,
             (hid_t plist_id, unsigned int *boot, unsigned int *freelist,
              unsigned int *stab, unsigned int *shhdr),
             (plist_id, boot, freelist, stab, shhdr), brahma::HDF5);

GOTCHA_MACRO(H5Pset_file_space, herr_t,
             (hid_t plist_id, H5F_file_space_type_t strategy,
              hsize_t threshold),
             (plist_id, strategy, threshold), brahma::HDF5);

GOTCHA_MACRO(H5Pget_file_space, herr_t,
             (hid_t plist_id, H5F_file_space_type_t *strategy,
              hsize_t *threshold),
             (plist_id, strategy, threshold), brahma::HDF5);

GOTCHA_MACRO(H5PLset_loading_state, herr_t, (unsigned int plugin_control_mask),
             (plugin_control_mask), brahma::HDF5);

GOTCHA_MACRO(H5PLget_loading_state, herr_t, (unsigned int *plugin_control_mask),
             (plugin_control_mask), brahma::HDF5);

GOTCHA_MACRO(H5PLappend, herr_t, (const char *search_path), (search_path),
             brahma::HDF5);

GOTCHA_MACRO(H5PLprepend, herr_t, (const char *search_path), (search_path),
             brahma::HDF5);

GOTCHA_MACRO(H5PLreplace, herr_t, (const char *search_path, unsigned int index),
             (search_path, index), brahma::HDF5);

GOTCHA_MACRO(H5PLinsert, herr_t, (const char *search_path, unsigned int index),
             (search_path, index), brahma::HDF5);

GOTCHA_MACRO(H5PLremove, herr_t, (unsigned int index), (index), brahma::HDF5);

GOTCHA_MACRO(H5PLget, ssize_t,
             (unsigned int index, char *path_buf, size_t buf_size),
             (index, path_buf, buf_size), brahma::HDF5);

GOTCHA_MACRO(H5PLsize, herr_t, (unsigned int *num_paths), (num_paths),
             brahma::HDF5);

GOTCHA_MACRO(H5Rcreate, herr_t,
             (void *ref, hid_t loc_id, const char *name, H5R_type_t ref_type,
              hid_t space_id),
             (ref, loc_id, name, ref_type, space_id), brahma::HDF5);

GOTCHA_MACRO(H5Rdereference2, hid_t,
             (hid_t obj_id, hid_t oapl_id, H5R_type_t ref_type,
              const void *ref),
             (obj_id, oapl_id, ref_type, ref), brahma::HDF5);

GOTCHA_MACRO(H5Rget_region, hid_t,
             (hid_t dataset, H5R_type_t ref_type, const void *ref),
             (dataset, ref_type, ref), brahma::HDF5);

GOTCHA_MACRO(H5Rget_obj_type2, herr_t,
             (hid_t id, H5R_type_t ref_type, const void *_ref,
              H5O_type_t *obj_type),
             (id, ref_type, _ref, obj_type), brahma::HDF5);

GOTCHA_MACRO(H5Rget_name, ssize_t,
             (hid_t loc_id, H5R_type_t ref_type, const void *ref, char *name,
              size_t size),
             (loc_id, ref_type, ref, name, size), brahma::HDF5);

GOTCHA_MACRO(H5Rget_obj_type1, H5G_obj_t,
             (hid_t id, H5R_type_t ref_type, const void *_ref),
             (id, ref_type, _ref), brahma::HDF5);

GOTCHA_MACRO(H5Rdereference1, hid_t,
             (hid_t obj_id, H5R_type_t ref_type, const void *ref),
             (obj_id, ref_type, ref), brahma::HDF5);

GOTCHA_MACRO(H5Screate, hid_t, (H5S_class_t type), (type), brahma::HDF5);

GOTCHA_MACRO(H5Screate_simple, hid_t,
             (int rank, const hsize_t dims[], const hsize_t maxdims[]),
             (rank, dims, maxdims), brahma::HDF5);

GOTCHA_MACRO(H5Sset_extent_simple, herr_t,
             (hid_t space_id, int rank, const hsize_t dims[],
              const hsize_t max[]),
             (space_id, rank, dims, max), brahma::HDF5);

GOTCHA_MACRO(H5Scopy, hid_t, (hid_t space_id), (space_id), brahma::HDF5);

GOTCHA_MACRO(H5Sclose, herr_t, (hid_t space_id), (space_id), brahma::HDF5);

GOTCHA_MACRO(H5Sencode, herr_t, (hid_t obj_id, void *buf, size_t *nalloc),
             (obj_id, buf, nalloc), brahma::HDF5);

GOTCHA_MACRO(H5Sdecode, hid_t, (const void *buf), (buf), brahma::HDF5);

GOTCHA_MACRO(H5Sget_simple_extent_npoints, hssize_t, (hid_t space_id),
             (space_id), brahma::HDF5);

GOTCHA_MACRO(H5Sget_simple_extent_ndims, int, (hid_t space_id), (space_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Sget_simple_extent_dims, int,
             (hid_t space_id, hsize_t dims[], hsize_t maxdims[]),
             (space_id, dims, maxdims), brahma::HDF5);

GOTCHA_MACRO(H5Sis_simple, htri_t, (hid_t space_id), (space_id), brahma::HDF5);

GOTCHA_MACRO(H5Sget_simple_extent_type, H5S_class_t, (hid_t space_id),
             (space_id), brahma::HDF5);

GOTCHA_MACRO(H5Sset_extent_none, herr_t, (hid_t space_id), (space_id),
             brahma::HDF5);

GOTCHA_MACRO(H5Sextent_copy, herr_t, (hid_t dst_id, hid_t src_id),
             (dst_id, src_id), brahma::HDF5);

GOTCHA_MACRO(H5Sextent_equal, htri_t, (hid_t sid1, hid_t sid2), (sid1, sid2),
             brahma::HDF5);

GOTCHA_MACRO(H5Sget_select_type, H5S_sel_type, (hid_t spaceid), (spaceid),
             brahma::HDF5);

GOTCHA_MACRO(H5Sget_select_npoints, hssize_t, (hid_t spaceid), (spaceid),
             brahma::HDF5);

GOTCHA_MACRO(H5Sselect_copy, herr_t, (hid_t dst_id, hid_t src_id),
             (dst_id, src_id), brahma::HDF5);

GOTCHA_MACRO(H5Sselect_valid, htri_t, (hid_t spaceid), (spaceid), brahma::HDF5);

GOTCHA_MACRO(H5Sselect_adjust, herr_t, (hid_t spaceid, const hssize_t *offset),
             (spaceid, offset), brahma::HDF5);

GOTCHA_MACRO(H5Sget_select_bounds, herr_t,
             (hid_t spaceid, hsize_t start[], hsize_t end[]),
             (spaceid, start, end), brahma::HDF5);

GOTCHA_MACRO(H5Sselect_shape_same, htri_t, (hid_t space1_id, hid_t space2_id),
             (space1_id, space2_id), brahma::HDF5);

GOTCHA_MACRO(H5Sselect_intersect_block, htri_t,
             (hid_t space_id, const hsize_t *start, const hsize_t *end),
             (space_id, start, end), brahma::HDF5);

GOTCHA_MACRO(H5Soffset_simple, herr_t, (hid_t space_id, const hssize_t *offset),
             (space_id, offset), brahma::HDF5);

GOTCHA_MACRO(H5Sselect_all, herr_t, (hid_t spaceid), (spaceid), brahma::HDF5);

GOTCHA_MACRO(H5Sselect_none, herr_t, (hid_t spaceid), (spaceid), brahma::HDF5);

GOTCHA_MACRO(H5Sselect_elements, herr_t,
             (hid_t space_id, H5S_seloper_t op, size_t num_elem,
              const hsize_t *coord),
             (space_id, op, num_elem, coord), brahma::HDF5);

GOTCHA_MACRO(H5Sget_select_elem_npoints, hssize_t, (hid_t spaceid), (spaceid),
             brahma::HDF5);

GOTCHA_MACRO(H5Sget_select_elem_pointlist, herr_t,
             (hid_t spaceid, hsize_t startpoint, hsize_t numpoints,
              hsize_t buf[]),
             (spaceid, startpoint, numpoints, buf), brahma::HDF5);

GOTCHA_MACRO(H5Sselect_hyperslab, herr_t,
             (hid_t space_id, H5S_seloper_t op, const hsize_t start[],
              const hsize_t _stride[], const hsize_t count[],
              const hsize_t _block[]),
             (space_id, op, start, _stride, count, _block), brahma::HDF5);

GOTCHA_MACRO(H5Scombine_hyperslab, hid_t,
             (hid_t space_id, H5S_seloper_t op, const hsize_t start[],
              const hsize_t _stride[], const hsize_t count[],
              const hsize_t _block[]),
             (space_id, op, start, _stride, count, _block), brahma::HDF5);

GOTCHA_MACRO(H5Smodify_select, herr_t,
             (hid_t space1_id, H5S_seloper_t op, hid_t space2_id),
             (space1_id, op, space2_id), brahma::HDF5);

GOTCHA_MACRO(H5Scombine_select, hid_t,
             (hid_t space1_id, H5S_seloper_t op, hid_t space2_id),
             (space1_id, op, space2_id), brahma::HDF5);

GOTCHA_MACRO(H5Sis_regular_hyperslab, htri_t, (hid_t spaceid), (spaceid),
             brahma::HDF5);

GOTCHA_MACRO(H5Sget_regular_hyperslab, htri_t,
             (hid_t spaceid, hsize_t start[], hsize_t stride[], hsize_t count[],
              hsize_t block[]),
             (spaceid, start, stride, count, block), brahma::HDF5);

GOTCHA_MACRO(H5Sget_select_hyper_nblocks, hssize_t, (hid_t spaceid), (spaceid),
             brahma::HDF5);

GOTCHA_MACRO(H5Sget_select_hyper_blocklist, herr_t,
             (hid_t spaceid, hsize_t startblock, hsize_t numblocks,
              hsize_t buf[]),
             (spaceid, startblock, numblocks, buf), brahma::HDF5);

GOTCHA_MACRO(H5Sselect_project_intersection, hid_t,
             (hid_t src_space_id, hid_t dst_space_id,
              hid_t src_intersect_space_id),
             (src_space_id, dst_space_id, src_intersect_space_id),
             brahma::HDF5);

GOTCHA_MACRO(H5FD_core_init, hid_t, (), (), brahma::HDF5);

GOTCHA_MACRO(H5Pset_fapl_core, herr_t,
             (hid_t fapl_id, size_t increment, hbool_t backing_store),
             (fapl_id, increment, backing_store), brahma::HDF5);

GOTCHA_MACRO(H5Pget_fapl_core, herr_t,
             (hid_t fapl_id, size_t *increment, hbool_t *backing_store),
             (fapl_id, increment, backing_store), brahma::HDF5);

GOTCHA_MACRO(H5FD_family_init, hid_t, (), (), brahma::HDF5);

GOTCHA_MACRO(H5Pset_fapl_family, herr_t,
             (hid_t fapl_id, hsize_t memb_size, hid_t memb_fapl_id),
             (fapl_id, memb_size, memb_fapl_id), brahma::HDF5);

GOTCHA_MACRO(H5Pget_fapl_family, herr_t,
             (hid_t fapl_id, hsize_t *memb_size, hid_t *memb_fapl_id),
             (fapl_id, memb_size, memb_fapl_id), brahma::HDF5);

GOTCHA_MACRO(H5FD_hdfs_init, hid_t, (), (), brahma::HDF5);

GOTCHA_MACRO(H5Pget_fapl_hdfs, herr_t,
             (hid_t fapl_id, H5FD_hdfs_fapl_t *fa_out), (fapl_id, fa_out),
             brahma::HDF5);

GOTCHA_MACRO(H5Pset_fapl_hdfs, herr_t, (hid_t fapl_id, H5FD_hdfs_fapl_t *fa),
             (fapl_id, fa), brahma::HDF5);

GOTCHA_MACRO(H5FD_log_init, hid_t, (), (), brahma::HDF5);

GOTCHA_MACRO(H5Pset_fapl_log, herr_t,
             (hid_t fapl_id, const char *logfile, unsigned long long flags,
              size_t buf_size),
             (fapl_id, logfile, flags, buf_size), brahma::HDF5);

GOTCHA_MACRO(H5FD_multi_init, hid_t, (), (), brahma::HDF5);

GOTCHA_MACRO(H5Pset_fapl_multi, herr_t,
             (hid_t fapl_id, const H5FD_mem_t *memb_map, const hid_t *memb_fapl,
              const char *const *memb_name, const haddr_t *memb_addr,
              hbool_t relax),
             (fapl_id, memb_map, memb_fapl, memb_name, memb_addr, relax),
             brahma::HDF5);

GOTCHA_MACRO(H5Pget_fapl_multi, herr_t,
             (hid_t fapl_id, H5FD_mem_t *memb_map, hid_t *memb_fapl,
              char **memb_name, haddr_t *memb_addr, hbool_t *relax),
             (fapl_id, memb_map, memb_fapl, memb_name, memb_addr, relax),
             brahma::HDF5);

GOTCHA_MACRO(H5Pset_fapl_split, herr_t,
             (hid_t fapl, const char *meta_ext, hid_t meta_plist_id,
              const char *raw_ext, hid_t raw_plist_id),
             (fapl, meta_ext, meta_plist_id, raw_ext, raw_plist_id),
             brahma::HDF5);

GOTCHA_MACRO(H5FD_ros3_init, hid_t, (), (), brahma::HDF5);

GOTCHA_MACRO(H5Pget_fapl_ros3, herr_t,
             (hid_t fapl_id, H5FD_ros3_fapl_t *fa_out), (fapl_id, fa_out),
             brahma::HDF5);

GOTCHA_MACRO(H5Pset_fapl_ros3, herr_t, (hid_t fapl_id, H5FD_ros3_fapl_t *fa),
             (fapl_id, fa), brahma::HDF5);

GOTCHA_MACRO(H5FD_sec2_init, hid_t, (), (), brahma::HDF5);

GOTCHA_MACRO(H5Pset_fapl_sec2, herr_t, (hid_t fapl_id), (fapl_id),
             brahma::HDF5);

GOTCHA_MACRO(H5FD_splitter_init, hid_t, (), (), brahma::HDF5);

GOTCHA_MACRO(H5Pset_fapl_splitter, herr_t,
             (hid_t fapl_id, H5FD_splitter_vfd_config_t *config_ptr),
             (fapl_id, config_ptr), brahma::HDF5);

GOTCHA_MACRO(H5Pget_fapl_splitter, herr_t,
             (hid_t fapl_id, H5FD_splitter_vfd_config_t *config_ptr),
             (fapl_id, config_ptr), brahma::HDF5);

GOTCHA_MACRO(H5FD_stdio_init, hid_t, (), (), brahma::HDF5);

GOTCHA_MACRO(H5Pset_fapl_stdio, herr_t, (hid_t fapl_id), (fapl_id),
             brahma::HDF5);

int update_hdf5(gotcha_binding_t *&bindings, size_t &binding_index) {
  GOTCHA_BINDING_MACRO(H5open);

  GOTCHA_BINDING_MACRO(H5close);

  GOTCHA_BINDING_MACRO(H5dont_atexit);

  GOTCHA_BINDING_MACRO(H5garbage_collect);

  GOTCHA_BINDING_MACRO(H5set_free_list_limits);

  GOTCHA_BINDING_MACRO(H5get_free_list_sizes);

  GOTCHA_BINDING_MACRO(H5get_alloc_stats);

  GOTCHA_BINDING_MACRO(H5get_libversion);

  GOTCHA_BINDING_MACRO(H5check_version);

  GOTCHA_BINDING_MACRO(H5is_library_threadsafe);

  GOTCHA_BINDING_MACRO(H5free_memory);

  GOTCHA_BINDING_MACRO(H5allocate_memory);

  GOTCHA_BINDING_MACRO(H5resize_memory);

  GOTCHA_BINDING_MACRO(H5Iregister);

  GOTCHA_BINDING_MACRO(H5Iobject_verify);

  GOTCHA_BINDING_MACRO(H5Iremove_verify);

  GOTCHA_BINDING_MACRO(H5Iget_type);

  GOTCHA_BINDING_MACRO(H5Iget_file_id);

  GOTCHA_BINDING_MACRO(H5Iget_name);

  GOTCHA_BINDING_MACRO(H5Iinc_ref);

  GOTCHA_BINDING_MACRO(H5Idec_ref);

  GOTCHA_BINDING_MACRO(H5Iget_ref);

  GOTCHA_BINDING_MACRO(H5Iregister_type);

  GOTCHA_BINDING_MACRO(H5Iclear_type);

  GOTCHA_BINDING_MACRO(H5Idestroy_type);

  GOTCHA_BINDING_MACRO(H5Iinc_type_ref);

  GOTCHA_BINDING_MACRO(H5Idec_type_ref);

  GOTCHA_BINDING_MACRO(H5Iget_type_ref);

  GOTCHA_BINDING_MACRO(H5Isearch);

  GOTCHA_BINDING_MACRO(H5Inmembers);

  GOTCHA_BINDING_MACRO(H5Itype_exists);

  GOTCHA_BINDING_MACRO(H5Iis_valid);

  GOTCHA_BINDING_MACRO(H5Tcreate);

  GOTCHA_BINDING_MACRO(H5Tcopy);

  GOTCHA_BINDING_MACRO(H5Tclose);

  GOTCHA_BINDING_MACRO(H5Tequal);

  GOTCHA_BINDING_MACRO(H5Tlock);

  GOTCHA_BINDING_MACRO(H5Tcommit2);

  GOTCHA_BINDING_MACRO(H5Topen2);

  GOTCHA_BINDING_MACRO(H5Tcommit_anon);

  GOTCHA_BINDING_MACRO(H5Tget_create_plist);

  GOTCHA_BINDING_MACRO(H5Tcommitted);

  GOTCHA_BINDING_MACRO(H5Tencode);

  GOTCHA_BINDING_MACRO(H5Tdecode);

  GOTCHA_BINDING_MACRO(H5Tflush);

  GOTCHA_BINDING_MACRO(H5Trefresh);

  GOTCHA_BINDING_MACRO(H5Tinsert);

  GOTCHA_BINDING_MACRO(H5Tpack);

  GOTCHA_BINDING_MACRO(H5Tenum_create);

  GOTCHA_BINDING_MACRO(H5Tenum_insert);

  GOTCHA_BINDING_MACRO(H5Tenum_nameof);

  GOTCHA_BINDING_MACRO(H5Tenum_valueof);

  GOTCHA_BINDING_MACRO(H5Tvlen_create);

  GOTCHA_BINDING_MACRO(H5Tarray_create2);

  GOTCHA_BINDING_MACRO(H5Tget_array_ndims);

  GOTCHA_BINDING_MACRO(H5Tget_array_dims2);

  GOTCHA_BINDING_MACRO(H5Tset_tag);

  GOTCHA_BINDING_MACRO(H5Tget_tag);

  GOTCHA_BINDING_MACRO(H5Tget_super);

  GOTCHA_BINDING_MACRO(H5Tget_class);

  GOTCHA_BINDING_MACRO(H5Tdetect_class);

  GOTCHA_BINDING_MACRO(H5Tget_size);

  GOTCHA_BINDING_MACRO(H5Tget_order);

  GOTCHA_BINDING_MACRO(H5Tget_precision);

  GOTCHA_BINDING_MACRO(H5Tget_offset);

  GOTCHA_BINDING_MACRO(H5Tget_pad);

  GOTCHA_BINDING_MACRO(H5Tget_sign);

  GOTCHA_BINDING_MACRO(H5Tget_fields);

  GOTCHA_BINDING_MACRO(H5Tget_ebias);

  GOTCHA_BINDING_MACRO(H5Tget_norm);

  GOTCHA_BINDING_MACRO(H5Tget_inpad);

  GOTCHA_BINDING_MACRO(H5Tget_strpad);

  GOTCHA_BINDING_MACRO(H5Tget_nmembers);

  GOTCHA_BINDING_MACRO(H5Tget_member_name);

  GOTCHA_BINDING_MACRO(H5Tget_member_index);

  GOTCHA_BINDING_MACRO(H5Tget_member_offset);

  GOTCHA_BINDING_MACRO(H5Tget_member_class);

  GOTCHA_BINDING_MACRO(H5Tget_member_type);

  GOTCHA_BINDING_MACRO(H5Tget_member_value);

  GOTCHA_BINDING_MACRO(H5Tget_cset);

  GOTCHA_BINDING_MACRO(H5Tis_variable_str);

  GOTCHA_BINDING_MACRO(H5Tget_native_type);

  GOTCHA_BINDING_MACRO(H5Tset_size);

  GOTCHA_BINDING_MACRO(H5Tset_order);

  GOTCHA_BINDING_MACRO(H5Tset_precision);

  GOTCHA_BINDING_MACRO(H5Tset_offset);

  GOTCHA_BINDING_MACRO(H5Tset_pad);

  GOTCHA_BINDING_MACRO(H5Tset_sign);

  GOTCHA_BINDING_MACRO(H5Tset_fields);

  GOTCHA_BINDING_MACRO(H5Tset_ebias);

  GOTCHA_BINDING_MACRO(H5Tset_norm);

  GOTCHA_BINDING_MACRO(H5Tset_inpad);

  GOTCHA_BINDING_MACRO(H5Tset_cset);

  GOTCHA_BINDING_MACRO(H5Tset_strpad);

  GOTCHA_BINDING_MACRO(H5Tregister);

  GOTCHA_BINDING_MACRO(H5Tunregister);

  GOTCHA_BINDING_MACRO(H5Tfind);

  GOTCHA_BINDING_MACRO(H5Tcompiler_conv);

  GOTCHA_BINDING_MACRO(H5Tconvert);

  GOTCHA_BINDING_MACRO(H5Tcommit1);

  GOTCHA_BINDING_MACRO(H5Topen1);

  GOTCHA_BINDING_MACRO(H5Tarray_create1);

  GOTCHA_BINDING_MACRO(H5Tget_array_dims1);

  GOTCHA_BINDING_MACRO(H5Lmove);

  GOTCHA_BINDING_MACRO(H5Lcopy);

  GOTCHA_BINDING_MACRO(H5Lcreate_hard);

  GOTCHA_BINDING_MACRO(H5Lcreate_soft);

  GOTCHA_BINDING_MACRO(H5Ldelete);

  GOTCHA_BINDING_MACRO(H5Ldelete_by_idx);

  GOTCHA_BINDING_MACRO(H5Lget_val);

  GOTCHA_BINDING_MACRO(H5Lget_val_by_idx);

  GOTCHA_BINDING_MACRO(H5Lexists);

  GOTCHA_BINDING_MACRO(H5Lget_info);

  GOTCHA_BINDING_MACRO(H5Lget_info_by_idx);

  GOTCHA_BINDING_MACRO(H5Lget_name_by_idx);

  GOTCHA_BINDING_MACRO(H5Literate);

  GOTCHA_BINDING_MACRO(H5Literate_by_name);

  GOTCHA_BINDING_MACRO(H5Lvisit);

  GOTCHA_BINDING_MACRO(H5Lvisit_by_name);

  GOTCHA_BINDING_MACRO(H5Lcreate_ud);

  GOTCHA_BINDING_MACRO(H5Lregister);

  GOTCHA_BINDING_MACRO(H5Lunregister);

  GOTCHA_BINDING_MACRO(H5Lis_registered);

  GOTCHA_BINDING_MACRO(H5Lunpack_elink_val);

  GOTCHA_BINDING_MACRO(H5Lcreate_external);

  GOTCHA_BINDING_MACRO(H5Oopen);

  GOTCHA_BINDING_MACRO(H5Oopen_by_addr);

  GOTCHA_BINDING_MACRO(H5Oopen_by_idx);

  GOTCHA_BINDING_MACRO(H5Oexists_by_name);

  GOTCHA_BINDING_MACRO(H5Oget_info2);

  GOTCHA_BINDING_MACRO(H5Oget_info_by_name2);

  GOTCHA_BINDING_MACRO(H5Oget_info_by_idx2);

  GOTCHA_BINDING_MACRO(H5Olink);

  GOTCHA_BINDING_MACRO(H5Oincr_refcount);

  GOTCHA_BINDING_MACRO(H5Odecr_refcount);

  GOTCHA_BINDING_MACRO(H5Ocopy);

  GOTCHA_BINDING_MACRO(H5Oset_comment);

  GOTCHA_BINDING_MACRO(H5Oset_comment_by_name);

  GOTCHA_BINDING_MACRO(H5Oget_comment);

  GOTCHA_BINDING_MACRO(H5Oget_comment_by_name);

  GOTCHA_BINDING_MACRO(H5Ovisit2);

  GOTCHA_BINDING_MACRO(H5Ovisit_by_name2);

  GOTCHA_BINDING_MACRO(H5Oclose);

  GOTCHA_BINDING_MACRO(H5Oflush);

  GOTCHA_BINDING_MACRO(H5Orefresh);

  GOTCHA_BINDING_MACRO(H5Odisable_mdc_flushes);

  GOTCHA_BINDING_MACRO(H5Oenable_mdc_flushes);

  GOTCHA_BINDING_MACRO(H5Oare_mdc_flushes_disabled);

  GOTCHA_BINDING_MACRO(H5Oget_info);

  GOTCHA_BINDING_MACRO(H5Oget_info_by_name);

  GOTCHA_BINDING_MACRO(H5Oget_info_by_idx);

  GOTCHA_BINDING_MACRO(H5Ovisit);

  GOTCHA_BINDING_MACRO(H5Ovisit_by_name);

  GOTCHA_BINDING_MACRO(H5Oget_info1);

  GOTCHA_BINDING_MACRO(H5Oget_info_by_name1);

  GOTCHA_BINDING_MACRO(H5Oget_info_by_idx1);

  GOTCHA_BINDING_MACRO(H5Ovisit1);

  GOTCHA_BINDING_MACRO(H5Ovisit_by_name1);

  GOTCHA_BINDING_MACRO(H5Acreate2);

  GOTCHA_BINDING_MACRO(H5Acreate_by_name);

  GOTCHA_BINDING_MACRO(H5Aopen);

  GOTCHA_BINDING_MACRO(H5Aopen_by_name);

  GOTCHA_BINDING_MACRO(H5Aopen_by_idx);

  GOTCHA_BINDING_MACRO(H5Awrite);

  GOTCHA_BINDING_MACRO(H5Aread);

  GOTCHA_BINDING_MACRO(H5Aclose);

  GOTCHA_BINDING_MACRO(H5Aget_space);

  GOTCHA_BINDING_MACRO(H5Aget_type);

  GOTCHA_BINDING_MACRO(H5Aget_create_plist);

  GOTCHA_BINDING_MACRO(H5Aget_name);

  GOTCHA_BINDING_MACRO(H5Aget_name_by_idx);

  GOTCHA_BINDING_MACRO(H5Aget_storage_size);

  GOTCHA_BINDING_MACRO(H5Aget_info);

  GOTCHA_BINDING_MACRO(H5Aget_info_by_name);

  GOTCHA_BINDING_MACRO(H5Aget_info_by_idx);

  GOTCHA_BINDING_MACRO(H5Arename);

  GOTCHA_BINDING_MACRO(H5Arename_by_name);

  GOTCHA_BINDING_MACRO(H5Aiterate2);

  GOTCHA_BINDING_MACRO(H5Aiterate_by_name);

  GOTCHA_BINDING_MACRO(H5Adelete);

  GOTCHA_BINDING_MACRO(H5Adelete_by_name);

  GOTCHA_BINDING_MACRO(H5Adelete_by_idx);

  GOTCHA_BINDING_MACRO(H5Aexists);

  GOTCHA_BINDING_MACRO(H5Aexists_by_name);

  GOTCHA_BINDING_MACRO(H5Acreate1);

  GOTCHA_BINDING_MACRO(H5Aopen_name);

  GOTCHA_BINDING_MACRO(H5Aopen_idx);

  GOTCHA_BINDING_MACRO(H5Aget_num_attrs);

  GOTCHA_BINDING_MACRO(H5Aiterate1);

  GOTCHA_BINDING_MACRO(H5Dcreate2);

  GOTCHA_BINDING_MACRO(H5Dcreate_anon);

  GOTCHA_BINDING_MACRO(H5Dopen2);

  GOTCHA_BINDING_MACRO(H5Dclose);

  GOTCHA_BINDING_MACRO(H5Dget_space);

  GOTCHA_BINDING_MACRO(H5Dget_space_status);

  GOTCHA_BINDING_MACRO(H5Dget_type);

  GOTCHA_BINDING_MACRO(H5Dget_create_plist);

  GOTCHA_BINDING_MACRO(H5Dget_access_plist);

  GOTCHA_BINDING_MACRO(H5Dget_storage_size);

  GOTCHA_BINDING_MACRO(H5Dget_chunk_storage_size);

  GOTCHA_BINDING_MACRO(H5Dget_num_chunks);

  GOTCHA_BINDING_MACRO(H5Dget_chunk_info_by_coord);

  GOTCHA_BINDING_MACRO(H5Dget_chunk_info);

  GOTCHA_BINDING_MACRO(H5Dget_offset);

  GOTCHA_BINDING_MACRO(H5Dread);

  GOTCHA_BINDING_MACRO(H5Dwrite);

  GOTCHA_BINDING_MACRO(H5Dwrite_chunk);

  GOTCHA_BINDING_MACRO(H5Dread_chunk);

  GOTCHA_BINDING_MACRO(H5Diterate);

  GOTCHA_BINDING_MACRO(H5Dvlen_reclaim);

  GOTCHA_BINDING_MACRO(H5Dvlen_get_buf_size);

  GOTCHA_BINDING_MACRO(H5Dfill);

  GOTCHA_BINDING_MACRO(H5Dset_extent);

  GOTCHA_BINDING_MACRO(H5Dflush);

  GOTCHA_BINDING_MACRO(H5Drefresh);

  GOTCHA_BINDING_MACRO(H5Dscatter);

  GOTCHA_BINDING_MACRO(H5Dgather);

  GOTCHA_BINDING_MACRO(H5Ddebug);

  GOTCHA_BINDING_MACRO(H5Dformat_convert);

  GOTCHA_BINDING_MACRO(H5Dget_chunk_index_type);

  GOTCHA_BINDING_MACRO(H5Dcreate1);

  GOTCHA_BINDING_MACRO(H5Dopen1);

  GOTCHA_BINDING_MACRO(H5Dextend);

  GOTCHA_BINDING_MACRO(H5Eregister_class);

  GOTCHA_BINDING_MACRO(H5Eunregister_class);

  GOTCHA_BINDING_MACRO(H5Eclose_msg);

  GOTCHA_BINDING_MACRO(H5Ecreate_msg);

  GOTCHA_BINDING_MACRO(H5Ecreate_stack);

  GOTCHA_BINDING_MACRO(H5Eget_current_stack);

  GOTCHA_BINDING_MACRO(H5Eclose_stack);

  GOTCHA_BINDING_MACRO(H5Eget_class_name);

  GOTCHA_BINDING_MACRO(H5Eset_current_stack);

  GOTCHA_BINDING_MACRO(H5Epush2);

  GOTCHA_BINDING_MACRO(H5Epop);

  GOTCHA_BINDING_MACRO(H5Eprint2);

  GOTCHA_BINDING_MACRO(H5Ewalk2);

  GOTCHA_BINDING_MACRO(H5Eget_auto2);

  GOTCHA_BINDING_MACRO(H5Eset_auto2);

  GOTCHA_BINDING_MACRO(H5Eclear2);

  GOTCHA_BINDING_MACRO(H5Eauto_is_v2);

  GOTCHA_BINDING_MACRO(H5Eget_msg);

  GOTCHA_BINDING_MACRO(H5Eget_num);

  GOTCHA_BINDING_MACRO(H5Eclear1);

  GOTCHA_BINDING_MACRO(H5Eget_auto1);

  GOTCHA_BINDING_MACRO(H5Epush1);

  GOTCHA_BINDING_MACRO(H5Eprint1);

  GOTCHA_BINDING_MACRO(H5Eset_auto1);

  GOTCHA_BINDING_MACRO(H5Ewalk1);

  GOTCHA_BINDING_MACRO(H5Eget_major);

  GOTCHA_BINDING_MACRO(H5Eget_minor);

  GOTCHA_BINDING_MACRO(H5Fis_hdf5);

  GOTCHA_BINDING_MACRO(H5Fcreate);

  GOTCHA_BINDING_MACRO(H5Fopen);

  GOTCHA_BINDING_MACRO(H5Freopen);

  GOTCHA_BINDING_MACRO(H5Fflush);

  GOTCHA_BINDING_MACRO(H5Fclose);

  GOTCHA_BINDING_MACRO(H5Fget_create_plist);

  GOTCHA_BINDING_MACRO(H5Fget_access_plist);

  GOTCHA_BINDING_MACRO(H5Fget_intent);

  GOTCHA_BINDING_MACRO(H5Fget_obj_count);

  GOTCHA_BINDING_MACRO(H5Fget_obj_ids);

  GOTCHA_BINDING_MACRO(H5Fget_vfd_handle);

  GOTCHA_BINDING_MACRO(H5Fmount);

  GOTCHA_BINDING_MACRO(H5Funmount);

  GOTCHA_BINDING_MACRO(H5Fget_freespace);

  GOTCHA_BINDING_MACRO(H5Fget_filesize);

  GOTCHA_BINDING_MACRO(H5Fget_eoa);

  GOTCHA_BINDING_MACRO(H5Fincrement_filesize);

  GOTCHA_BINDING_MACRO(H5Fget_file_image);

  GOTCHA_BINDING_MACRO(H5Fget_mdc_config);

  GOTCHA_BINDING_MACRO(H5Fset_mdc_config);

  GOTCHA_BINDING_MACRO(H5Fget_mdc_hit_rate);

  GOTCHA_BINDING_MACRO(H5Fget_mdc_size);

  GOTCHA_BINDING_MACRO(H5Freset_mdc_hit_rate_stats);

  GOTCHA_BINDING_MACRO(H5Fget_name);

  GOTCHA_BINDING_MACRO(H5Fget_info2);

  GOTCHA_BINDING_MACRO(H5Fget_metadata_read_retry_info);

  GOTCHA_BINDING_MACRO(H5Fstart_swmr_write);

  GOTCHA_BINDING_MACRO(H5Fget_free_sections);

  GOTCHA_BINDING_MACRO(H5Fclear_elink_file_cache);

  GOTCHA_BINDING_MACRO(H5Fset_libver_bounds);

  GOTCHA_BINDING_MACRO(H5Fstart_mdc_logging);

  GOTCHA_BINDING_MACRO(H5Fstop_mdc_logging);

  GOTCHA_BINDING_MACRO(H5Fget_mdc_logging_status);

  GOTCHA_BINDING_MACRO(H5Fformat_convert);

  GOTCHA_BINDING_MACRO(H5Freset_page_buffering_stats);

  GOTCHA_BINDING_MACRO(H5Fget_page_buffering_stats);

  GOTCHA_BINDING_MACRO(H5Fget_mdc_image_info);

  GOTCHA_BINDING_MACRO(H5Fget_dset_no_attrs_hint);

  GOTCHA_BINDING_MACRO(H5Fset_dset_no_attrs_hint);

  GOTCHA_BINDING_MACRO(H5Fget_info1);

  GOTCHA_BINDING_MACRO(H5Fset_latest_format);

  GOTCHA_BINDING_MACRO(H5FDregister);

  GOTCHA_BINDING_MACRO(H5FDunregister);

  GOTCHA_BINDING_MACRO(H5FDopen);

  GOTCHA_BINDING_MACRO(H5FDclose);

  GOTCHA_BINDING_MACRO(H5FDcmp);

  GOTCHA_BINDING_MACRO(H5FDquery);

  GOTCHA_BINDING_MACRO(H5FDalloc);

  GOTCHA_BINDING_MACRO(H5FDfree);

  GOTCHA_BINDING_MACRO(H5FDget_eoa);

  GOTCHA_BINDING_MACRO(H5FDset_eoa);

  GOTCHA_BINDING_MACRO(H5FDget_eof);

  GOTCHA_BINDING_MACRO(H5FDget_vfd_handle);

  GOTCHA_BINDING_MACRO(H5FDread);

  GOTCHA_BINDING_MACRO(H5FDwrite);

  GOTCHA_BINDING_MACRO(H5FDflush);

  GOTCHA_BINDING_MACRO(H5FDtruncate);

  GOTCHA_BINDING_MACRO(H5FDlock);

  GOTCHA_BINDING_MACRO(H5FDunlock);

  GOTCHA_BINDING_MACRO(H5FDdriver_query);

  GOTCHA_BINDING_MACRO(H5Gcreate2);

  GOTCHA_BINDING_MACRO(H5Gcreate_anon);

  GOTCHA_BINDING_MACRO(H5Gopen2);

  GOTCHA_BINDING_MACRO(H5Gget_create_plist);

  GOTCHA_BINDING_MACRO(H5Gget_info);

  GOTCHA_BINDING_MACRO(H5Gget_info_by_name);

  GOTCHA_BINDING_MACRO(H5Gget_info_by_idx);

  GOTCHA_BINDING_MACRO(H5Gclose);

  GOTCHA_BINDING_MACRO(H5Gflush);

  GOTCHA_BINDING_MACRO(H5Grefresh);

  GOTCHA_BINDING_MACRO(H5Gcreate1);

  GOTCHA_BINDING_MACRO(H5Gopen1);

  GOTCHA_BINDING_MACRO(H5Glink);

  GOTCHA_BINDING_MACRO(H5Glink2);

  GOTCHA_BINDING_MACRO(H5Gmove);

  GOTCHA_BINDING_MACRO(H5Gmove2);

  GOTCHA_BINDING_MACRO(H5Gunlink);

  GOTCHA_BINDING_MACRO(H5Gget_linkval);

  GOTCHA_BINDING_MACRO(H5Gset_comment);

  GOTCHA_BINDING_MACRO(H5Gget_comment);

  GOTCHA_BINDING_MACRO(H5Giterate);

  GOTCHA_BINDING_MACRO(H5Gget_num_objs);

  GOTCHA_BINDING_MACRO(H5Gget_objinfo);

  GOTCHA_BINDING_MACRO(H5Gget_objname_by_idx);

  GOTCHA_BINDING_MACRO(H5Gget_objtype_by_idx);

  GOTCHA_BINDING_MACRO(H5Zregister);

  GOTCHA_BINDING_MACRO(H5Zunregister);

  GOTCHA_BINDING_MACRO(H5Zfilter_avail);

  GOTCHA_BINDING_MACRO(H5Zget_filter_info);

  GOTCHA_BINDING_MACRO(H5Pcreate_class);

  GOTCHA_BINDING_MACRO(H5Pget_class_name);

  GOTCHA_BINDING_MACRO(H5Pcreate);

  GOTCHA_BINDING_MACRO(H5Pregister2);

  GOTCHA_BINDING_MACRO(H5Pinsert2);

  GOTCHA_BINDING_MACRO(H5Pset);

  GOTCHA_BINDING_MACRO(H5Pexist);

  GOTCHA_BINDING_MACRO(H5Pencode);

  GOTCHA_BINDING_MACRO(H5Pdecode);

  GOTCHA_BINDING_MACRO(H5Pget_size);

  GOTCHA_BINDING_MACRO(H5Pget_nprops);

  GOTCHA_BINDING_MACRO(H5Pget_class);

  GOTCHA_BINDING_MACRO(H5Pget_class_parent);

  GOTCHA_BINDING_MACRO(H5Pget);

  GOTCHA_BINDING_MACRO(H5Pequal);

  GOTCHA_BINDING_MACRO(H5Pisa_class);

  GOTCHA_BINDING_MACRO(H5Piterate);

  GOTCHA_BINDING_MACRO(H5Pcopy_prop);

  GOTCHA_BINDING_MACRO(H5Premove);

  GOTCHA_BINDING_MACRO(H5Punregister);

  GOTCHA_BINDING_MACRO(H5Pclose_class);

  GOTCHA_BINDING_MACRO(H5Pclose);

  GOTCHA_BINDING_MACRO(H5Pcopy);

  GOTCHA_BINDING_MACRO(H5Pset_attr_phase_change);

  GOTCHA_BINDING_MACRO(H5Pget_attr_phase_change);

  GOTCHA_BINDING_MACRO(H5Pset_attr_creation_order);

  GOTCHA_BINDING_MACRO(H5Pget_attr_creation_order);

  GOTCHA_BINDING_MACRO(H5Pset_obj_track_times);

  GOTCHA_BINDING_MACRO(H5Pget_obj_track_times);

  GOTCHA_BINDING_MACRO(H5Pmodify_filter);

  GOTCHA_BINDING_MACRO(H5Pset_filter);

  GOTCHA_BINDING_MACRO(H5Pget_nfilters);

  GOTCHA_BINDING_MACRO(H5Pget_filter2);

  GOTCHA_BINDING_MACRO(H5Pget_filter_by_id2);

  GOTCHA_BINDING_MACRO(H5Pall_filters_avail);

  GOTCHA_BINDING_MACRO(H5Premove_filter);

  GOTCHA_BINDING_MACRO(H5Pset_deflate);

  GOTCHA_BINDING_MACRO(H5Pset_fletcher32);

  GOTCHA_BINDING_MACRO(H5Pset_userblock);

  GOTCHA_BINDING_MACRO(H5Pget_userblock);

  GOTCHA_BINDING_MACRO(H5Pset_sizes);

  GOTCHA_BINDING_MACRO(H5Pget_sizes);

  GOTCHA_BINDING_MACRO(H5Pset_sym_k);

  GOTCHA_BINDING_MACRO(H5Pget_sym_k);

  GOTCHA_BINDING_MACRO(H5Pset_istore_k);

  GOTCHA_BINDING_MACRO(H5Pget_istore_k);

  GOTCHA_BINDING_MACRO(H5Pset_shared_mesg_nindexes);

  GOTCHA_BINDING_MACRO(H5Pget_shared_mesg_nindexes);

  GOTCHA_BINDING_MACRO(H5Pset_shared_mesg_index);

  GOTCHA_BINDING_MACRO(H5Pget_shared_mesg_index);

  GOTCHA_BINDING_MACRO(H5Pset_shared_mesg_phase_change);

  GOTCHA_BINDING_MACRO(H5Pget_shared_mesg_phase_change);

  GOTCHA_BINDING_MACRO(H5Pset_file_space_strategy);

  GOTCHA_BINDING_MACRO(H5Pget_file_space_strategy);

  GOTCHA_BINDING_MACRO(H5Pset_file_space_page_size);

  GOTCHA_BINDING_MACRO(H5Pget_file_space_page_size);

  GOTCHA_BINDING_MACRO(H5Pset_alignment);

  GOTCHA_BINDING_MACRO(H5Pget_alignment);

  GOTCHA_BINDING_MACRO(H5Pset_driver);

  GOTCHA_BINDING_MACRO(H5Pget_driver);

  GOTCHA_BINDING_MACRO(H5Pget_driver_info);

  GOTCHA_BINDING_MACRO(H5Pset_family_offset);

  GOTCHA_BINDING_MACRO(H5Pget_family_offset);

  GOTCHA_BINDING_MACRO(H5Pset_multi_type);

  GOTCHA_BINDING_MACRO(H5Pget_multi_type);

  GOTCHA_BINDING_MACRO(H5Pset_cache);

  GOTCHA_BINDING_MACRO(H5Pget_cache);

  GOTCHA_BINDING_MACRO(H5Pset_mdc_config);

  GOTCHA_BINDING_MACRO(H5Pget_mdc_config);

  GOTCHA_BINDING_MACRO(H5Pset_gc_references);

  GOTCHA_BINDING_MACRO(H5Pget_gc_references);

  GOTCHA_BINDING_MACRO(H5Pset_fclose_degree);

  GOTCHA_BINDING_MACRO(H5Pget_fclose_degree);

  GOTCHA_BINDING_MACRO(H5Pset_meta_block_size);

  GOTCHA_BINDING_MACRO(H5Pget_meta_block_size);

  GOTCHA_BINDING_MACRO(H5Pset_sieve_buf_size);

  GOTCHA_BINDING_MACRO(H5Pget_sieve_buf_size);

  GOTCHA_BINDING_MACRO(H5Pset_small_data_block_size);

  GOTCHA_BINDING_MACRO(H5Pget_small_data_block_size);

  GOTCHA_BINDING_MACRO(H5Pset_libver_bounds);

  GOTCHA_BINDING_MACRO(H5Pget_libver_bounds);

  GOTCHA_BINDING_MACRO(H5Pset_elink_file_cache_size);

  GOTCHA_BINDING_MACRO(H5Pget_elink_file_cache_size);

  GOTCHA_BINDING_MACRO(H5Pset_file_image);

  GOTCHA_BINDING_MACRO(H5Pget_file_image);

  GOTCHA_BINDING_MACRO(H5Pset_file_image_callbacks);

  GOTCHA_BINDING_MACRO(H5Pget_file_image_callbacks);

  GOTCHA_BINDING_MACRO(H5Pset_core_write_tracking);

  GOTCHA_BINDING_MACRO(H5Pget_core_write_tracking);

  GOTCHA_BINDING_MACRO(H5Pset_metadata_read_attempts);

  GOTCHA_BINDING_MACRO(H5Pget_metadata_read_attempts);

  GOTCHA_BINDING_MACRO(H5Pset_object_flush_cb);

  GOTCHA_BINDING_MACRO(H5Pget_object_flush_cb);

  GOTCHA_BINDING_MACRO(H5Pset_mdc_log_options);

  GOTCHA_BINDING_MACRO(H5Pget_mdc_log_options);

  GOTCHA_BINDING_MACRO(H5Pset_evict_on_close);

  GOTCHA_BINDING_MACRO(H5Pget_evict_on_close);

  GOTCHA_BINDING_MACRO(H5Pset_file_locking);

  GOTCHA_BINDING_MACRO(H5Pget_file_locking);

  GOTCHA_BINDING_MACRO(H5Pset_mdc_image_config);

  GOTCHA_BINDING_MACRO(H5Pget_mdc_image_config);

  GOTCHA_BINDING_MACRO(H5Pset_page_buffer_size);

  GOTCHA_BINDING_MACRO(H5Pget_page_buffer_size);

  GOTCHA_BINDING_MACRO(H5Pset_layout);

  GOTCHA_BINDING_MACRO(H5Pget_layout);

  GOTCHA_BINDING_MACRO(H5Pset_chunk);

  GOTCHA_BINDING_MACRO(H5Pget_chunk);

  GOTCHA_BINDING_MACRO(H5Pset_virtual);

  GOTCHA_BINDING_MACRO(H5Pget_virtual_count);

  GOTCHA_BINDING_MACRO(H5Pget_virtual_vspace);

  GOTCHA_BINDING_MACRO(H5Pget_virtual_srcspace);

  GOTCHA_BINDING_MACRO(H5Pget_virtual_filename);

  GOTCHA_BINDING_MACRO(H5Pget_virtual_dsetname);

  GOTCHA_BINDING_MACRO(H5Pset_external);

  GOTCHA_BINDING_MACRO(H5Pset_chunk_opts);

  GOTCHA_BINDING_MACRO(H5Pget_chunk_opts);

  GOTCHA_BINDING_MACRO(H5Pget_external_count);

  GOTCHA_BINDING_MACRO(H5Pget_external);

  GOTCHA_BINDING_MACRO(H5Pset_szip);

  GOTCHA_BINDING_MACRO(H5Pset_shuffle);

  GOTCHA_BINDING_MACRO(H5Pset_nbit);

  GOTCHA_BINDING_MACRO(H5Pset_scaleoffset);

  GOTCHA_BINDING_MACRO(H5Pset_fill_value);

  GOTCHA_BINDING_MACRO(H5Pget_fill_value);

  GOTCHA_BINDING_MACRO(H5Pfill_value_defined);

  GOTCHA_BINDING_MACRO(H5Pset_alloc_time);

  GOTCHA_BINDING_MACRO(H5Pget_alloc_time);

  GOTCHA_BINDING_MACRO(H5Pset_fill_time);

  GOTCHA_BINDING_MACRO(H5Pget_fill_time);

  GOTCHA_BINDING_MACRO(H5Pget_dset_no_attrs_hint);

  GOTCHA_BINDING_MACRO(H5Pset_dset_no_attrs_hint);

  GOTCHA_BINDING_MACRO(H5Pset_chunk_cache);

  GOTCHA_BINDING_MACRO(H5Pget_chunk_cache);

  GOTCHA_BINDING_MACRO(H5Pset_virtual_view);

  GOTCHA_BINDING_MACRO(H5Pget_virtual_view);

  GOTCHA_BINDING_MACRO(H5Pset_virtual_printf_gap);

  GOTCHA_BINDING_MACRO(H5Pget_virtual_printf_gap);

  GOTCHA_BINDING_MACRO(H5Pset_virtual_prefix);

  GOTCHA_BINDING_MACRO(H5Pget_virtual_prefix);

  GOTCHA_BINDING_MACRO(H5Pset_append_flush);

  GOTCHA_BINDING_MACRO(H5Pget_append_flush);

  GOTCHA_BINDING_MACRO(H5Pset_efile_prefix);

  GOTCHA_BINDING_MACRO(H5Pget_efile_prefix);

  GOTCHA_BINDING_MACRO(H5Pset_data_transform);

  GOTCHA_BINDING_MACRO(H5Pget_data_transform);

  GOTCHA_BINDING_MACRO(H5Pset_buffer);

  GOTCHA_BINDING_MACRO(H5Pget_buffer);

  GOTCHA_BINDING_MACRO(H5Pset_preserve);

  GOTCHA_BINDING_MACRO(H5Pget_preserve);

  GOTCHA_BINDING_MACRO(H5Pset_edc_check);

  GOTCHA_BINDING_MACRO(H5Pget_edc_check);

  GOTCHA_BINDING_MACRO(H5Pset_filter_callback);

  GOTCHA_BINDING_MACRO(H5Pset_btree_ratios);

  GOTCHA_BINDING_MACRO(H5Pget_btree_ratios);

  GOTCHA_BINDING_MACRO(H5Pset_vlen_mem_manager);

  GOTCHA_BINDING_MACRO(H5Pget_vlen_mem_manager);

  GOTCHA_BINDING_MACRO(H5Pset_hyper_vector_size);

  GOTCHA_BINDING_MACRO(H5Pget_hyper_vector_size);

  GOTCHA_BINDING_MACRO(H5Pset_type_conv_cb);

  GOTCHA_BINDING_MACRO(H5Pget_type_conv_cb);

  GOTCHA_BINDING_MACRO(H5Pset_create_intermediate_group);

  GOTCHA_BINDING_MACRO(H5Pget_create_intermediate_group);

  GOTCHA_BINDING_MACRO(H5Pset_local_heap_size_hint);

  GOTCHA_BINDING_MACRO(H5Pget_local_heap_size_hint);

  GOTCHA_BINDING_MACRO(H5Pset_link_phase_change);

  GOTCHA_BINDING_MACRO(H5Pget_link_phase_change);

  GOTCHA_BINDING_MACRO(H5Pset_est_link_info);

  GOTCHA_BINDING_MACRO(H5Pget_est_link_info);

  GOTCHA_BINDING_MACRO(H5Pset_link_creation_order);

  GOTCHA_BINDING_MACRO(H5Pget_link_creation_order);

  GOTCHA_BINDING_MACRO(H5Pset_char_encoding);

  GOTCHA_BINDING_MACRO(H5Pget_char_encoding);

  GOTCHA_BINDING_MACRO(H5Pset_nlinks);

  GOTCHA_BINDING_MACRO(H5Pget_nlinks);

  GOTCHA_BINDING_MACRO(H5Pset_elink_prefix);

  GOTCHA_BINDING_MACRO(H5Pget_elink_prefix);

  GOTCHA_BINDING_MACRO(H5Pget_elink_fapl);

  GOTCHA_BINDING_MACRO(H5Pset_elink_fapl);

  GOTCHA_BINDING_MACRO(H5Pset_elink_acc_flags);

  GOTCHA_BINDING_MACRO(H5Pget_elink_acc_flags);

  GOTCHA_BINDING_MACRO(H5Pset_elink_cb);

  GOTCHA_BINDING_MACRO(H5Pget_elink_cb);

  GOTCHA_BINDING_MACRO(H5Pset_copy_object);

  GOTCHA_BINDING_MACRO(H5Pget_copy_object);

  GOTCHA_BINDING_MACRO(H5Padd_merge_committed_dtype_path);

  GOTCHA_BINDING_MACRO(H5Pfree_merge_committed_dtype_paths);

  GOTCHA_BINDING_MACRO(H5Pset_mcdt_search_cb);

  GOTCHA_BINDING_MACRO(H5Pget_mcdt_search_cb);

  GOTCHA_BINDING_MACRO(H5Pregister1);

  GOTCHA_BINDING_MACRO(H5Pinsert1);

  GOTCHA_BINDING_MACRO(H5Pget_filter1);

  GOTCHA_BINDING_MACRO(H5Pget_filter_by_id1);

  GOTCHA_BINDING_MACRO(H5Pget_version);

  GOTCHA_BINDING_MACRO(H5Pset_file_space);

  GOTCHA_BINDING_MACRO(H5Pget_file_space);

  GOTCHA_BINDING_MACRO(H5PLset_loading_state);

  GOTCHA_BINDING_MACRO(H5PLget_loading_state);

  GOTCHA_BINDING_MACRO(H5PLappend);

  GOTCHA_BINDING_MACRO(H5PLprepend);

  GOTCHA_BINDING_MACRO(H5PLreplace);

  GOTCHA_BINDING_MACRO(H5PLinsert);

  GOTCHA_BINDING_MACRO(H5PLremove);

  GOTCHA_BINDING_MACRO(H5PLget);

  GOTCHA_BINDING_MACRO(H5PLsize);

  GOTCHA_BINDING_MACRO(H5Rcreate);

  GOTCHA_BINDING_MACRO(H5Rdereference2);

  GOTCHA_BINDING_MACRO(H5Rget_region);

  GOTCHA_BINDING_MACRO(H5Rget_obj_type2);

  GOTCHA_BINDING_MACRO(H5Rget_name);

  GOTCHA_BINDING_MACRO(H5Rget_obj_type1);

  GOTCHA_BINDING_MACRO(H5Rdereference1);

  GOTCHA_BINDING_MACRO(H5Screate);

  GOTCHA_BINDING_MACRO(H5Screate_simple);

  GOTCHA_BINDING_MACRO(H5Sset_extent_simple);

  GOTCHA_BINDING_MACRO(H5Scopy);

  GOTCHA_BINDING_MACRO(H5Sclose);

  GOTCHA_BINDING_MACRO(H5Sencode);

  GOTCHA_BINDING_MACRO(H5Sdecode);

  GOTCHA_BINDING_MACRO(H5Sget_simple_extent_npoints);

  GOTCHA_BINDING_MACRO(H5Sget_simple_extent_ndims);

  GOTCHA_BINDING_MACRO(H5Sget_simple_extent_dims);

  GOTCHA_BINDING_MACRO(H5Sis_simple);

  GOTCHA_BINDING_MACRO(H5Sget_simple_extent_type);

  GOTCHA_BINDING_MACRO(H5Sset_extent_none);

  GOTCHA_BINDING_MACRO(H5Sextent_copy);

  GOTCHA_BINDING_MACRO(H5Sextent_equal);

  GOTCHA_BINDING_MACRO(H5Sget_select_type);

  GOTCHA_BINDING_MACRO(H5Sget_select_npoints);

  GOTCHA_BINDING_MACRO(H5Sselect_copy);

  GOTCHA_BINDING_MACRO(H5Sselect_valid);

  GOTCHA_BINDING_MACRO(H5Sselect_adjust);

  GOTCHA_BINDING_MACRO(H5Sget_select_bounds);

  GOTCHA_BINDING_MACRO(H5Sselect_shape_same);

  GOTCHA_BINDING_MACRO(H5Sselect_intersect_block);

  GOTCHA_BINDING_MACRO(H5Soffset_simple);

  GOTCHA_BINDING_MACRO(H5Sselect_all);

  GOTCHA_BINDING_MACRO(H5Sselect_none);

  GOTCHA_BINDING_MACRO(H5Sselect_elements);

  GOTCHA_BINDING_MACRO(H5Sget_select_elem_npoints);

  GOTCHA_BINDING_MACRO(H5Sget_select_elem_pointlist);

  GOTCHA_BINDING_MACRO(H5Sselect_hyperslab);

  GOTCHA_BINDING_MACRO(H5Scombine_hyperslab);

  GOTCHA_BINDING_MACRO(H5Smodify_select);

  GOTCHA_BINDING_MACRO(H5Scombine_select);

  GOTCHA_BINDING_MACRO(H5Sis_regular_hyperslab);

  GOTCHA_BINDING_MACRO(H5Sget_regular_hyperslab);

  GOTCHA_BINDING_MACRO(H5Sget_select_hyper_nblocks);

  GOTCHA_BINDING_MACRO(H5Sget_select_hyper_blocklist);

  GOTCHA_BINDING_MACRO(H5Sselect_project_intersection);

  GOTCHA_BINDING_MACRO(H5FD_core_init);

  GOTCHA_BINDING_MACRO(H5Pset_fapl_core);

  GOTCHA_BINDING_MACRO(H5Pget_fapl_core);

  GOTCHA_BINDING_MACRO(H5FD_family_init);

  GOTCHA_BINDING_MACRO(H5Pset_fapl_family);

  GOTCHA_BINDING_MACRO(H5Pget_fapl_family);

  GOTCHA_BINDING_MACRO(H5FD_hdfs_init);

  GOTCHA_BINDING_MACRO(H5Pget_fapl_hdfs);

  GOTCHA_BINDING_MACRO(H5Pset_fapl_hdfs);

  GOTCHA_BINDING_MACRO(H5FD_log_init);

  GOTCHA_BINDING_MACRO(H5Pset_fapl_log);

  GOTCHA_BINDING_MACRO(H5FD_multi_init);

  GOTCHA_BINDING_MACRO(H5Pset_fapl_multi);

  GOTCHA_BINDING_MACRO(H5Pget_fapl_multi);

  GOTCHA_BINDING_MACRO(H5Pset_fapl_split);

  GOTCHA_BINDING_MACRO(H5FD_ros3_init);

  GOTCHA_BINDING_MACRO(H5Pget_fapl_ros3);

  GOTCHA_BINDING_MACRO(H5Pset_fapl_ros3);

  GOTCHA_BINDING_MACRO(H5FD_sec2_init);

  GOTCHA_BINDING_MACRO(H5Pset_fapl_sec2);

  GOTCHA_BINDING_MACRO(H5FD_splitter_init);

  GOTCHA_BINDING_MACRO(H5Pset_fapl_splitter);

  GOTCHA_BINDING_MACRO(H5Pget_fapl_splitter);

  GOTCHA_BINDING_MACRO(H5FD_stdio_init);

  GOTCHA_BINDING_MACRO(H5Pset_fapl_stdio);

  return 0;
}

size_t count_hdf5() { return 614; }

namespace brahma {
std::shared_ptr<HDF5> HDF5::my_instance = nullptr;

std::shared_ptr<HDF5> HDF5::get_instance() {
  if (my_instance == nullptr) {
    BRAHMA_LOG_INFO("HDF5 class not intercepted but used", "");
    my_instance = std::make_shared<HDF5>();
  }
  return my_instance;
}

int HDF5::set_instance(std::shared_ptr<HDF5> instance_i) {
  if (instance_i != nullptr) {
    my_instance = instance_i;
    return 0;
  } else {
    BRAHMA_LOG_ERROR("%s instance_i is not set", "HDF5");
    throw std::runtime_error("instance_i is not set");
  }
}

herr_t HDF5::H5open() {
  BRAHMA_UNWRAPPED_FUNC(H5open, herr_t, ());
  return result;
}

herr_t HDF5::H5close() {
  BRAHMA_UNWRAPPED_FUNC(H5close, herr_t, ());
  return result;
}

herr_t HDF5::H5dont_atexit() {
  BRAHMA_UNWRAPPED_FUNC(H5dont_atexit, herr_t, ());
  return result;
}

herr_t HDF5::H5garbage_collect() {
  BRAHMA_UNWRAPPED_FUNC(H5garbage_collect, herr_t, ());
  return result;
}

herr_t HDF5::H5set_free_list_limits(int reg_global_lim, int reg_list_lim,
                                    int arr_global_lim, int arr_list_lim,
                                    int blk_global_lim, int blk_list_lim) {
  BRAHMA_UNWRAPPED_FUNC(H5set_free_list_limits, herr_t,
                        (reg_global_lim, reg_list_lim, arr_global_lim,
                         arr_list_lim, blk_global_lim, blk_list_lim));
  return result;
}

herr_t HDF5::H5get_free_list_sizes(size_t *reg_size, size_t *arr_size,
                                   size_t *blk_size, size_t *fac_size) {
  BRAHMA_UNWRAPPED_FUNC(H5get_free_list_sizes, herr_t,
                        (reg_size, arr_size, blk_size, fac_size));
  return result;
}

herr_t HDF5::H5get_alloc_stats(H5_alloc_stats_t *stats) {
  BRAHMA_UNWRAPPED_FUNC(H5get_alloc_stats, herr_t, (stats));
  return result;
}

herr_t HDF5::H5get_libversion(unsigned int *majnum, unsigned int *minnum,
                              unsigned int *relnum) {
  BRAHMA_UNWRAPPED_FUNC(H5get_libversion, herr_t, (majnum, minnum, relnum));
  return result;
}

herr_t HDF5::H5check_version(unsigned int majnum, unsigned int minnum,
                             unsigned int relnum) {
  BRAHMA_UNWRAPPED_FUNC(H5check_version, herr_t, (majnum, minnum, relnum));
  return result;
}

herr_t HDF5::H5is_library_threadsafe(hbool_t *is_ts) {
  BRAHMA_UNWRAPPED_FUNC(H5is_library_threadsafe, herr_t, (is_ts));
  return result;
}

herr_t HDF5::H5free_memory(void *mem) {
  BRAHMA_UNWRAPPED_FUNC(H5free_memory, herr_t, (mem));
  return result;
}

void *HDF5::H5allocate_memory(size_t size, hbool_t clear) {
  BRAHMA_UNWRAPPED_FUNC(H5allocate_memory, void *, (size, clear));
  return result;
}

void *HDF5::H5resize_memory(void *mem, size_t size) {
  BRAHMA_UNWRAPPED_FUNC(H5resize_memory, void *, (mem, size));
  return result;
}

hid_t HDF5::H5Iregister(H5I_type_t type, const void *object) {
  BRAHMA_UNWRAPPED_FUNC(H5Iregister, hid_t, (type, object));
  return result;
}

void *HDF5::H5Iobject_verify(hid_t id, H5I_type_t id_type) {
  BRAHMA_UNWRAPPED_FUNC(H5Iobject_verify, void *, (id, id_type));
  return result;
}

void *HDF5::H5Iremove_verify(hid_t id, H5I_type_t id_type) {
  BRAHMA_UNWRAPPED_FUNC(H5Iremove_verify, void *, (id, id_type));
  return result;
}

H5I_type_t HDF5::H5Iget_type(hid_t id) {
  BRAHMA_UNWRAPPED_FUNC(H5Iget_type, H5I_type_t, (id));
  return result;
}

hid_t HDF5::H5Iget_file_id(hid_t id) {
  BRAHMA_UNWRAPPED_FUNC(H5Iget_file_id, hid_t, (id));
  return result;
}

ssize_t HDF5::H5Iget_name(hid_t id, char *name, size_t size) {
  BRAHMA_UNWRAPPED_FUNC(H5Iget_name, ssize_t, (id, name, size));
  return result;
}

int HDF5::H5Iinc_ref(hid_t id) {
  BRAHMA_UNWRAPPED_FUNC(H5Iinc_ref, int, (id));
  return result;
}

int HDF5::H5Idec_ref(hid_t id) {
  BRAHMA_UNWRAPPED_FUNC(H5Idec_ref, int, (id));
  return result;
}

int HDF5::H5Iget_ref(hid_t id) {
  BRAHMA_UNWRAPPED_FUNC(H5Iget_ref, int, (id));
  return result;
}

H5I_type_t HDF5::H5Iregister_type(size_t hash_size, unsigned int reserved,
                                  H5I_free_t free_func) {
  BRAHMA_UNWRAPPED_FUNC(H5Iregister_type, H5I_type_t,
                        (hash_size, reserved, free_func));
  return result;
}

herr_t HDF5::H5Iclear_type(H5I_type_t type, hbool_t force) {
  BRAHMA_UNWRAPPED_FUNC(H5Iclear_type, herr_t, (type, force));
  return result;
}

herr_t HDF5::H5Idestroy_type(H5I_type_t type) {
  BRAHMA_UNWRAPPED_FUNC(H5Idestroy_type, herr_t, (type));
  return result;
}

int HDF5::H5Iinc_type_ref(H5I_type_t type) {
  BRAHMA_UNWRAPPED_FUNC(H5Iinc_type_ref, int, (type));
  return result;
}

int HDF5::H5Idec_type_ref(H5I_type_t type) {
  BRAHMA_UNWRAPPED_FUNC(H5Idec_type_ref, int, (type));
  return result;
}

int HDF5::H5Iget_type_ref(H5I_type_t type) {
  BRAHMA_UNWRAPPED_FUNC(H5Iget_type_ref, int, (type));
  return result;
}

void *HDF5::H5Isearch(H5I_type_t type, H5I_search_func_t func, void *key) {
  BRAHMA_UNWRAPPED_FUNC(H5Isearch, void *, (type, func, key));
  return result;
}

herr_t HDF5::H5Inmembers(H5I_type_t type, hsize_t *num_members) {
  BRAHMA_UNWRAPPED_FUNC(H5Inmembers, herr_t, (type, num_members));
  return result;
}

htri_t HDF5::H5Itype_exists(H5I_type_t type) {
  BRAHMA_UNWRAPPED_FUNC(H5Itype_exists, htri_t, (type));
  return result;
}

htri_t HDF5::H5Iis_valid(hid_t id) {
  BRAHMA_UNWRAPPED_FUNC(H5Iis_valid, htri_t, (id));
  return result;
}

hid_t HDF5::H5Tcreate(H5T_class_t type, size_t size) {
  BRAHMA_UNWRAPPED_FUNC(H5Tcreate, hid_t, (type, size));
  return result;
}

hid_t HDF5::H5Tcopy(hid_t type_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Tcopy, hid_t, (type_id));
  return result;
}

herr_t HDF5::H5Tclose(hid_t type_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Tclose, herr_t, (type_id));
  return result;
}

htri_t HDF5::H5Tequal(hid_t type1_id, hid_t type2_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Tequal, htri_t, (type1_id, type2_id));
  return result;
}

herr_t HDF5::H5Tlock(hid_t type_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Tlock, herr_t, (type_id));
  return result;
}

herr_t HDF5::H5Tcommit2(hid_t loc_id, const char *name, hid_t type_id,
                        hid_t lcpl_id, hid_t tcpl_id, hid_t tapl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Tcommit2, herr_t,
                        (loc_id, name, type_id, lcpl_id, tcpl_id, tapl_id));
  return result;
}

hid_t HDF5::H5Topen2(hid_t loc_id, const char *name, hid_t tapl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Topen2, hid_t, (loc_id, name, tapl_id));
  return result;
}

herr_t HDF5::H5Tcommit_anon(hid_t loc_id, hid_t type_id, hid_t tcpl_id,
                            hid_t tapl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Tcommit_anon, herr_t,
                        (loc_id, type_id, tcpl_id, tapl_id));
  return result;
}

hid_t HDF5::H5Tget_create_plist(hid_t type_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Tget_create_plist, hid_t, (type_id));
  return result;
}

htri_t HDF5::H5Tcommitted(hid_t type_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Tcommitted, htri_t, (type_id));
  return result;
}

herr_t HDF5::H5Tencode(hid_t obj_id, void *buf, size_t *nalloc) {
  BRAHMA_UNWRAPPED_FUNC(H5Tencode, herr_t, (obj_id, buf, nalloc));
  return result;
}

hid_t HDF5::H5Tdecode(const void *buf) {
  BRAHMA_UNWRAPPED_FUNC(H5Tdecode, hid_t, (buf));
  return result;
}

herr_t HDF5::H5Tflush(hid_t type_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Tflush, herr_t, (type_id));
  return result;
}

herr_t HDF5::H5Trefresh(hid_t type_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Trefresh, herr_t, (type_id));
  return result;
}

herr_t HDF5::H5Tinsert(hid_t parent_id, const char *name, size_t offset,
                       hid_t member_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Tinsert, herr_t,
                        (parent_id, name, offset, member_id));
  return result;
}

herr_t HDF5::H5Tpack(hid_t type_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Tpack, herr_t, (type_id));
  return result;
}

hid_t HDF5::H5Tenum_create(hid_t base_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Tenum_create, hid_t, (base_id));
  return result;
}

herr_t HDF5::H5Tenum_insert(hid_t type, const char *name, const void *value) {
  BRAHMA_UNWRAPPED_FUNC(H5Tenum_insert, herr_t, (type, name, value));
  return result;
}

herr_t HDF5::H5Tenum_nameof(hid_t type, const void *value, char *name,
                            size_t size) {
  BRAHMA_UNWRAPPED_FUNC(H5Tenum_nameof, herr_t, (type, value, name, size));
  return result;
}

herr_t HDF5::H5Tenum_valueof(hid_t type, const char *name, void *value) {
  BRAHMA_UNWRAPPED_FUNC(H5Tenum_valueof, herr_t, (type, name, value));
  return result;
}

hid_t HDF5::H5Tvlen_create(hid_t base_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Tvlen_create, hid_t, (base_id));
  return result;
}

hid_t HDF5::H5Tarray_create2(hid_t base_id, unsigned int ndims,
                             const hsize_t dim[]) {
  BRAHMA_UNWRAPPED_FUNC(H5Tarray_create2, hid_t, (base_id, ndims, dim));
  return result;
}

int HDF5::H5Tget_array_ndims(hid_t type_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Tget_array_ndims, int, (type_id));
  return result;
}

int HDF5::H5Tget_array_dims2(hid_t type_id, hsize_t dims[]) {
  BRAHMA_UNWRAPPED_FUNC(H5Tget_array_dims2, int, (type_id, dims));
  return result;
}

herr_t HDF5::H5Tset_tag(hid_t type, const char *tag) {
  BRAHMA_UNWRAPPED_FUNC(H5Tset_tag, herr_t, (type, tag));
  return result;
}

char *HDF5::H5Tget_tag(hid_t type) {
  BRAHMA_UNWRAPPED_FUNC(H5Tget_tag, char *, (type));
  return result;
}

hid_t HDF5::H5Tget_super(hid_t type) {
  BRAHMA_UNWRAPPED_FUNC(H5Tget_super, hid_t, (type));
  return result;
}

H5T_class_t HDF5::H5Tget_class(hid_t type_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Tget_class, H5T_class_t, (type_id));
  return result;
}

htri_t HDF5::H5Tdetect_class(hid_t type_id, H5T_class_t cls) {
  BRAHMA_UNWRAPPED_FUNC(H5Tdetect_class, htri_t, (type_id, cls));
  return result;
}

size_t HDF5::H5Tget_size(hid_t type_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Tget_size, size_t, (type_id));
  return result;
}

H5T_order_t HDF5::H5Tget_order(hid_t type_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Tget_order, H5T_order_t, (type_id));
  return result;
}

size_t HDF5::H5Tget_precision(hid_t type_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Tget_precision, size_t, (type_id));
  return result;
}

int HDF5::H5Tget_offset(hid_t type_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Tget_offset, int, (type_id));
  return result;
}

herr_t HDF5::H5Tget_pad(hid_t type_id, H5T_pad_t *lsb, H5T_pad_t *msb) {
  BRAHMA_UNWRAPPED_FUNC(H5Tget_pad, herr_t, (type_id, lsb, msb));
  return result;
}

H5T_sign_t HDF5::H5Tget_sign(hid_t type_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Tget_sign, H5T_sign_t, (type_id));
  return result;
}

herr_t HDF5::H5Tget_fields(hid_t type_id, size_t *spos, size_t *epos,
                           size_t *esize, size_t *mpos, size_t *msize) {
  BRAHMA_UNWRAPPED_FUNC(H5Tget_fields, herr_t,
                        (type_id, spos, epos, esize, mpos, msize));
  return result;
}

size_t HDF5::H5Tget_ebias(hid_t type_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Tget_ebias, size_t, (type_id));
  return result;
}

H5T_norm_t HDF5::H5Tget_norm(hid_t type_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Tget_norm, H5T_norm_t, (type_id));
  return result;
}

H5T_pad_t HDF5::H5Tget_inpad(hid_t type_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Tget_inpad, H5T_pad_t, (type_id));
  return result;
}

H5T_str_t HDF5::H5Tget_strpad(hid_t type_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Tget_strpad, H5T_str_t, (type_id));
  return result;
}

int HDF5::H5Tget_nmembers(hid_t type_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Tget_nmembers, int, (type_id));
  return result;
}

char *HDF5::H5Tget_member_name(hid_t type_id, unsigned int membno) {
  BRAHMA_UNWRAPPED_FUNC(H5Tget_member_name, char *, (type_id, membno));
  return result;
}

int HDF5::H5Tget_member_index(hid_t type_id, const char *name) {
  BRAHMA_UNWRAPPED_FUNC(H5Tget_member_index, int, (type_id, name));
  return result;
}

size_t HDF5::H5Tget_member_offset(hid_t type_id, unsigned int membno) {
  BRAHMA_UNWRAPPED_FUNC(H5Tget_member_offset, size_t, (type_id, membno));
  return result;
}

H5T_class_t HDF5::H5Tget_member_class(hid_t type_id, unsigned int membno) {
  BRAHMA_UNWRAPPED_FUNC(H5Tget_member_class, H5T_class_t, (type_id, membno));
  return result;
}

hid_t HDF5::H5Tget_member_type(hid_t type_id, unsigned int membno) {
  BRAHMA_UNWRAPPED_FUNC(H5Tget_member_type, hid_t, (type_id, membno));
  return result;
}

herr_t HDF5::H5Tget_member_value(hid_t type_id, unsigned int membno,
                                 void *value) {
  BRAHMA_UNWRAPPED_FUNC(H5Tget_member_value, herr_t, (type_id, membno, value));
  return result;
}

H5T_cset_t HDF5::H5Tget_cset(hid_t type_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Tget_cset, H5T_cset_t, (type_id));
  return result;
}

htri_t HDF5::H5Tis_variable_str(hid_t type_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Tis_variable_str, htri_t, (type_id));
  return result;
}

hid_t HDF5::H5Tget_native_type(hid_t type_id, H5T_direction_t direction) {
  BRAHMA_UNWRAPPED_FUNC(H5Tget_native_type, hid_t, (type_id, direction));
  return result;
}

herr_t HDF5::H5Tset_size(hid_t type_id, size_t size) {
  BRAHMA_UNWRAPPED_FUNC(H5Tset_size, herr_t, (type_id, size));
  return result;
}

herr_t HDF5::H5Tset_order(hid_t type_id, H5T_order_t order) {
  BRAHMA_UNWRAPPED_FUNC(H5Tset_order, herr_t, (type_id, order));
  return result;
}

herr_t HDF5::H5Tset_precision(hid_t type_id, size_t prec) {
  BRAHMA_UNWRAPPED_FUNC(H5Tset_precision, herr_t, (type_id, prec));
  return result;
}

herr_t HDF5::H5Tset_offset(hid_t type_id, size_t offset) {
  BRAHMA_UNWRAPPED_FUNC(H5Tset_offset, herr_t, (type_id, offset));
  return result;
}

herr_t HDF5::H5Tset_pad(hid_t type_id, H5T_pad_t lsb, H5T_pad_t msb) {
  BRAHMA_UNWRAPPED_FUNC(H5Tset_pad, herr_t, (type_id, lsb, msb));
  return result;
}

herr_t HDF5::H5Tset_sign(hid_t type_id, H5T_sign_t sign) {
  BRAHMA_UNWRAPPED_FUNC(H5Tset_sign, herr_t, (type_id, sign));
  return result;
}

herr_t HDF5::H5Tset_fields(hid_t type_id, size_t spos, size_t epos,
                           size_t esize, size_t mpos, size_t msize) {
  BRAHMA_UNWRAPPED_FUNC(H5Tset_fields, herr_t,
                        (type_id, spos, epos, esize, mpos, msize));
  return result;
}

herr_t HDF5::H5Tset_ebias(hid_t type_id, size_t ebias) {
  BRAHMA_UNWRAPPED_FUNC(H5Tset_ebias, herr_t, (type_id, ebias));
  return result;
}

herr_t HDF5::H5Tset_norm(hid_t type_id, H5T_norm_t norm) {
  BRAHMA_UNWRAPPED_FUNC(H5Tset_norm, herr_t, (type_id, norm));
  return result;
}

herr_t HDF5::H5Tset_inpad(hid_t type_id, H5T_pad_t pad) {
  BRAHMA_UNWRAPPED_FUNC(H5Tset_inpad, herr_t, (type_id, pad));
  return result;
}

herr_t HDF5::H5Tset_cset(hid_t type_id, H5T_cset_t cset) {
  BRAHMA_UNWRAPPED_FUNC(H5Tset_cset, herr_t, (type_id, cset));
  return result;
}

herr_t HDF5::H5Tset_strpad(hid_t type_id, H5T_str_t strpad) {
  BRAHMA_UNWRAPPED_FUNC(H5Tset_strpad, herr_t, (type_id, strpad));
  return result;
}

herr_t HDF5::H5Tregister(H5T_pers_t pers, const char *name, hid_t src_id,
                         hid_t dst_id, H5T_conv_t func) {
  BRAHMA_UNWRAPPED_FUNC(H5Tregister, herr_t,
                        (pers, name, src_id, dst_id, func));
  return result;
}

herr_t HDF5::H5Tunregister(H5T_pers_t pers, const char *name, hid_t src_id,
                           hid_t dst_id, H5T_conv_t func) {
  BRAHMA_UNWRAPPED_FUNC(H5Tunregister, herr_t,
                        (pers, name, src_id, dst_id, func));
  return result;
}

H5T_conv_t HDF5::H5Tfind(hid_t src_id, hid_t dst_id, H5T_cdata_t **pcdata) {
  BRAHMA_UNWRAPPED_FUNC(H5Tfind, H5T_conv_t, (src_id, dst_id, pcdata));
  return result;
}

htri_t HDF5::H5Tcompiler_conv(hid_t src_id, hid_t dst_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Tcompiler_conv, htri_t, (src_id, dst_id));
  return result;
}

herr_t HDF5::H5Tconvert(hid_t src_id, hid_t dst_id, size_t nelmts, void *buf,
                        void *background, hid_t plist_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Tconvert, herr_t,
                        (src_id, dst_id, nelmts, buf, background, plist_id));
  return result;
}

herr_t HDF5::H5Tcommit1(hid_t loc_id, const char *name, hid_t type_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Tcommit1, herr_t, (loc_id, name, type_id));
  return result;
}

hid_t HDF5::H5Topen1(hid_t loc_id, const char *name) {
  BRAHMA_UNWRAPPED_FUNC(H5Topen1, hid_t, (loc_id, name));
  return result;
}

hid_t HDF5::H5Tarray_create1(hid_t base_id, int ndims, const hsize_t dim[],
                             const int perm[]) {
  BRAHMA_UNWRAPPED_FUNC(H5Tarray_create1, hid_t, (base_id, ndims, dim, perm));
  return result;
}

int HDF5::H5Tget_array_dims1(hid_t type_id, hsize_t dims[], int perm[]) {
  BRAHMA_UNWRAPPED_FUNC(H5Tget_array_dims1, int, (type_id, dims, perm));
  return result;
}

herr_t HDF5::H5Lmove(hid_t src_loc, const char *src_name, hid_t dst_loc,
                     const char *dst_name, hid_t lcpl_id, hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Lmove, herr_t,
      (src_loc, src_name, dst_loc, dst_name, lcpl_id, lapl_id));
  return result;
}

herr_t HDF5::H5Lcopy(hid_t src_loc, const char *src_name, hid_t dst_loc,
                     const char *dst_name, hid_t lcpl_id, hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Lcopy, herr_t,
      (src_loc, src_name, dst_loc, dst_name, lcpl_id, lapl_id));
  return result;
}

herr_t HDF5::H5Lcreate_hard(hid_t cur_loc, const char *cur_name, hid_t dst_loc,
                            const char *dst_name, hid_t lcpl_id,
                            hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Lcreate_hard, herr_t,
      (cur_loc, cur_name, dst_loc, dst_name, lcpl_id, lapl_id));
  return result;
}

herr_t HDF5::H5Lcreate_soft(const char *link_target, hid_t link_loc_id,
                            const char *link_name, hid_t lcpl_id,
                            hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Lcreate_soft, herr_t,
      (link_target, link_loc_id, link_name, lcpl_id, lapl_id));
  return result;
}

herr_t HDF5::H5Ldelete(hid_t loc_id, const char *name, hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Ldelete, herr_t, (loc_id, name, lapl_id));
  return result;
}

herr_t HDF5::H5Ldelete_by_idx(hid_t loc_id, const char *group_name,
                              H5_index_t idx_type, H5_iter_order_t order,
                              hsize_t n, hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Ldelete_by_idx, herr_t,
                        (loc_id, group_name, idx_type, order, n, lapl_id));
  return result;
}

herr_t HDF5::H5Lget_val(hid_t loc_id, const char *name, void *buf, size_t size,
                        hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Lget_val, herr_t, (loc_id, name, buf, size, lapl_id));
  return result;
}

herr_t HDF5::H5Lget_val_by_idx(hid_t loc_id, const char *group_name,
                               H5_index_t idx_type, H5_iter_order_t order,
                               hsize_t n, void *buf, size_t size,
                               hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Lget_val_by_idx, herr_t,
      (loc_id, group_name, idx_type, order, n, buf, size, lapl_id));
  return result;
}

htri_t HDF5::H5Lexists(hid_t loc_id, const char *name, hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Lexists, htri_t, (loc_id, name, lapl_id));
  return result;
}

herr_t HDF5::H5Lget_info(hid_t loc_id, const char *name, H5L_info_t *linfo,
                         hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Lget_info, herr_t, (loc_id, name, linfo, lapl_id));
  return result;
}

herr_t HDF5::H5Lget_info_by_idx(hid_t loc_id, const char *group_name,
                                H5_index_t idx_type, H5_iter_order_t order,
                                hsize_t n, H5L_info_t *linfo, hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Lget_info_by_idx, herr_t,
      (loc_id, group_name, idx_type, order, n, linfo, lapl_id));
  return result;
}

ssize_t HDF5::H5Lget_name_by_idx(hid_t loc_id, const char *group_name,
                                 H5_index_t idx_type, H5_iter_order_t order,
                                 hsize_t n, char *name, size_t size,
                                 hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Lget_name_by_idx, ssize_t,
      (loc_id, group_name, idx_type, order, n, name, size, lapl_id));
  return result;
}

herr_t HDF5::H5Literate(hid_t grp_id, H5_index_t idx_type,
                        H5_iter_order_t order, hsize_t *idx, H5L_iterate_t op,
                        void *op_data) {
  BRAHMA_UNWRAPPED_FUNC(H5Literate, herr_t,
                        (grp_id, idx_type, order, idx, op, op_data));
  return result;
}

herr_t HDF5::H5Literate_by_name(hid_t loc_id, const char *group_name,
                                H5_index_t idx_type, H5_iter_order_t order,
                                hsize_t *idx, H5L_iterate_t op, void *op_data,
                                hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Literate_by_name, herr_t,
      (loc_id, group_name, idx_type, order, idx, op, op_data, lapl_id));
  return result;
}

herr_t HDF5::H5Lvisit(hid_t grp_id, H5_index_t idx_type, H5_iter_order_t order,
                      H5L_iterate_t op, void *op_data) {
  BRAHMA_UNWRAPPED_FUNC(H5Lvisit, herr_t,
                        (grp_id, idx_type, order, op, op_data));
  return result;
}

herr_t HDF5::H5Lvisit_by_name(hid_t loc_id, const char *group_name,
                              H5_index_t idx_type, H5_iter_order_t order,
                              H5L_iterate_t op, void *op_data, hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Lvisit_by_name, herr_t,
      (loc_id, group_name, idx_type, order, op, op_data, lapl_id));
  return result;
}

herr_t HDF5::H5Lcreate_ud(hid_t link_loc_id, const char *link_name,
                          H5L_type_t link_type, const void *udata,
                          size_t udata_size, hid_t lcpl_id, hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Lcreate_ud, herr_t,
      (link_loc_id, link_name, link_type, udata, udata_size, lcpl_id, lapl_id));
  return result;
}

herr_t HDF5::H5Lregister(const H5L_class_t *cls) {
  BRAHMA_UNWRAPPED_FUNC(H5Lregister, herr_t, (cls));
  return result;
}

herr_t HDF5::H5Lunregister(H5L_type_t id) {
  BRAHMA_UNWRAPPED_FUNC(H5Lunregister, herr_t, (id));
  return result;
}

htri_t HDF5::H5Lis_registered(H5L_type_t id) {
  BRAHMA_UNWRAPPED_FUNC(H5Lis_registered, htri_t, (id));
  return result;
}

herr_t HDF5::H5Lunpack_elink_val(const void *ext_linkval, size_t link_size,
                                 unsigned int *flags, const char **filename,
                                 const char **obj_path) {
  BRAHMA_UNWRAPPED_FUNC(H5Lunpack_elink_val, herr_t,
                        (ext_linkval, link_size, flags, filename, obj_path));
  return result;
}

herr_t HDF5::H5Lcreate_external(const char *file_name, const char *obj_name,
                                hid_t link_loc_id, const char *link_name,
                                hid_t lcpl_id, hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Lcreate_external, herr_t,
      (file_name, obj_name, link_loc_id, link_name, lcpl_id, lapl_id));
  return result;
}

hid_t HDF5::H5Oopen(hid_t loc_id, const char *name, hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Oopen, hid_t, (loc_id, name, lapl_id));
  return result;
}

hid_t HDF5::H5Oopen_by_addr(hid_t loc_id, haddr_t addr) {
  BRAHMA_UNWRAPPED_FUNC(H5Oopen_by_addr, hid_t, (loc_id, addr));
  return result;
}

hid_t HDF5::H5Oopen_by_idx(hid_t loc_id, const char *group_name,
                           H5_index_t idx_type, H5_iter_order_t order,
                           hsize_t n, hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Oopen_by_idx, hid_t,
                        (loc_id, group_name, idx_type, order, n, lapl_id));
  return result;
}

htri_t HDF5::H5Oexists_by_name(hid_t loc_id, const char *name, hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Oexists_by_name, htri_t, (loc_id, name, lapl_id));
  return result;
}

herr_t HDF5::H5Oget_info2(hid_t loc_id, H5O_info_t *oinfo,
                          unsigned int fields) {
  BRAHMA_UNWRAPPED_FUNC(H5Oget_info2, herr_t, (loc_id, oinfo, fields));
  return result;
}

herr_t HDF5::H5Oget_info_by_name2(hid_t loc_id, const char *name,
                                  H5O_info_t *oinfo, unsigned int fields,
                                  hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Oget_info_by_name2, herr_t,
                        (loc_id, name, oinfo, fields, lapl_id));
  return result;
}

herr_t HDF5::H5Oget_info_by_idx2(hid_t loc_id, const char *group_name,
                                 H5_index_t idx_type, H5_iter_order_t order,
                                 hsize_t n, H5O_info_t *oinfo,
                                 unsigned int fields, hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Oget_info_by_idx2, herr_t,
      (loc_id, group_name, idx_type, order, n, oinfo, fields, lapl_id));
  return result;
}

herr_t HDF5::H5Olink(hid_t obj_id, hid_t new_loc_id, const char *new_name,
                     hid_t lcpl_id, hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Olink, herr_t,
                        (obj_id, new_loc_id, new_name, lcpl_id, lapl_id));
  return result;
}

herr_t HDF5::H5Oincr_refcount(hid_t object_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Oincr_refcount, herr_t, (object_id));
  return result;
}

herr_t HDF5::H5Odecr_refcount(hid_t object_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Odecr_refcount, herr_t, (object_id));
  return result;
}

herr_t HDF5::H5Ocopy(hid_t src_loc_id, const char *src_name, hid_t dst_loc_id,
                     const char *dst_name, hid_t ocpypl_id, hid_t lcpl_id) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Ocopy, herr_t,
      (src_loc_id, src_name, dst_loc_id, dst_name, ocpypl_id, lcpl_id));
  return result;
}

herr_t HDF5::H5Oset_comment(hid_t obj_id, const char *comment) {
  BRAHMA_UNWRAPPED_FUNC(H5Oset_comment, herr_t, (obj_id, comment));
  return result;
}

herr_t HDF5::H5Oset_comment_by_name(hid_t loc_id, const char *name,
                                    const char *comment, hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Oset_comment_by_name, herr_t,
                        (loc_id, name, comment, lapl_id));
  return result;
}

ssize_t HDF5::H5Oget_comment(hid_t obj_id, char *comment, size_t bufsize) {
  BRAHMA_UNWRAPPED_FUNC(H5Oget_comment, ssize_t, (obj_id, comment, bufsize));
  return result;
}

ssize_t HDF5::H5Oget_comment_by_name(hid_t loc_id, const char *name,
                                     char *comment, size_t bufsize,
                                     hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Oget_comment_by_name, ssize_t,
                        (loc_id, name, comment, bufsize, lapl_id));
  return result;
}

herr_t HDF5::H5Ovisit2(hid_t obj_id, H5_index_t idx_type, H5_iter_order_t order,
                       H5O_iterate_t op, void *op_data, unsigned int fields) {
  BRAHMA_UNWRAPPED_FUNC(H5Ovisit2, herr_t,
                        (obj_id, idx_type, order, op, op_data, fields));
  return result;
}

herr_t HDF5::H5Ovisit_by_name2(hid_t loc_id, const char *obj_name,
                               H5_index_t idx_type, H5_iter_order_t order,
                               H5O_iterate_t op, void *op_data,
                               unsigned int fields, hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Ovisit_by_name2, herr_t,
      (loc_id, obj_name, idx_type, order, op, op_data, fields, lapl_id));
  return result;
}

herr_t HDF5::H5Oclose(hid_t object_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Oclose, herr_t, (object_id));
  return result;
}

herr_t HDF5::H5Oflush(hid_t obj_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Oflush, herr_t, (obj_id));
  return result;
}

herr_t HDF5::H5Orefresh(hid_t oid) {
  BRAHMA_UNWRAPPED_FUNC(H5Orefresh, herr_t, (oid));
  return result;
}

herr_t HDF5::H5Odisable_mdc_flushes(hid_t object_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Odisable_mdc_flushes, herr_t, (object_id));
  return result;
}

herr_t HDF5::H5Oenable_mdc_flushes(hid_t object_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Oenable_mdc_flushes, herr_t, (object_id));
  return result;
}

herr_t HDF5::H5Oare_mdc_flushes_disabled(hid_t object_id,
                                         hbool_t *are_disabled) {
  BRAHMA_UNWRAPPED_FUNC(H5Oare_mdc_flushes_disabled, herr_t,
                        (object_id, are_disabled));
  return result;
}

herr_t HDF5::H5Oget_info(hid_t loc_id, H5O_info_t *oinfo) {
  BRAHMA_UNWRAPPED_FUNC(H5Oget_info, herr_t, (loc_id, oinfo));
  return result;
}

herr_t HDF5::H5Oget_info_by_name(hid_t loc_id, const char *name,
                                 H5O_info_t *oinfo, hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Oget_info_by_name, herr_t,
                        (loc_id, name, oinfo, lapl_id));
  return result;
}

herr_t HDF5::H5Oget_info_by_idx(hid_t loc_id, const char *group_name,
                                H5_index_t idx_type, H5_iter_order_t order,
                                hsize_t n, H5O_info_t *oinfo, hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Oget_info_by_idx, herr_t,
      (loc_id, group_name, idx_type, order, n, oinfo, lapl_id));
  return result;
}

herr_t HDF5::H5Ovisit(hid_t obj_id, H5_index_t idx_type, H5_iter_order_t order,
                      H5O_iterate_t op, void *op_data) {
  BRAHMA_UNWRAPPED_FUNC(H5Ovisit, herr_t,
                        (obj_id, idx_type, order, op, op_data));
  return result;
}

herr_t HDF5::H5Ovisit_by_name(hid_t loc_id, const char *obj_name,
                              H5_index_t idx_type, H5_iter_order_t order,
                              H5O_iterate_t op, void *op_data, hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Ovisit_by_name, herr_t,
      (loc_id, obj_name, idx_type, order, op, op_data, lapl_id));
  return result;
}

herr_t HDF5::H5Oget_info1(hid_t loc_id, H5O_info_t *oinfo) {
  BRAHMA_UNWRAPPED_FUNC(H5Oget_info1, herr_t, (loc_id, oinfo));
  return result;
}

herr_t HDF5::H5Oget_info_by_name1(hid_t loc_id, const char *name,
                                  H5O_info_t *oinfo, hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Oget_info_by_name1, herr_t,
                        (loc_id, name, oinfo, lapl_id));
  return result;
}

herr_t HDF5::H5Oget_info_by_idx1(hid_t loc_id, const char *group_name,
                                 H5_index_t idx_type, H5_iter_order_t order,
                                 hsize_t n, H5O_info_t *oinfo, hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Oget_info_by_idx1, herr_t,
      (loc_id, group_name, idx_type, order, n, oinfo, lapl_id));
  return result;
}

herr_t HDF5::H5Ovisit1(hid_t obj_id, H5_index_t idx_type, H5_iter_order_t order,
                       H5O_iterate_t op, void *op_data) {
  BRAHMA_UNWRAPPED_FUNC(H5Ovisit1, herr_t,
                        (obj_id, idx_type, order, op, op_data));
  return result;
}

herr_t HDF5::H5Ovisit_by_name1(hid_t loc_id, const char *obj_name,
                               H5_index_t idx_type, H5_iter_order_t order,
                               H5O_iterate_t op, void *op_data, hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Ovisit_by_name1, herr_t,
      (loc_id, obj_name, idx_type, order, op, op_data, lapl_id));
  return result;
}

hid_t HDF5::H5Acreate2(hid_t loc_id, const char *attr_name, hid_t type_id,
                       hid_t space_id, hid_t acpl_id, hid_t aapl_id) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Acreate2, hid_t,
      (loc_id, attr_name, type_id, space_id, acpl_id, aapl_id));
  return result;
}

hid_t HDF5::H5Acreate_by_name(hid_t loc_id, const char *obj_name,
                              const char *attr_name, hid_t type_id,
                              hid_t space_id, hid_t acpl_id, hid_t aapl_id,
                              hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Acreate_by_name, hid_t,
                        (loc_id, obj_name, attr_name, type_id, space_id,
                         acpl_id, aapl_id, lapl_id));
  return result;
}

hid_t HDF5::H5Aopen(hid_t obj_id, const char *attr_name, hid_t aapl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Aopen, hid_t, (obj_id, attr_name, aapl_id));
  return result;
}

hid_t HDF5::H5Aopen_by_name(hid_t loc_id, const char *obj_name,
                            const char *attr_name, hid_t aapl_id,
                            hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Aopen_by_name, hid_t,
                        (loc_id, obj_name, attr_name, aapl_id, lapl_id));
  return result;
}

hid_t HDF5::H5Aopen_by_idx(hid_t loc_id, const char *obj_name,
                           H5_index_t idx_type, H5_iter_order_t order,
                           hsize_t n, hid_t aapl_id, hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Aopen_by_idx, hid_t,
      (loc_id, obj_name, idx_type, order, n, aapl_id, lapl_id));
  return result;
}

herr_t HDF5::H5Awrite(hid_t attr_id, hid_t type_id, const void *buf) {
  BRAHMA_UNWRAPPED_FUNC(H5Awrite, herr_t, (attr_id, type_id, buf));
  return result;
}

herr_t HDF5::H5Aread(hid_t attr_id, hid_t type_id, void *buf) {
  BRAHMA_UNWRAPPED_FUNC(H5Aread, herr_t, (attr_id, type_id, buf));
  return result;
}

herr_t HDF5::H5Aclose(hid_t attr_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Aclose, herr_t, (attr_id));
  return result;
}

hid_t HDF5::H5Aget_space(hid_t attr_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Aget_space, hid_t, (attr_id));
  return result;
}

hid_t HDF5::H5Aget_type(hid_t attr_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Aget_type, hid_t, (attr_id));
  return result;
}

hid_t HDF5::H5Aget_create_plist(hid_t attr_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Aget_create_plist, hid_t, (attr_id));
  return result;
}

ssize_t HDF5::H5Aget_name(hid_t attr_id, size_t buf_size, char *buf) {
  BRAHMA_UNWRAPPED_FUNC(H5Aget_name, ssize_t, (attr_id, buf_size, buf));
  return result;
}

ssize_t HDF5::H5Aget_name_by_idx(hid_t loc_id, const char *obj_name,
                                 H5_index_t idx_type, H5_iter_order_t order,
                                 hsize_t n, char *name, size_t size,
                                 hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Aget_name_by_idx, ssize_t,
      (loc_id, obj_name, idx_type, order, n, name, size, lapl_id));
  return result;
}

hsize_t HDF5::H5Aget_storage_size(hid_t attr_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Aget_storage_size, hsize_t, (attr_id));
  return result;
}

herr_t HDF5::H5Aget_info(hid_t attr_id, H5A_info_t *ainfo) {
  BRAHMA_UNWRAPPED_FUNC(H5Aget_info, herr_t, (attr_id, ainfo));
  return result;
}

herr_t HDF5::H5Aget_info_by_name(hid_t loc_id, const char *obj_name,
                                 const char *attr_name, H5A_info_t *ainfo,
                                 hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Aget_info_by_name, herr_t,
                        (loc_id, obj_name, attr_name, ainfo, lapl_id));
  return result;
}

herr_t HDF5::H5Aget_info_by_idx(hid_t loc_id, const char *obj_name,
                                H5_index_t idx_type, H5_iter_order_t order,
                                hsize_t n, H5A_info_t *ainfo, hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Aget_info_by_idx, herr_t,
                        (loc_id, obj_name, idx_type, order, n, ainfo, lapl_id));
  return result;
}

herr_t HDF5::H5Arename(hid_t loc_id, const char *old_name,
                       const char *new_name) {
  BRAHMA_UNWRAPPED_FUNC(H5Arename, herr_t, (loc_id, old_name, new_name));
  return result;
}

herr_t HDF5::H5Arename_by_name(hid_t loc_id, const char *obj_name,
                               const char *old_attr_name,
                               const char *new_attr_name, hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Arename_by_name, herr_t,
      (loc_id, obj_name, old_attr_name, new_attr_name, lapl_id));
  return result;
}

herr_t HDF5::H5Aiterate2(hid_t loc_id, H5_index_t idx_type,
                         H5_iter_order_t order, hsize_t *idx,
                         H5A_operator2_t op, void *op_data) {
  BRAHMA_UNWRAPPED_FUNC(H5Aiterate2, herr_t,
                        (loc_id, idx_type, order, idx, op, op_data));
  return result;
}

herr_t HDF5::H5Aiterate_by_name(hid_t loc_id, const char *obj_name,
                                H5_index_t idx_type, H5_iter_order_t order,
                                hsize_t *idx, H5A_operator2_t op, void *op_data,
                                hid_t lapd_id) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Aiterate_by_name, herr_t,
      (loc_id, obj_name, idx_type, order, idx, op, op_data, lapd_id));
  return result;
}

herr_t HDF5::H5Adelete(hid_t loc_id, const char *name) {
  BRAHMA_UNWRAPPED_FUNC(H5Adelete, herr_t, (loc_id, name));
  return result;
}

herr_t HDF5::H5Adelete_by_name(hid_t loc_id, const char *obj_name,
                               const char *attr_name, hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Adelete_by_name, herr_t,
                        (loc_id, obj_name, attr_name, lapl_id));
  return result;
}

herr_t HDF5::H5Adelete_by_idx(hid_t loc_id, const char *obj_name,
                              H5_index_t idx_type, H5_iter_order_t order,
                              hsize_t n, hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Adelete_by_idx, herr_t,
                        (loc_id, obj_name, idx_type, order, n, lapl_id));
  return result;
}

htri_t HDF5::H5Aexists(hid_t obj_id, const char *attr_name) {
  BRAHMA_UNWRAPPED_FUNC(H5Aexists, htri_t, (obj_id, attr_name));
  return result;
}

htri_t HDF5::H5Aexists_by_name(hid_t obj_id, const char *obj_name,
                               const char *attr_name, hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Aexists_by_name, htri_t,
                        (obj_id, obj_name, attr_name, lapl_id));
  return result;
}

hid_t HDF5::H5Acreate1(hid_t loc_id, const char *name, hid_t type_id,
                       hid_t space_id, hid_t acpl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Acreate1, hid_t,
                        (loc_id, name, type_id, space_id, acpl_id));
  return result;
}

hid_t HDF5::H5Aopen_name(hid_t loc_id, const char *name) {
  BRAHMA_UNWRAPPED_FUNC(H5Aopen_name, hid_t, (loc_id, name));
  return result;
}

hid_t HDF5::H5Aopen_idx(hid_t loc_id, unsigned int idx) {
  BRAHMA_UNWRAPPED_FUNC(H5Aopen_idx, hid_t, (loc_id, idx));
  return result;
}

int HDF5::H5Aget_num_attrs(hid_t loc_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Aget_num_attrs, int, (loc_id));
  return result;
}

herr_t HDF5::H5Aiterate1(hid_t loc_id, unsigned int *attr_num,
                         H5A_operator1_t op, void *op_data) {
  BRAHMA_UNWRAPPED_FUNC(H5Aiterate1, herr_t, (loc_id, attr_num, op, op_data));
  return result;
}

hid_t HDF5::H5Dcreate2(hid_t loc_id, const char *name, hid_t type_id,
                       hid_t space_id, hid_t lcpl_id, hid_t dcpl_id,
                       hid_t dapl_id) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Dcreate2, hid_t,
      (loc_id, name, type_id, space_id, lcpl_id, dcpl_id, dapl_id));
  return result;
}

hid_t HDF5::H5Dcreate_anon(hid_t file_id, hid_t type_id, hid_t space_id,
                           hid_t plist_id, hid_t dapl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Dcreate_anon, hid_t,
                        (file_id, type_id, space_id, plist_id, dapl_id));
  return result;
}

hid_t HDF5::H5Dopen2(hid_t file_id, const char *name, hid_t dapl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Dopen2, hid_t, (file_id, name, dapl_id));
  return result;
}

herr_t HDF5::H5Dclose(hid_t dset_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Dclose, herr_t, (dset_id));
  return result;
}

hid_t HDF5::H5Dget_space(hid_t dset_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Dget_space, hid_t, (dset_id));
  return result;
}

herr_t HDF5::H5Dget_space_status(hid_t dset_id,
                                 H5D_space_status_t *allocation) {
  BRAHMA_UNWRAPPED_FUNC(H5Dget_space_status, herr_t, (dset_id, allocation));
  return result;
}

hid_t HDF5::H5Dget_type(hid_t dset_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Dget_type, hid_t, (dset_id));
  return result;
}

hid_t HDF5::H5Dget_create_plist(hid_t dset_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Dget_create_plist, hid_t, (dset_id));
  return result;
}

hid_t HDF5::H5Dget_access_plist(hid_t dset_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Dget_access_plist, hid_t, (dset_id));
  return result;
}

hsize_t HDF5::H5Dget_storage_size(hid_t dset_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Dget_storage_size, hsize_t, (dset_id));
  return result;
}

herr_t HDF5::H5Dget_chunk_storage_size(hid_t dset_id, const hsize_t *offset,
                                       hsize_t *chunk_bytes) {
  BRAHMA_UNWRAPPED_FUNC(H5Dget_chunk_storage_size, herr_t,
                        (dset_id, offset, chunk_bytes));
  return result;
}

herr_t HDF5::H5Dget_num_chunks(hid_t dset_id, hid_t fspace_id,
                               hsize_t *nchunks) {
  BRAHMA_UNWRAPPED_FUNC(H5Dget_num_chunks, herr_t,
                        (dset_id, fspace_id, nchunks));
  return result;
}

herr_t HDF5::H5Dget_chunk_info_by_coord(hid_t dset_id, const hsize_t *coord,
                                        unsigned int *filter_mask,
                                        haddr_t *addr, hsize_t *size) {
  BRAHMA_UNWRAPPED_FUNC(H5Dget_chunk_info_by_coord, herr_t,
                        (dset_id, coord, filter_mask, addr, size));
  return result;
}

herr_t HDF5::H5Dget_chunk_info(hid_t dset_id, hid_t fspace_id, hsize_t chk_idx,
                               hsize_t *coord, unsigned int *filter_mask,
                               haddr_t *addr, hsize_t *size) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Dget_chunk_info, herr_t,
      (dset_id, fspace_id, chk_idx, coord, filter_mask, addr, size));
  return result;
}

haddr_t HDF5::H5Dget_offset(hid_t dset_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Dget_offset, haddr_t, (dset_id));
  return result;
}

herr_t HDF5::H5Dread(hid_t dset_id, hid_t mem_type_id, hid_t mem_space_id,
                     hid_t file_space_id, hid_t plist_id, void *buf) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Dread, herr_t,
      (dset_id, mem_type_id, mem_space_id, file_space_id, plist_id, buf));
  return result;
}

herr_t HDF5::H5Dwrite(hid_t dset_id, hid_t mem_type_id, hid_t mem_space_id,
                      hid_t file_space_id, hid_t plist_id, const void *buf) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Dwrite, herr_t,
      (dset_id, mem_type_id, mem_space_id, file_space_id, plist_id, buf));
  return result;
}

herr_t HDF5::H5Dwrite_chunk(hid_t dset_id, hid_t dxpl_id, uint32_t filters,
                            const hsize_t *offset, size_t data_size,
                            const void *buf) {
  BRAHMA_UNWRAPPED_FUNC(H5Dwrite_chunk, herr_t,
                        (dset_id, dxpl_id, filters, offset, data_size, buf));
  return result;
}

herr_t HDF5::H5Dread_chunk(hid_t dset_id, hid_t dxpl_id, const hsize_t *offset,
                           uint32_t *filters, void *buf) {
  BRAHMA_UNWRAPPED_FUNC(H5Dread_chunk, herr_t,
                        (dset_id, dxpl_id, offset, filters, buf));
  return result;
}

herr_t HDF5::H5Diterate(void *buf, hid_t type_id, hid_t space_id,
                        H5D_operator_t op, void *operator_data) {
  BRAHMA_UNWRAPPED_FUNC(H5Diterate, herr_t,
                        (buf, type_id, space_id, op, operator_data));
  return result;
}

herr_t HDF5::H5Dvlen_reclaim(hid_t type_id, hid_t space_id, hid_t plist_id,
                             void *buf) {
  BRAHMA_UNWRAPPED_FUNC(H5Dvlen_reclaim, herr_t,
                        (type_id, space_id, plist_id, buf));
  return result;
}

herr_t HDF5::H5Dvlen_get_buf_size(hid_t dataset_id, hid_t type_id,
                                  hid_t space_id, hsize_t *size) {
  BRAHMA_UNWRAPPED_FUNC(H5Dvlen_get_buf_size, herr_t,
                        (dataset_id, type_id, space_id, size));
  return result;
}

herr_t HDF5::H5Dfill(const void *fill, hid_t fill_type, void *buf,
                     hid_t buf_type, hid_t space) {
  BRAHMA_UNWRAPPED_FUNC(H5Dfill, herr_t,
                        (fill, fill_type, buf, buf_type, space));
  return result;
}

herr_t HDF5::H5Dset_extent(hid_t dset_id, const hsize_t size[]) {
  BRAHMA_UNWRAPPED_FUNC(H5Dset_extent, herr_t, (dset_id, size));
  return result;
}

herr_t HDF5::H5Dflush(hid_t dset_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Dflush, herr_t, (dset_id));
  return result;
}

herr_t HDF5::H5Drefresh(hid_t dset_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Drefresh, herr_t, (dset_id));
  return result;
}

herr_t HDF5::H5Dscatter(H5D_scatter_func_t op, void *op_data, hid_t type_id,
                        hid_t dst_space_id, void *dst_buf) {
  BRAHMA_UNWRAPPED_FUNC(H5Dscatter, herr_t,
                        (op, op_data, type_id, dst_space_id, dst_buf));
  return result;
}

herr_t HDF5::H5Dgather(hid_t src_space_id, const void *src_buf, hid_t type_id,
                       size_t dst_buf_size, void *dst_buf, H5D_gather_func_t op,
                       void *op_data) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Dgather, herr_t,
      (src_space_id, src_buf, type_id, dst_buf_size, dst_buf, op, op_data));
  return result;
}

herr_t HDF5::H5Ddebug(hid_t dset_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Ddebug, herr_t, (dset_id));
  return result;
}

herr_t HDF5::H5Dformat_convert(hid_t dset_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Dformat_convert, herr_t, (dset_id));
  return result;
}

herr_t HDF5::H5Dget_chunk_index_type(hid_t did, H5D_chunk_index_t *idx_type) {
  BRAHMA_UNWRAPPED_FUNC(H5Dget_chunk_index_type, herr_t, (did, idx_type));
  return result;
}

hid_t HDF5::H5Dcreate1(hid_t file_id, const char *name, hid_t type_id,
                       hid_t space_id, hid_t dcpl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Dcreate1, hid_t,
                        (file_id, name, type_id, space_id, dcpl_id));
  return result;
}

hid_t HDF5::H5Dopen1(hid_t file_id, const char *name) {
  BRAHMA_UNWRAPPED_FUNC(H5Dopen1, hid_t, (file_id, name));
  return result;
}

herr_t HDF5::H5Dextend(hid_t dset_id, const hsize_t size[]) {
  BRAHMA_UNWRAPPED_FUNC(H5Dextend, herr_t, (dset_id, size));
  return result;
}

hid_t HDF5::H5Eregister_class(const char *cls_name, const char *lib_name,
                              const char *version) {
  BRAHMA_UNWRAPPED_FUNC(H5Eregister_class, hid_t,
                        (cls_name, lib_name, version));
  return result;
}

herr_t HDF5::H5Eunregister_class(hid_t class_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Eunregister_class, herr_t, (class_id));
  return result;
}

herr_t HDF5::H5Eclose_msg(hid_t err_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Eclose_msg, herr_t, (err_id));
  return result;
}

hid_t HDF5::H5Ecreate_msg(hid_t cls, H5E_type_t msg_type, const char *msg) {
  BRAHMA_UNWRAPPED_FUNC(H5Ecreate_msg, hid_t, (cls, msg_type, msg));
  return result;
}

hid_t HDF5::H5Ecreate_stack() {
  BRAHMA_UNWRAPPED_FUNC(H5Ecreate_stack, hid_t, ());
  return result;
}

hid_t HDF5::H5Eget_current_stack() {
  BRAHMA_UNWRAPPED_FUNC(H5Eget_current_stack, hid_t, ());
  return result;
}

herr_t HDF5::H5Eclose_stack(hid_t stack_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Eclose_stack, herr_t, (stack_id));
  return result;
}

ssize_t HDF5::H5Eget_class_name(hid_t class_id, char *name, size_t size) {
  BRAHMA_UNWRAPPED_FUNC(H5Eget_class_name, ssize_t, (class_id, name, size));
  return result;
}

herr_t HDF5::H5Eset_current_stack(hid_t err_stack_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Eset_current_stack, herr_t, (err_stack_id));
  return result;
}

herr_t HDF5::H5Epush2(hid_t err_stack, const char *file, const char *func,
                      unsigned int line, hid_t cls_id, hid_t maj_id,
                      hid_t min_id, const char *msg) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Epush2, herr_t,
      (err_stack, file, func, line, cls_id, maj_id, min_id, msg));
  return result;
}

herr_t HDF5::H5Epop(hid_t err_stack, size_t count) {
  BRAHMA_UNWRAPPED_FUNC(H5Epop, herr_t, (err_stack, count));
  return result;
}

herr_t HDF5::H5Eprint2(hid_t err_stack, FILE *stream) {
  BRAHMA_UNWRAPPED_FUNC(H5Eprint2, herr_t, (err_stack, stream));
  return result;
}

herr_t HDF5::H5Ewalk2(hid_t err_stack, H5E_direction_t direction,
                      H5E_walk2_t func, void *client_data) {
  BRAHMA_UNWRAPPED_FUNC(H5Ewalk2, herr_t,
                        (err_stack, direction, func, client_data));
  return result;
}

herr_t HDF5::H5Eget_auto2(hid_t estack_id, H5E_auto2_t *func,
                          void **client_data) {
  BRAHMA_UNWRAPPED_FUNC(H5Eget_auto2, herr_t, (estack_id, func, client_data));
  return result;
}

herr_t HDF5::H5Eset_auto2(hid_t estack_id, H5E_auto2_t func,
                          void *client_data) {
  BRAHMA_UNWRAPPED_FUNC(H5Eset_auto2, herr_t, (estack_id, func, client_data));
  return result;
}

herr_t HDF5::H5Eclear2(hid_t err_stack) {
  BRAHMA_UNWRAPPED_FUNC(H5Eclear2, herr_t, (err_stack));
  return result;
}

herr_t HDF5::H5Eauto_is_v2(hid_t err_stack, unsigned int *is_stack) {
  BRAHMA_UNWRAPPED_FUNC(H5Eauto_is_v2, herr_t, (err_stack, is_stack));
  return result;
}

ssize_t HDF5::H5Eget_msg(hid_t msg_id, H5E_type_t *type, char *msg,
                         size_t size) {
  BRAHMA_UNWRAPPED_FUNC(H5Eget_msg, ssize_t, (msg_id, type, msg, size));
  return result;
}

ssize_t HDF5::H5Eget_num(hid_t error_stack_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Eget_num, ssize_t, (error_stack_id));
  return result;
}

herr_t HDF5::H5Eclear1() {
  BRAHMA_UNWRAPPED_FUNC(H5Eclear1, herr_t, ());
  return result;
}

herr_t HDF5::H5Eget_auto1(H5E_auto1_t *func, void **client_data) {
  BRAHMA_UNWRAPPED_FUNC(H5Eget_auto1, herr_t, (func, client_data));
  return result;
}

herr_t HDF5::H5Epush1(const char *file, const char *func, unsigned int line,
                      H5E_major_t maj, H5E_minor_t min, const char *str) {
  BRAHMA_UNWRAPPED_FUNC(H5Epush1, herr_t, (file, func, line, maj, min, str));
  return result;
}

herr_t HDF5::H5Eprint1(FILE *stream) {
  BRAHMA_UNWRAPPED_FUNC(H5Eprint1, herr_t, (stream));
  return result;
}

herr_t HDF5::H5Eset_auto1(H5E_auto1_t func, void *client_data) {
  BRAHMA_UNWRAPPED_FUNC(H5Eset_auto1, herr_t, (func, client_data));
  return result;
}

herr_t HDF5::H5Ewalk1(H5E_direction_t direction, H5E_walk1_t func,
                      void *client_data) {
  BRAHMA_UNWRAPPED_FUNC(H5Ewalk1, herr_t, (direction, func, client_data));
  return result;
}

char *HDF5::H5Eget_major(H5E_major_t maj) {
  BRAHMA_UNWRAPPED_FUNC(H5Eget_major, char *, (maj));
  return result;
}

char *HDF5::H5Eget_minor(H5E_minor_t min) {
  BRAHMA_UNWRAPPED_FUNC(H5Eget_minor, char *, (min));
  return result;
}

htri_t HDF5::H5Fis_hdf5(const char *filename) {
  BRAHMA_UNWRAPPED_FUNC(H5Fis_hdf5, htri_t, (filename));
  return result;
}

hid_t HDF5::H5Fcreate(const char *filename, unsigned int flags,
                      hid_t create_plist, hid_t access_plist) {
  BRAHMA_UNWRAPPED_FUNC(H5Fcreate, hid_t,
                        (filename, flags, create_plist, access_plist));
  return result;
}

hid_t HDF5::H5Fopen(const char *filename, unsigned int flags,
                    hid_t access_plist) {
  BRAHMA_UNWRAPPED_FUNC(H5Fopen, hid_t, (filename, flags, access_plist));
  return result;
}

hid_t HDF5::H5Freopen(hid_t file_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Freopen, hid_t, (file_id));
  return result;
}

herr_t HDF5::H5Fflush(hid_t object_id, H5F_scope_t scope) {
  BRAHMA_UNWRAPPED_FUNC(H5Fflush, herr_t, (object_id, scope));
  return result;
}

herr_t HDF5::H5Fclose(hid_t file_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Fclose, herr_t, (file_id));
  return result;
}

hid_t HDF5::H5Fget_create_plist(hid_t file_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Fget_create_plist, hid_t, (file_id));
  return result;
}

hid_t HDF5::H5Fget_access_plist(hid_t file_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Fget_access_plist, hid_t, (file_id));
  return result;
}

herr_t HDF5::H5Fget_intent(hid_t file_id, unsigned int *intent) {
  BRAHMA_UNWRAPPED_FUNC(H5Fget_intent, herr_t, (file_id, intent));
  return result;
}

ssize_t HDF5::H5Fget_obj_count(hid_t file_id, unsigned int types) {
  BRAHMA_UNWRAPPED_FUNC(H5Fget_obj_count, ssize_t, (file_id, types));
  return result;
}

ssize_t HDF5::H5Fget_obj_ids(hid_t file_id, unsigned int types, size_t max_objs,
                             hid_t *obj_id_list) {
  BRAHMA_UNWRAPPED_FUNC(H5Fget_obj_ids, ssize_t,
                        (file_id, types, max_objs, obj_id_list));
  return result;
}

herr_t HDF5::H5Fget_vfd_handle(hid_t file_id, hid_t fapl, void **file_handle) {
  BRAHMA_UNWRAPPED_FUNC(H5Fget_vfd_handle, herr_t,
                        (file_id, fapl, file_handle));
  return result;
}

herr_t HDF5::H5Fmount(hid_t loc, const char *name, hid_t child, hid_t plist) {
  BRAHMA_UNWRAPPED_FUNC(H5Fmount, herr_t, (loc, name, child, plist));
  return result;
}

herr_t HDF5::H5Funmount(hid_t loc, const char *name) {
  BRAHMA_UNWRAPPED_FUNC(H5Funmount, herr_t, (loc, name));
  return result;
}

hssize_t HDF5::H5Fget_freespace(hid_t file_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Fget_freespace, hssize_t, (file_id));
  return result;
}

herr_t HDF5::H5Fget_filesize(hid_t file_id, hsize_t *size) {
  BRAHMA_UNWRAPPED_FUNC(H5Fget_filesize, herr_t, (file_id, size));
  return result;
}

herr_t HDF5::H5Fget_eoa(hid_t file_id, haddr_t *eoa) {
  BRAHMA_UNWRAPPED_FUNC(H5Fget_eoa, herr_t, (file_id, eoa));
  return result;
}

herr_t HDF5::H5Fincrement_filesize(hid_t file_id, hsize_t increment) {
  BRAHMA_UNWRAPPED_FUNC(H5Fincrement_filesize, herr_t, (file_id, increment));
  return result;
}

ssize_t HDF5::H5Fget_file_image(hid_t file_id, void *buf_ptr, size_t buf_len) {
  BRAHMA_UNWRAPPED_FUNC(H5Fget_file_image, ssize_t,
                        (file_id, buf_ptr, buf_len));
  return result;
}

herr_t HDF5::H5Fget_mdc_config(hid_t file_id, H5AC_cache_config_t *config_ptr) {
  BRAHMA_UNWRAPPED_FUNC(H5Fget_mdc_config, herr_t, (file_id, config_ptr));
  return result;
}

herr_t HDF5::H5Fset_mdc_config(hid_t file_id, H5AC_cache_config_t *config_ptr) {
  BRAHMA_UNWRAPPED_FUNC(H5Fset_mdc_config, herr_t, (file_id, config_ptr));
  return result;
}

herr_t HDF5::H5Fget_mdc_hit_rate(hid_t file_id, double *hit_rate_ptr) {
  BRAHMA_UNWRAPPED_FUNC(H5Fget_mdc_hit_rate, herr_t, (file_id, hit_rate_ptr));
  return result;
}

herr_t HDF5::H5Fget_mdc_size(hid_t file_id, size_t *max_size_ptr,
                             size_t *min_clean_size_ptr, size_t *cur_size_ptr,
                             int *cur_num_entries_ptr) {
  BRAHMA_UNWRAPPED_FUNC(H5Fget_mdc_size, herr_t,
                        (file_id, max_size_ptr, min_clean_size_ptr,
                         cur_size_ptr, cur_num_entries_ptr));
  return result;
}

herr_t HDF5::H5Freset_mdc_hit_rate_stats(hid_t file_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Freset_mdc_hit_rate_stats, herr_t, (file_id));
  return result;
}

ssize_t HDF5::H5Fget_name(hid_t obj_id, char *name, size_t size) {
  BRAHMA_UNWRAPPED_FUNC(H5Fget_name, ssize_t, (obj_id, name, size));
  return result;
}

herr_t HDF5::H5Fget_info2(hid_t obj_id, H5F_info2_t *finfo) {
  BRAHMA_UNWRAPPED_FUNC(H5Fget_info2, herr_t, (obj_id, finfo));
  return result;
}

herr_t HDF5::H5Fget_metadata_read_retry_info(hid_t file_id,
                                             H5F_retry_info_t *info) {
  BRAHMA_UNWRAPPED_FUNC(H5Fget_metadata_read_retry_info, herr_t,
                        (file_id, info));
  return result;
}

herr_t HDF5::H5Fstart_swmr_write(hid_t file_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Fstart_swmr_write, herr_t, (file_id));
  return result;
}

ssize_t HDF5::H5Fget_free_sections(hid_t file_id, H5F_mem_t type, size_t nsects,
                                   H5F_sect_info_t *sect_info) {
  BRAHMA_UNWRAPPED_FUNC(H5Fget_free_sections, ssize_t,
                        (file_id, type, nsects, sect_info));
  return result;
}

herr_t HDF5::H5Fclear_elink_file_cache(hid_t file_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Fclear_elink_file_cache, herr_t, (file_id));
  return result;
}

herr_t HDF5::H5Fset_libver_bounds(hid_t file_id, H5F_libver_t low,
                                  H5F_libver_t high) {
  BRAHMA_UNWRAPPED_FUNC(H5Fset_libver_bounds, herr_t, (file_id, low, high));
  return result;
}

herr_t HDF5::H5Fstart_mdc_logging(hid_t file_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Fstart_mdc_logging, herr_t, (file_id));
  return result;
}

herr_t HDF5::H5Fstop_mdc_logging(hid_t file_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Fstop_mdc_logging, herr_t, (file_id));
  return result;
}

herr_t HDF5::H5Fget_mdc_logging_status(hid_t file_id, hbool_t *is_enabled,
                                       hbool_t *is_currently_logging) {
  BRAHMA_UNWRAPPED_FUNC(H5Fget_mdc_logging_status, herr_t,
                        (file_id, is_enabled, is_currently_logging));
  return result;
}

herr_t HDF5::H5Fformat_convert(hid_t fid) {
  BRAHMA_UNWRAPPED_FUNC(H5Fformat_convert, herr_t, (fid));
  return result;
}

herr_t HDF5::H5Freset_page_buffering_stats(hid_t file_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Freset_page_buffering_stats, herr_t, (file_id));
  return result;
}

herr_t HDF5::H5Fget_page_buffering_stats(hid_t file_id,
                                         unsigned int accesses[2],
                                         unsigned int hits[2],
                                         unsigned int misses[2],
                                         unsigned int evictions[2],
                                         unsigned int bypasses[2]) {
  BRAHMA_UNWRAPPED_FUNC(H5Fget_page_buffering_stats, herr_t,
                        (file_id, accesses, hits, misses, evictions, bypasses));
  return result;
}

herr_t HDF5::H5Fget_mdc_image_info(hid_t file_id, haddr_t *image_addr,
                                   hsize_t *image_size) {
  BRAHMA_UNWRAPPED_FUNC(H5Fget_mdc_image_info, herr_t,
                        (file_id, image_addr, image_size));
  return result;
}

herr_t HDF5::H5Fget_dset_no_attrs_hint(hid_t file_id, hbool_t *minimize) {
  BRAHMA_UNWRAPPED_FUNC(H5Fget_dset_no_attrs_hint, herr_t, (file_id, minimize));
  return result;
}

herr_t HDF5::H5Fset_dset_no_attrs_hint(hid_t file_id, hbool_t minimize) {
  BRAHMA_UNWRAPPED_FUNC(H5Fset_dset_no_attrs_hint, herr_t, (file_id, minimize));
  return result;
}

herr_t HDF5::H5Fget_info1(hid_t obj_id, H5F_info1_t *finfo) {
  BRAHMA_UNWRAPPED_FUNC(H5Fget_info1, herr_t, (obj_id, finfo));
  return result;
}

herr_t HDF5::H5Fset_latest_format(hid_t file_id, hbool_t latest_format) {
  BRAHMA_UNWRAPPED_FUNC(H5Fset_latest_format, herr_t, (file_id, latest_format));
  return result;
}

hid_t HDF5::H5FDregister(const H5FD_class_t *cls) {
  BRAHMA_UNWRAPPED_FUNC(H5FDregister, hid_t, (cls));
  return result;
}

herr_t HDF5::H5FDunregister(hid_t driver_id) {
  BRAHMA_UNWRAPPED_FUNC(H5FDunregister, herr_t, (driver_id));
  return result;
}

H5FD_t *HDF5::H5FDopen(const char *name, unsigned int flags, hid_t fapl_id,
                       haddr_t maxaddr) {
  BRAHMA_UNWRAPPED_FUNC(H5FDopen, H5FD_t *, (name, flags, fapl_id, maxaddr));
  return result;
}

herr_t HDF5::H5FDclose(H5FD_t *file) {
  BRAHMA_UNWRAPPED_FUNC(H5FDclose, herr_t, (file));
  return result;
}

int HDF5::H5FDcmp(const H5FD_t *f1, const H5FD_t *f2) {
  BRAHMA_UNWRAPPED_FUNC(H5FDcmp, int, (f1, f2));
  return result;
}

int HDF5::H5FDquery(const H5FD_t *f, unsigned long *flags) {
  BRAHMA_UNWRAPPED_FUNC(H5FDquery, int, (f, flags));
  return result;
}

haddr_t HDF5::H5FDalloc(H5FD_t *file, H5FD_mem_t type, hid_t dxpl_id,
                        hsize_t size) {
  BRAHMA_UNWRAPPED_FUNC(H5FDalloc, haddr_t, (file, type, dxpl_id, size));
  return result;
}

herr_t HDF5::H5FDfree(H5FD_t *file, H5FD_mem_t type, hid_t dxpl_id,
                      haddr_t addr, hsize_t size) {
  BRAHMA_UNWRAPPED_FUNC(H5FDfree, herr_t, (file, type, dxpl_id, addr, size));
  return result;
}

haddr_t HDF5::H5FDget_eoa(H5FD_t *file, H5FD_mem_t type) {
  BRAHMA_UNWRAPPED_FUNC(H5FDget_eoa, haddr_t, (file, type));
  return result;
}

herr_t HDF5::H5FDset_eoa(H5FD_t *file, H5FD_mem_t type, haddr_t eoa) {
  BRAHMA_UNWRAPPED_FUNC(H5FDset_eoa, herr_t, (file, type, eoa));
  return result;
}

haddr_t HDF5::H5FDget_eof(H5FD_t *file, H5FD_mem_t type) {
  BRAHMA_UNWRAPPED_FUNC(H5FDget_eof, haddr_t, (file, type));
  return result;
}

herr_t HDF5::H5FDget_vfd_handle(H5FD_t *file, hid_t fapl, void **file_handle) {
  BRAHMA_UNWRAPPED_FUNC(H5FDget_vfd_handle, herr_t, (file, fapl, file_handle));
  return result;
}

herr_t HDF5::H5FDread(H5FD_t *file, H5FD_mem_t type, hid_t dxpl_id,
                      haddr_t addr, size_t size, void *buf) {
  BRAHMA_UNWRAPPED_FUNC(H5FDread, herr_t,
                        (file, type, dxpl_id, addr, size, buf));
  return result;
}

herr_t HDF5::H5FDwrite(H5FD_t *file, H5FD_mem_t type, hid_t dxpl_id,
                       haddr_t addr, size_t size, const void *buf) {
  BRAHMA_UNWRAPPED_FUNC(H5FDwrite, herr_t,
                        (file, type, dxpl_id, addr, size, buf));
  return result;
}

herr_t HDF5::H5FDflush(H5FD_t *file, hid_t dxpl_id, hbool_t closing) {
  BRAHMA_UNWRAPPED_FUNC(H5FDflush, herr_t, (file, dxpl_id, closing));
  return result;
}

herr_t HDF5::H5FDtruncate(H5FD_t *file, hid_t dxpl_id, hbool_t closing) {
  BRAHMA_UNWRAPPED_FUNC(H5FDtruncate, herr_t, (file, dxpl_id, closing));
  return result;
}

herr_t HDF5::H5FDlock(H5FD_t *file, hbool_t rw) {
  BRAHMA_UNWRAPPED_FUNC(H5FDlock, herr_t, (file, rw));
  return result;
}

herr_t HDF5::H5FDunlock(H5FD_t *file) {
  BRAHMA_UNWRAPPED_FUNC(H5FDunlock, herr_t, (file));
  return result;
}

herr_t HDF5::H5FDdriver_query(hid_t driver_id, unsigned long *flags) {
  BRAHMA_UNWRAPPED_FUNC(H5FDdriver_query, herr_t, (driver_id, flags));
  return result;
}

hid_t HDF5::H5Gcreate2(hid_t loc_id, const char *name, hid_t lcpl_id,
                       hid_t gcpl_id, hid_t gapl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Gcreate2, hid_t,
                        (loc_id, name, lcpl_id, gcpl_id, gapl_id));
  return result;
}

hid_t HDF5::H5Gcreate_anon(hid_t loc_id, hid_t gcpl_id, hid_t gapl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Gcreate_anon, hid_t, (loc_id, gcpl_id, gapl_id));
  return result;
}

hid_t HDF5::H5Gopen2(hid_t loc_id, const char *name, hid_t gapl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Gopen2, hid_t, (loc_id, name, gapl_id));
  return result;
}

hid_t HDF5::H5Gget_create_plist(hid_t group_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Gget_create_plist, hid_t, (group_id));
  return result;
}

herr_t HDF5::H5Gget_info(hid_t loc_id, H5G_info_t *ginfo) {
  BRAHMA_UNWRAPPED_FUNC(H5Gget_info, herr_t, (loc_id, ginfo));
  return result;
}

herr_t HDF5::H5Gget_info_by_name(hid_t loc_id, const char *name,
                                 H5G_info_t *ginfo, hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Gget_info_by_name, herr_t,
                        (loc_id, name, ginfo, lapl_id));
  return result;
}

herr_t HDF5::H5Gget_info_by_idx(hid_t loc_id, const char *group_name,
                                H5_index_t idx_type, H5_iter_order_t order,
                                hsize_t n, H5G_info_t *ginfo, hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Gget_info_by_idx, herr_t,
      (loc_id, group_name, idx_type, order, n, ginfo, lapl_id));
  return result;
}

herr_t HDF5::H5Gclose(hid_t group_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Gclose, herr_t, (group_id));
  return result;
}

herr_t HDF5::H5Gflush(hid_t group_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Gflush, herr_t, (group_id));
  return result;
}

herr_t HDF5::H5Grefresh(hid_t group_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Grefresh, herr_t, (group_id));
  return result;
}

hid_t HDF5::H5Gcreate1(hid_t loc_id, const char *name, size_t size_hint) {
  BRAHMA_UNWRAPPED_FUNC(H5Gcreate1, hid_t, (loc_id, name, size_hint));
  return result;
}

hid_t HDF5::H5Gopen1(hid_t loc_id, const char *name) {
  BRAHMA_UNWRAPPED_FUNC(H5Gopen1, hid_t, (loc_id, name));
  return result;
}

herr_t HDF5::H5Glink(hid_t cur_loc_id, H5L_type_t type, const char *cur_name,
                     const char *new_name) {
  BRAHMA_UNWRAPPED_FUNC(H5Glink, herr_t,
                        (cur_loc_id, type, cur_name, new_name));
  return result;
}

herr_t HDF5::H5Glink2(hid_t cur_loc_id, const char *cur_name, H5L_type_t type,
                      hid_t new_loc_id, const char *new_name) {
  BRAHMA_UNWRAPPED_FUNC(H5Glink2, herr_t,
                        (cur_loc_id, cur_name, type, new_loc_id, new_name));
  return result;
}

herr_t HDF5::H5Gmove(hid_t src_loc_id, const char *src_name,
                     const char *dst_name) {
  BRAHMA_UNWRAPPED_FUNC(H5Gmove, herr_t, (src_loc_id, src_name, dst_name));
  return result;
}

herr_t HDF5::H5Gmove2(hid_t src_loc_id, const char *src_name, hid_t dst_loc_id,
                      const char *dst_name) {
  BRAHMA_UNWRAPPED_FUNC(H5Gmove2, herr_t,
                        (src_loc_id, src_name, dst_loc_id, dst_name));
  return result;
}

herr_t HDF5::H5Gunlink(hid_t loc_id, const char *name) {
  BRAHMA_UNWRAPPED_FUNC(H5Gunlink, herr_t, (loc_id, name));
  return result;
}

herr_t HDF5::H5Gget_linkval(hid_t loc_id, const char *name, size_t size,
                            char *buf) {
  BRAHMA_UNWRAPPED_FUNC(H5Gget_linkval, herr_t, (loc_id, name, size, buf));
  return result;
}

herr_t HDF5::H5Gset_comment(hid_t loc_id, const char *name,
                            const char *comment) {
  BRAHMA_UNWRAPPED_FUNC(H5Gset_comment, herr_t, (loc_id, name, comment));
  return result;
}

int HDF5::H5Gget_comment(hid_t loc_id, const char *name, size_t bufsize,
                         char *buf) {
  BRAHMA_UNWRAPPED_FUNC(H5Gget_comment, int, (loc_id, name, bufsize, buf));
  return result;
}

herr_t HDF5::H5Giterate(hid_t loc_id, const char *name, int *idx,
                        H5G_iterate_t op, void *op_data) {
  BRAHMA_UNWRAPPED_FUNC(H5Giterate, herr_t, (loc_id, name, idx, op, op_data));
  return result;
}

herr_t HDF5::H5Gget_num_objs(hid_t loc_id, hsize_t *num_objs) {
  BRAHMA_UNWRAPPED_FUNC(H5Gget_num_objs, herr_t, (loc_id, num_objs));
  return result;
}

herr_t HDF5::H5Gget_objinfo(hid_t loc_id, const char *name, hbool_t follow_link,
                            H5G_stat_t *statbuf) {
  BRAHMA_UNWRAPPED_FUNC(H5Gget_objinfo, herr_t,
                        (loc_id, name, follow_link, statbuf));
  return result;
}

ssize_t HDF5::H5Gget_objname_by_idx(hid_t loc_id, hsize_t idx, char *name,
                                    size_t size) {
  BRAHMA_UNWRAPPED_FUNC(H5Gget_objname_by_idx, ssize_t,
                        (loc_id, idx, name, size));
  return result;
}

H5G_obj_t HDF5::H5Gget_objtype_by_idx(hid_t loc_id, hsize_t idx) {
  BRAHMA_UNWRAPPED_FUNC(H5Gget_objtype_by_idx, H5G_obj_t, (loc_id, idx));
  return result;
}

herr_t HDF5::H5Zregister(const void *cls) {
  BRAHMA_UNWRAPPED_FUNC(H5Zregister, herr_t, (cls));
  return result;
}

herr_t HDF5::H5Zunregister(H5Z_filter_t id) {
  BRAHMA_UNWRAPPED_FUNC(H5Zunregister, herr_t, (id));
  return result;
}

htri_t HDF5::H5Zfilter_avail(H5Z_filter_t id) {
  BRAHMA_UNWRAPPED_FUNC(H5Zfilter_avail, htri_t, (id));
  return result;
}

herr_t HDF5::H5Zget_filter_info(H5Z_filter_t filter,
                                unsigned int *filter_config_flags) {
  BRAHMA_UNWRAPPED_FUNC(H5Zget_filter_info, herr_t,
                        (filter, filter_config_flags));
  return result;
}

hid_t HDF5::H5Pcreate_class(hid_t parent, const char *name,
                            H5P_cls_create_func_t cls_create, void *create_data,
                            H5P_cls_copy_func_t cls_copy, void *copy_data,
                            H5P_cls_close_func_t cls_close, void *close_data) {
  BRAHMA_UNWRAPPED_FUNC(H5Pcreate_class, hid_t,
                        (parent, name, cls_create, create_data, cls_copy,
                         copy_data, cls_close, close_data));
  return result;
}

char *HDF5::H5Pget_class_name(hid_t pclass_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_class_name, char *, (pclass_id));
  return result;
}

hid_t HDF5::H5Pcreate(hid_t cls_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Pcreate, hid_t, (cls_id));
  return result;
}

herr_t HDF5::H5Pregister2(hid_t cls_id, const char *name, size_t size,
                          void *def_value, H5P_prp_create_func_t prp_create,
                          H5P_prp_set_func_t prp_set,
                          H5P_prp_get_func_t prp_get,
                          H5P_prp_delete_func_t prp_del,
                          H5P_prp_copy_func_t prp_copy,
                          H5P_prp_compare_func_t prp_cmp,
                          H5P_prp_close_func_t prp_close) {
  BRAHMA_UNWRAPPED_FUNC(H5Pregister2, herr_t,
                        (cls_id, name, size, def_value, prp_create, prp_set,
                         prp_get, prp_del, prp_copy, prp_cmp, prp_close));
  return result;
}

herr_t HDF5::H5Pinsert2(hid_t plist_id, const char *name, size_t size,
                        void *value, H5P_prp_set_func_t prp_set,
                        H5P_prp_get_func_t prp_get,
                        H5P_prp_delete_func_t prp_delete,
                        H5P_prp_copy_func_t prp_copy,
                        H5P_prp_compare_func_t prp_cmp,
                        H5P_prp_close_func_t prp_close) {
  BRAHMA_UNWRAPPED_FUNC(H5Pinsert2, herr_t,
                        (plist_id, name, size, value, prp_set, prp_get,
                         prp_delete, prp_copy, prp_cmp, prp_close));
  return result;
}

herr_t HDF5::H5Pset(hid_t plist_id, const char *name, const void *value) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset, herr_t, (plist_id, name, value));
  return result;
}

htri_t HDF5::H5Pexist(hid_t plist_id, const char *name) {
  BRAHMA_UNWRAPPED_FUNC(H5Pexist, htri_t, (plist_id, name));
  return result;
}

herr_t HDF5::H5Pencode(hid_t plist_id, void *buf, size_t *nalloc) {
  BRAHMA_UNWRAPPED_FUNC(H5Pencode, herr_t, (plist_id, buf, nalloc));
  return result;
}

hid_t HDF5::H5Pdecode(const void *buf) {
  BRAHMA_UNWRAPPED_FUNC(H5Pdecode, hid_t, (buf));
  return result;
}

herr_t HDF5::H5Pget_size(hid_t id, const char *name, size_t *size) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_size, herr_t, (id, name, size));
  return result;
}

herr_t HDF5::H5Pget_nprops(hid_t id, size_t *nprops) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_nprops, herr_t, (id, nprops));
  return result;
}

hid_t HDF5::H5Pget_class(hid_t plist_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_class, hid_t, (plist_id));
  return result;
}

hid_t HDF5::H5Pget_class_parent(hid_t pclass_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_class_parent, hid_t, (pclass_id));
  return result;
}

herr_t HDF5::H5Pget(hid_t plist_id, const char *name, void *value) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget, herr_t, (plist_id, name, value));
  return result;
}

htri_t HDF5::H5Pequal(hid_t id1, hid_t id2) {
  BRAHMA_UNWRAPPED_FUNC(H5Pequal, htri_t, (id1, id2));
  return result;
}

htri_t HDF5::H5Pisa_class(hid_t plist_id, hid_t pclass_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Pisa_class, htri_t, (plist_id, pclass_id));
  return result;
}

int HDF5::H5Piterate(hid_t id, int *idx, H5P_iterate_t iter_func,
                     void *iter_data) {
  BRAHMA_UNWRAPPED_FUNC(H5Piterate, int, (id, idx, iter_func, iter_data));
  return result;
}

herr_t HDF5::H5Pcopy_prop(hid_t dst_id, hid_t src_id, const char *name) {
  BRAHMA_UNWRAPPED_FUNC(H5Pcopy_prop, herr_t, (dst_id, src_id, name));
  return result;
}

herr_t HDF5::H5Premove(hid_t plist_id, const char *name) {
  BRAHMA_UNWRAPPED_FUNC(H5Premove, herr_t, (plist_id, name));
  return result;
}

herr_t HDF5::H5Punregister(hid_t pclass_id, const char *name) {
  BRAHMA_UNWRAPPED_FUNC(H5Punregister, herr_t, (pclass_id, name));
  return result;
}

herr_t HDF5::H5Pclose_class(hid_t plist_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Pclose_class, herr_t, (plist_id));
  return result;
}

herr_t HDF5::H5Pclose(hid_t plist_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Pclose, herr_t, (plist_id));
  return result;
}

hid_t HDF5::H5Pcopy(hid_t plist_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Pcopy, hid_t, (plist_id));
  return result;
}

herr_t HDF5::H5Pset_attr_phase_change(hid_t plist_id, unsigned int max_compact,
                                      unsigned int min_dense) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_attr_phase_change, herr_t,
                        (plist_id, max_compact, min_dense));
  return result;
}

herr_t HDF5::H5Pget_attr_phase_change(hid_t plist_id, unsigned int *max_compact,
                                      unsigned int *min_dense) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_attr_phase_change, herr_t,
                        (plist_id, max_compact, min_dense));
  return result;
}

herr_t HDF5::H5Pset_attr_creation_order(hid_t plist_id,
                                        unsigned int crt_order_flags) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_attr_creation_order, herr_t,
                        (plist_id, crt_order_flags));
  return result;
}

herr_t HDF5::H5Pget_attr_creation_order(hid_t plist_id,
                                        unsigned int *crt_order_flags) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_attr_creation_order, herr_t,
                        (plist_id, crt_order_flags));
  return result;
}

herr_t HDF5::H5Pset_obj_track_times(hid_t plist_id, hbool_t track_times) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_obj_track_times, herr_t,
                        (plist_id, track_times));
  return result;
}

herr_t HDF5::H5Pget_obj_track_times(hid_t plist_id, hbool_t *track_times) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_obj_track_times, herr_t,
                        (plist_id, track_times));
  return result;
}

herr_t HDF5::H5Pmodify_filter(hid_t plist_id, H5Z_filter_t filter,
                              unsigned int flags, size_t cd_nelmts,
                              const unsigned int cd_values[]) {
  BRAHMA_UNWRAPPED_FUNC(H5Pmodify_filter, herr_t,
                        (plist_id, filter, flags, cd_nelmts, cd_values));
  return result;
}

herr_t HDF5::H5Pset_filter(hid_t plist_id, H5Z_filter_t filter,
                           unsigned int flags, size_t cd_nelmts,
                           const unsigned int c_values[]) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_filter, herr_t,
                        (plist_id, filter, flags, cd_nelmts, c_values));
  return result;
}

int HDF5::H5Pget_nfilters(hid_t plist_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_nfilters, int, (plist_id));
  return result;
}

H5Z_filter_t HDF5::H5Pget_filter2(hid_t plist_id, unsigned int filter,
                                  unsigned int *flags, size_t *cd_nelmts,
                                  unsigned int cd_values[], size_t namelen,
                                  char name[], unsigned int *filter_config) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_filter2, H5Z_filter_t,
                        (plist_id, filter, flags, cd_nelmts, cd_values, namelen,
                         name, filter_config));
  return result;
}

herr_t HDF5::H5Pget_filter_by_id2(hid_t plist_id, H5Z_filter_t id,
                                  unsigned int *flags, size_t *cd_nelmts,
                                  unsigned int cd_values[], size_t namelen,
                                  char name[], unsigned int *filter_config) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_filter_by_id2, herr_t,
                        (plist_id, id, flags, cd_nelmts, cd_values, namelen,
                         name, filter_config));
  return result;
}

htri_t HDF5::H5Pall_filters_avail(hid_t plist_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Pall_filters_avail, htri_t, (plist_id));
  return result;
}

herr_t HDF5::H5Premove_filter(hid_t plist_id, H5Z_filter_t filter) {
  BRAHMA_UNWRAPPED_FUNC(H5Premove_filter, herr_t, (plist_id, filter));
  return result;
}

herr_t HDF5::H5Pset_deflate(hid_t plist_id, unsigned int aggression) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_deflate, herr_t, (plist_id, aggression));
  return result;
}

herr_t HDF5::H5Pset_fletcher32(hid_t plist_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_fletcher32, herr_t, (plist_id));
  return result;
}

herr_t HDF5::H5Pset_userblock(hid_t plist_id, hsize_t size) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_userblock, herr_t, (plist_id, size));
  return result;
}

herr_t HDF5::H5Pget_userblock(hid_t plist_id, hsize_t *size) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_userblock, herr_t, (plist_id, size));
  return result;
}

herr_t HDF5::H5Pset_sizes(hid_t plist_id, size_t sizeof_addr,
                          size_t sizeof_size) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_sizes, herr_t,
                        (plist_id, sizeof_addr, sizeof_size));
  return result;
}

herr_t HDF5::H5Pget_sizes(hid_t plist_id, size_t *sizeof_addr,
                          size_t *sizeof_size) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_sizes, herr_t,
                        (plist_id, sizeof_addr, sizeof_size));
  return result;
}

herr_t HDF5::H5Pset_sym_k(hid_t plist_id, unsigned int ik, unsigned int lk) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_sym_k, herr_t, (plist_id, ik, lk));
  return result;
}

herr_t HDF5::H5Pget_sym_k(hid_t plist_id, unsigned int *ik, unsigned int *lk) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_sym_k, herr_t, (plist_id, ik, lk));
  return result;
}

herr_t HDF5::H5Pset_istore_k(hid_t plist_id, unsigned int ik) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_istore_k, herr_t, (plist_id, ik));
  return result;
}

herr_t HDF5::H5Pget_istore_k(hid_t plist_id, unsigned int *ik) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_istore_k, herr_t, (plist_id, ik));
  return result;
}

herr_t HDF5::H5Pset_shared_mesg_nindexes(hid_t plist_id,
                                         unsigned int nindexes) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_shared_mesg_nindexes, herr_t,
                        (plist_id, nindexes));
  return result;
}

herr_t HDF5::H5Pget_shared_mesg_nindexes(hid_t plist_id,
                                         unsigned int *nindexes) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_shared_mesg_nindexes, herr_t,
                        (plist_id, nindexes));
  return result;
}

herr_t HDF5::H5Pset_shared_mesg_index(hid_t plist_id, unsigned int index_num,
                                      unsigned int mesg_type_flags,
                                      unsigned int min_mesg_size) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_shared_mesg_index, herr_t,
                        (plist_id, index_num, mesg_type_flags, min_mesg_size));
  return result;
}

herr_t HDF5::H5Pget_shared_mesg_index(hid_t plist_id, unsigned int index_num,
                                      unsigned int *mesg_type_flags,
                                      unsigned int *min_mesg_size) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_shared_mesg_index, herr_t,
                        (plist_id, index_num, mesg_type_flags, min_mesg_size));
  return result;
}

herr_t HDF5::H5Pset_shared_mesg_phase_change(hid_t plist_id,
                                             unsigned int max_list,
                                             unsigned int min_btree) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_shared_mesg_phase_change, herr_t,
                        (plist_id, max_list, min_btree));
  return result;
}

herr_t HDF5::H5Pget_shared_mesg_phase_change(hid_t plist_id,
                                             unsigned int *max_list,
                                             unsigned int *min_btree) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_shared_mesg_phase_change, herr_t,
                        (plist_id, max_list, min_btree));
  return result;
}

herr_t HDF5::H5Pset_file_space_strategy(hid_t plist_id,
                                        H5F_fspace_strategy_t strategy,
                                        hbool_t persist, hsize_t threshold) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_file_space_strategy, herr_t,
                        (plist_id, strategy, persist, threshold));
  return result;
}

herr_t HDF5::H5Pget_file_space_strategy(hid_t plist_id,
                                        H5F_fspace_strategy_t *strategy,
                                        hbool_t *persist, hsize_t *threshold) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_file_space_strategy, herr_t,
                        (plist_id, strategy, persist, threshold));
  return result;
}

herr_t HDF5::H5Pset_file_space_page_size(hid_t plist_id, hsize_t fsp_size) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_file_space_page_size, herr_t,
                        (plist_id, fsp_size));
  return result;
}

herr_t HDF5::H5Pget_file_space_page_size(hid_t plist_id, hsize_t *fsp_size) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_file_space_page_size, herr_t,
                        (plist_id, fsp_size));
  return result;
}

herr_t HDF5::H5Pset_alignment(hid_t fapl_id, hsize_t threshold,
                              hsize_t alignment) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_alignment, herr_t,
                        (fapl_id, threshold, alignment));
  return result;
}

herr_t HDF5::H5Pget_alignment(hid_t fapl_id, hsize_t *threshold,
                              hsize_t *alignment) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_alignment, herr_t,
                        (fapl_id, threshold, alignment));
  return result;
}

herr_t HDF5::H5Pset_driver(hid_t plist_id, hid_t driver_id,
                           const void *driver_info) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_driver, herr_t,
                        (plist_id, driver_id, driver_info));
  return result;
}

hid_t HDF5::H5Pget_driver(hid_t plist_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_driver, hid_t, (plist_id));
  return result;
}

const void *HDF5::H5Pget_driver_info(hid_t plist_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_driver_info, const void *, (plist_id));
  return result;
}

herr_t HDF5::H5Pset_family_offset(hid_t fapl_id, hsize_t offset) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_family_offset, herr_t, (fapl_id, offset));
  return result;
}

herr_t HDF5::H5Pget_family_offset(hid_t fapl_id, hsize_t *offset) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_family_offset, herr_t, (fapl_id, offset));
  return result;
}

herr_t HDF5::H5Pset_multi_type(hid_t fapl_id, H5FD_mem_t type) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_multi_type, herr_t, (fapl_id, type));
  return result;
}

herr_t HDF5::H5Pget_multi_type(hid_t fapl_id, H5FD_mem_t *type) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_multi_type, herr_t, (fapl_id, type));
  return result;
}

herr_t HDF5::H5Pset_cache(hid_t plist_id, int mdc_nelmts, size_t rdcc_nslots,
                          size_t rdcc_nbytes, double rdcc_w0) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Pset_cache, herr_t,
      (plist_id, mdc_nelmts, rdcc_nslots, rdcc_nbytes, rdcc_w0));
  return result;
}

herr_t HDF5::H5Pget_cache(hid_t plist_id, int *mdc_nelmts, size_t *rdcc_nslots,
                          size_t *rdcc_nbytes, double *rdcc_w0) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Pget_cache, herr_t,
      (plist_id, mdc_nelmts, rdcc_nslots, rdcc_nbytes, rdcc_w0));
  return result;
}

herr_t HDF5::H5Pset_mdc_config(hid_t plist_id,
                               H5AC_cache_config_t *config_ptr) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_mdc_config, herr_t, (plist_id, config_ptr));
  return result;
}

herr_t HDF5::H5Pget_mdc_config(hid_t plist_id,
                               H5AC_cache_config_t *config_ptr) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_mdc_config, herr_t, (plist_id, config_ptr));
  return result;
}

herr_t HDF5::H5Pset_gc_references(hid_t fapl_id, unsigned int gc_ref) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_gc_references, herr_t, (fapl_id, gc_ref));
  return result;
}

herr_t HDF5::H5Pget_gc_references(hid_t fapl_id, unsigned int *gc_ref) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_gc_references, herr_t, (fapl_id, gc_ref));
  return result;
}

herr_t HDF5::H5Pset_fclose_degree(hid_t fapl_id, H5F_close_degree_t degree) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_fclose_degree, herr_t, (fapl_id, degree));
  return result;
}

herr_t HDF5::H5Pget_fclose_degree(hid_t fapl_id, H5F_close_degree_t *degree) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_fclose_degree, herr_t, (fapl_id, degree));
  return result;
}

herr_t HDF5::H5Pset_meta_block_size(hid_t fapl_id, hsize_t size) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_meta_block_size, herr_t, (fapl_id, size));
  return result;
}

herr_t HDF5::H5Pget_meta_block_size(hid_t fapl_id, hsize_t *size) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_meta_block_size, herr_t, (fapl_id, size));
  return result;
}

herr_t HDF5::H5Pset_sieve_buf_size(hid_t fapl_id, size_t size) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_sieve_buf_size, herr_t, (fapl_id, size));
  return result;
}

herr_t HDF5::H5Pget_sieve_buf_size(hid_t fapl_id, size_t *size) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_sieve_buf_size, herr_t, (fapl_id, size));
  return result;
}

herr_t HDF5::H5Pset_small_data_block_size(hid_t fapl_id, hsize_t size) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_small_data_block_size, herr_t, (fapl_id, size));
  return result;
}

herr_t HDF5::H5Pget_small_data_block_size(hid_t fapl_id, hsize_t *size) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_small_data_block_size, herr_t, (fapl_id, size));
  return result;
}

herr_t HDF5::H5Pset_libver_bounds(hid_t plist_id, H5F_libver_t low,
                                  H5F_libver_t high) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_libver_bounds, herr_t, (plist_id, low, high));
  return result;
}

herr_t HDF5::H5Pget_libver_bounds(hid_t plist_id, H5F_libver_t *low,
                                  H5F_libver_t *high) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_libver_bounds, herr_t, (plist_id, low, high));
  return result;
}

herr_t HDF5::H5Pset_elink_file_cache_size(hid_t plist_id,
                                          unsigned int efc_size) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_elink_file_cache_size, herr_t,
                        (plist_id, efc_size));
  return result;
}

herr_t HDF5::H5Pget_elink_file_cache_size(hid_t plist_id,
                                          unsigned int *efc_size) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_elink_file_cache_size, herr_t,
                        (plist_id, efc_size));
  return result;
}

herr_t HDF5::H5Pset_file_image(hid_t fapl_id, void *buf_ptr, size_t buf_len) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_file_image, herr_t, (fapl_id, buf_ptr, buf_len));
  return result;
}

herr_t HDF5::H5Pget_file_image(hid_t fapl_id, void **buf_ptr_ptr,
                               size_t *buf_len_ptr) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_file_image, herr_t,
                        (fapl_id, buf_ptr_ptr, buf_len_ptr));
  return result;
}

herr_t HDF5::H5Pset_file_image_callbacks(
    hid_t fapl_id, H5FD_file_image_callbacks_t *callbacks_ptr) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_file_image_callbacks, herr_t,
                        (fapl_id, callbacks_ptr));
  return result;
}

herr_t HDF5::H5Pget_file_image_callbacks(
    hid_t fapl_id, H5FD_file_image_callbacks_t *callbacks_ptr) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_file_image_callbacks, herr_t,
                        (fapl_id, callbacks_ptr));
  return result;
}

herr_t HDF5::H5Pset_core_write_tracking(hid_t fapl_id, hbool_t is_enabled,
                                        size_t page_size) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_core_write_tracking, herr_t,
                        (fapl_id, is_enabled, page_size));
  return result;
}

herr_t HDF5::H5Pget_core_write_tracking(hid_t fapl_id, hbool_t *is_enabled,
                                        size_t *page_size) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_core_write_tracking, herr_t,
                        (fapl_id, is_enabled, page_size));
  return result;
}

herr_t HDF5::H5Pset_metadata_read_attempts(hid_t plist_id,
                                           unsigned int attempts) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_metadata_read_attempts, herr_t,
                        (plist_id, attempts));
  return result;
}

herr_t HDF5::H5Pget_metadata_read_attempts(hid_t plist_id,
                                           unsigned int *attempts) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_metadata_read_attempts, herr_t,
                        (plist_id, attempts));
  return result;
}

herr_t HDF5::H5Pset_object_flush_cb(hid_t plist_id, H5F_flush_cb_t func,
                                    void *udata) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_object_flush_cb, herr_t,
                        (plist_id, func, udata));
  return result;
}

herr_t HDF5::H5Pget_object_flush_cb(hid_t plist_id, H5F_flush_cb_t *func,
                                    void **udata) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_object_flush_cb, herr_t,
                        (plist_id, func, udata));
  return result;
}

herr_t HDF5::H5Pset_mdc_log_options(hid_t plist_id, hbool_t is_enabled,
                                    const char *location,
                                    hbool_t start_on_access) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_mdc_log_options, herr_t,
                        (plist_id, is_enabled, location, start_on_access));
  return result;
}

herr_t HDF5::H5Pget_mdc_log_options(hid_t plist_id, hbool_t *is_enabled,
                                    char *location, size_t *location_size,
                                    hbool_t *start_on_access) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Pget_mdc_log_options, herr_t,
      (plist_id, is_enabled, location, location_size, start_on_access));
  return result;
}

herr_t HDF5::H5Pset_evict_on_close(hid_t fapl_id, hbool_t evict_on_close) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_evict_on_close, herr_t,
                        (fapl_id, evict_on_close));
  return result;
}

herr_t HDF5::H5Pget_evict_on_close(hid_t fapl_id, hbool_t *evict_on_close) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_evict_on_close, herr_t,
                        (fapl_id, evict_on_close));
  return result;
}

herr_t HDF5::H5Pset_file_locking(hid_t fapl_id, hbool_t use_file_locking,
                                 hbool_t ignore_when_disabled) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_file_locking, herr_t,
                        (fapl_id, use_file_locking, ignore_when_disabled));
  return result;
}

herr_t HDF5::H5Pget_file_locking(hid_t fapl_id, hbool_t *use_file_locking,
                                 hbool_t *ignore_when_disabled) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_file_locking, herr_t,
                        (fapl_id, use_file_locking, ignore_when_disabled));
  return result;
}

herr_t HDF5::H5Pset_mdc_image_config(hid_t plist_id,
                                     H5AC_cache_image_config_t *config_ptr) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_mdc_image_config, herr_t,
                        (plist_id, config_ptr));
  return result;
}

herr_t HDF5::H5Pget_mdc_image_config(hid_t plist_id,
                                     H5AC_cache_image_config_t *config_ptr) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_mdc_image_config, herr_t,
                        (plist_id, config_ptr));
  return result;
}

herr_t HDF5::H5Pset_page_buffer_size(hid_t plist_id, size_t buf_size,
                                     unsigned int min_meta_per,
                                     unsigned int min_raw_per) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_page_buffer_size, herr_t,
                        (plist_id, buf_size, min_meta_per, min_raw_per));
  return result;
}

herr_t HDF5::H5Pget_page_buffer_size(hid_t plist_id, size_t *buf_size,
                                     unsigned int *min_meta_per,
                                     unsigned int *min_raw_per) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_page_buffer_size, herr_t,
                        (plist_id, buf_size, min_meta_per, min_raw_per));
  return result;
}

herr_t HDF5::H5Pset_layout(hid_t plist_id, H5D_layout_t layout) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_layout, herr_t, (plist_id, layout));
  return result;
}

H5D_layout_t HDF5::H5Pget_layout(hid_t plist_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_layout, H5D_layout_t, (plist_id));
  return result;
}

herr_t HDF5::H5Pset_chunk(hid_t plist_id, int ndims, const hsize_t dim[]) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_chunk, herr_t, (plist_id, ndims, dim));
  return result;
}

int HDF5::H5Pget_chunk(hid_t plist_id, int max_ndims, hsize_t dim[]) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_chunk, int, (plist_id, max_ndims, dim));
  return result;
}

herr_t HDF5::H5Pset_virtual(hid_t dcpl_id, hid_t vspace_id,
                            const char *src_file_name,
                            const char *src_dset_name, hid_t src_space_id) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Pset_virtual, herr_t,
      (dcpl_id, vspace_id, src_file_name, src_dset_name, src_space_id));
  return result;
}

herr_t HDF5::H5Pget_virtual_count(hid_t dcpl_id, size_t *count) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_virtual_count, herr_t, (dcpl_id, count));
  return result;
}

hid_t HDF5::H5Pget_virtual_vspace(hid_t dcpl_id, size_t index) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_virtual_vspace, hid_t, (dcpl_id, index));
  return result;
}

hid_t HDF5::H5Pget_virtual_srcspace(hid_t dcpl_id, size_t index) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_virtual_srcspace, hid_t, (dcpl_id, index));
  return result;
}

ssize_t HDF5::H5Pget_virtual_filename(hid_t dcpl_id, size_t index, char *name,
                                      size_t size) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_virtual_filename, ssize_t,
                        (dcpl_id, index, name, size));
  return result;
}

ssize_t HDF5::H5Pget_virtual_dsetname(hid_t dcpl_id, size_t index, char *name,
                                      size_t size) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_virtual_dsetname, ssize_t,
                        (dcpl_id, index, name, size));
  return result;
}

herr_t HDF5::H5Pset_external(hid_t plist_id, const char *name, off_t offset,
                             hsize_t size) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_external, herr_t,
                        (plist_id, name, offset, size));
  return result;
}

herr_t HDF5::H5Pset_chunk_opts(hid_t plist_id, unsigned int opts) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_chunk_opts, herr_t, (plist_id, opts));
  return result;
}

herr_t HDF5::H5Pget_chunk_opts(hid_t plist_id, unsigned int *opts) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_chunk_opts, herr_t, (plist_id, opts));
  return result;
}

int HDF5::H5Pget_external_count(hid_t plist_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_external_count, int, (plist_id));
  return result;
}

herr_t HDF5::H5Pget_external(hid_t plist_id, unsigned int idx, size_t name_size,
                             char *name, off_t *offset, hsize_t *size) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_external, herr_t,
                        (plist_id, idx, name_size, name, offset, size));
  return result;
}

herr_t HDF5::H5Pset_szip(hid_t plist_id, unsigned int options_mask,
                         unsigned int pixels_per_block) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_szip, herr_t,
                        (plist_id, options_mask, pixels_per_block));
  return result;
}

herr_t HDF5::H5Pset_shuffle(hid_t plist_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_shuffle, herr_t, (plist_id));
  return result;
}

herr_t HDF5::H5Pset_nbit(hid_t plist_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_nbit, herr_t, (plist_id));
  return result;
}

herr_t HDF5::H5Pset_scaleoffset(hid_t plist_id, H5Z_SO_scale_type_t scale_type,
                                int scale_factor) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_scaleoffset, herr_t,
                        (plist_id, scale_type, scale_factor));
  return result;
}

herr_t HDF5::H5Pset_fill_value(hid_t plist_id, hid_t type_id,
                               const void *value) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_fill_value, herr_t, (plist_id, type_id, value));
  return result;
}

herr_t HDF5::H5Pget_fill_value(hid_t plist_id, hid_t type_id, void *value) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_fill_value, herr_t, (plist_id, type_id, value));
  return result;
}

herr_t HDF5::H5Pfill_value_defined(hid_t plist, H5D_fill_value_t *status) {
  BRAHMA_UNWRAPPED_FUNC(H5Pfill_value_defined, herr_t, (plist, status));
  return result;
}

herr_t HDF5::H5Pset_alloc_time(hid_t plist_id, H5D_alloc_time_t alloc_time) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_alloc_time, herr_t, (plist_id, alloc_time));
  return result;
}

herr_t HDF5::H5Pget_alloc_time(hid_t plist_id, H5D_alloc_time_t *alloc_time) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_alloc_time, herr_t, (plist_id, alloc_time));
  return result;
}

herr_t HDF5::H5Pset_fill_time(hid_t plist_id, H5D_fill_time_t fill_time) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_fill_time, herr_t, (plist_id, fill_time));
  return result;
}

herr_t HDF5::H5Pget_fill_time(hid_t plist_id, H5D_fill_time_t *fill_time) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_fill_time, herr_t, (plist_id, fill_time));
  return result;
}

herr_t HDF5::H5Pget_dset_no_attrs_hint(hid_t dcpl_id, hbool_t *minimize) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_dset_no_attrs_hint, herr_t, (dcpl_id, minimize));
  return result;
}

herr_t HDF5::H5Pset_dset_no_attrs_hint(hid_t dcpl_id, hbool_t minimize) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_dset_no_attrs_hint, herr_t, (dcpl_id, minimize));
  return result;
}

herr_t HDF5::H5Pset_chunk_cache(hid_t dapl_id, size_t rdcc_nslots,
                                size_t rdcc_nbytes, double rdcc_w0) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_chunk_cache, herr_t,
                        (dapl_id, rdcc_nslots, rdcc_nbytes, rdcc_w0));
  return result;
}

herr_t HDF5::H5Pget_chunk_cache(hid_t dapl_id, size_t *rdcc_nslots,
                                size_t *rdcc_nbytes, double *rdcc_w0) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_chunk_cache, herr_t,
                        (dapl_id, rdcc_nslots, rdcc_nbytes, rdcc_w0));
  return result;
}

herr_t HDF5::H5Pset_virtual_view(hid_t plist_id, H5D_vds_view_t view) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_virtual_view, herr_t, (plist_id, view));
  return result;
}

herr_t HDF5::H5Pget_virtual_view(hid_t plist_id, H5D_vds_view_t *view) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_virtual_view, herr_t, (plist_id, view));
  return result;
}

herr_t HDF5::H5Pset_virtual_printf_gap(hid_t plist_id, hsize_t gap_size) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_virtual_printf_gap, herr_t,
                        (plist_id, gap_size));
  return result;
}

herr_t HDF5::H5Pget_virtual_printf_gap(hid_t plist_id, hsize_t *gap_size) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_virtual_printf_gap, herr_t,
                        (plist_id, gap_size));
  return result;
}

herr_t HDF5::H5Pset_virtual_prefix(hid_t dapl_id, const char *prefix) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_virtual_prefix, herr_t, (dapl_id, prefix));
  return result;
}

ssize_t HDF5::H5Pget_virtual_prefix(hid_t dapl_id, char *prefix, size_t size) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_virtual_prefix, ssize_t,
                        (dapl_id, prefix, size));
  return result;
}

herr_t HDF5::H5Pset_append_flush(hid_t plist_id, unsigned int ndims,
                                 const hsize_t boundary[], H5D_append_cb_t func,
                                 void *udata) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_append_flush, herr_t,
                        (plist_id, ndims, boundary, func, udata));
  return result;
}

herr_t HDF5::H5Pget_append_flush(hid_t plist_id, unsigned int dims,
                                 hsize_t boundary[], H5D_append_cb_t *func,
                                 void **udata) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_append_flush, herr_t,
                        (plist_id, dims, boundary, func, udata));
  return result;
}

herr_t HDF5::H5Pset_efile_prefix(hid_t dapl_id, const char *prefix) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_efile_prefix, herr_t, (dapl_id, prefix));
  return result;
}

ssize_t HDF5::H5Pget_efile_prefix(hid_t dapl_id, char *prefix, size_t size) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_efile_prefix, ssize_t, (dapl_id, prefix, size));
  return result;
}

herr_t HDF5::H5Pset_data_transform(hid_t plist_id, const char *expression) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_data_transform, herr_t, (plist_id, expression));
  return result;
}

ssize_t HDF5::H5Pget_data_transform(hid_t plist_id, char *expression,
                                    size_t size) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_data_transform, ssize_t,
                        (plist_id, expression, size));
  return result;
}

herr_t HDF5::H5Pset_buffer(hid_t plist_id, size_t size, void *tconv,
                           void *bkg) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_buffer, herr_t, (plist_id, size, tconv, bkg));
  return result;
}

size_t HDF5::H5Pget_buffer(hid_t plist_id, void **tconv, void **bkg) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_buffer, size_t, (plist_id, tconv, bkg));
  return result;
}

herr_t HDF5::H5Pset_preserve(hid_t plist_id, hbool_t status) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_preserve, herr_t, (plist_id, status));
  return result;
}

int HDF5::H5Pget_preserve(hid_t plist_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_preserve, int, (plist_id));
  return result;
}

herr_t HDF5::H5Pset_edc_check(hid_t plist_id, H5Z_EDC_t check) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_edc_check, herr_t, (plist_id, check));
  return result;
}

H5Z_EDC_t HDF5::H5Pget_edc_check(hid_t plist_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_edc_check, H5Z_EDC_t, (plist_id));
  return result;
}

herr_t HDF5::H5Pset_filter_callback(hid_t plist_id, H5Z_filter_func_t func,
                                    void *op_data) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_filter_callback, herr_t,
                        (plist_id, func, op_data));
  return result;
}

herr_t HDF5::H5Pset_btree_ratios(hid_t plist_id, double left, double middle,
                                 double right) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_btree_ratios, herr_t,
                        (plist_id, left, middle, right));
  return result;
}

herr_t HDF5::H5Pget_btree_ratios(hid_t plist_id, double *left, double *middle,
                                 double *right) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_btree_ratios, herr_t,
                        (plist_id, left, middle, right));
  return result;
}

herr_t HDF5::H5Pset_vlen_mem_manager(hid_t plist_id, H5MM_allocate_t alloc_func,
                                     void *alloc_info, H5MM_free_t free_func,
                                     void *free_info) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Pset_vlen_mem_manager, herr_t,
      (plist_id, alloc_func, alloc_info, free_func, free_info));
  return result;
}

herr_t HDF5::H5Pget_vlen_mem_manager(hid_t plist_id,
                                     H5MM_allocate_t *alloc_func,
                                     void **alloc_info, H5MM_free_t *free_func,
                                     void **free_info) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Pget_vlen_mem_manager, herr_t,
      (plist_id, alloc_func, alloc_info, free_func, free_info));
  return result;
}

herr_t HDF5::H5Pset_hyper_vector_size(hid_t fapl_id, size_t size) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_hyper_vector_size, herr_t, (fapl_id, size));
  return result;
}

herr_t HDF5::H5Pget_hyper_vector_size(hid_t fapl_id, size_t *size) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_hyper_vector_size, herr_t, (fapl_id, size));
  return result;
}

herr_t HDF5::H5Pset_type_conv_cb(hid_t dxpl_id, H5T_conv_except_func_t op,
                                 void *operate_data) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_type_conv_cb, herr_t,
                        (dxpl_id, op, operate_data));
  return result;
}

herr_t HDF5::H5Pget_type_conv_cb(hid_t dxpl_id, H5T_conv_except_func_t *op,
                                 void **operate_data) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_type_conv_cb, herr_t,
                        (dxpl_id, op, operate_data));
  return result;
}

herr_t HDF5::H5Pset_create_intermediate_group(hid_t plist_id,
                                              unsigned int crt_intmd) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_create_intermediate_group, herr_t,
                        (plist_id, crt_intmd));
  return result;
}

herr_t HDF5::H5Pget_create_intermediate_group(hid_t plist_id,
                                              unsigned int *crt_intmd) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_create_intermediate_group, herr_t,
                        (plist_id, crt_intmd));
  return result;
}

herr_t HDF5::H5Pset_local_heap_size_hint(hid_t plist_id, size_t size_hint) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_local_heap_size_hint, herr_t,
                        (plist_id, size_hint));
  return result;
}

herr_t HDF5::H5Pget_local_heap_size_hint(hid_t plist_id, size_t *size_hint) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_local_heap_size_hint, herr_t,
                        (plist_id, size_hint));
  return result;
}

herr_t HDF5::H5Pset_link_phase_change(hid_t plist_id, unsigned int max_compact,
                                      unsigned int min_dense) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_link_phase_change, herr_t,
                        (plist_id, max_compact, min_dense));
  return result;
}

herr_t HDF5::H5Pget_link_phase_change(hid_t plist_id, unsigned int *max_compact,
                                      unsigned int *min_dense) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_link_phase_change, herr_t,
                        (plist_id, max_compact, min_dense));
  return result;
}

herr_t HDF5::H5Pset_est_link_info(hid_t plist_id, unsigned int est_num_entries,
                                  unsigned int est_name_len) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_est_link_info, herr_t,
                        (plist_id, est_num_entries, est_name_len));
  return result;
}

herr_t HDF5::H5Pget_est_link_info(hid_t plist_id, unsigned int *est_num_entries,
                                  unsigned int *est_name_len) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_est_link_info, herr_t,
                        (plist_id, est_num_entries, est_name_len));
  return result;
}

herr_t HDF5::H5Pset_link_creation_order(hid_t plist_id,
                                        unsigned int crt_order_flags) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_link_creation_order, herr_t,
                        (plist_id, crt_order_flags));
  return result;
}

herr_t HDF5::H5Pget_link_creation_order(hid_t plist_id,
                                        unsigned int *crt_order_flags) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_link_creation_order, herr_t,
                        (plist_id, crt_order_flags));
  return result;
}

herr_t HDF5::H5Pset_char_encoding(hid_t plist_id, H5T_cset_t encoding) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_char_encoding, herr_t, (plist_id, encoding));
  return result;
}

herr_t HDF5::H5Pget_char_encoding(hid_t plist_id, H5T_cset_t *encoding) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_char_encoding, herr_t, (plist_id, encoding));
  return result;
}

herr_t HDF5::H5Pset_nlinks(hid_t plist_id, size_t nlinks) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_nlinks, herr_t, (plist_id, nlinks));
  return result;
}

herr_t HDF5::H5Pget_nlinks(hid_t plist_id, size_t *nlinks) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_nlinks, herr_t, (plist_id, nlinks));
  return result;
}

herr_t HDF5::H5Pset_elink_prefix(hid_t plist_id, const char *prefix) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_elink_prefix, herr_t, (plist_id, prefix));
  return result;
}

ssize_t HDF5::H5Pget_elink_prefix(hid_t plist_id, char *prefix, size_t size) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_elink_prefix, ssize_t, (plist_id, prefix, size));
  return result;
}

hid_t HDF5::H5Pget_elink_fapl(hid_t lapl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_elink_fapl, hid_t, (lapl_id));
  return result;
}

herr_t HDF5::H5Pset_elink_fapl(hid_t lapl_id, hid_t fapl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_elink_fapl, herr_t, (lapl_id, fapl_id));
  return result;
}

herr_t HDF5::H5Pset_elink_acc_flags(hid_t lapl_id, unsigned int flags) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_elink_acc_flags, herr_t, (lapl_id, flags));
  return result;
}

herr_t HDF5::H5Pget_elink_acc_flags(hid_t lapl_id, unsigned int *flags) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_elink_acc_flags, herr_t, (lapl_id, flags));
  return result;
}

herr_t HDF5::H5Pset_elink_cb(hid_t lapl_id, H5L_elink_traverse_t func,
                             void *op_data) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_elink_cb, herr_t, (lapl_id, func, op_data));
  return result;
}

herr_t HDF5::H5Pget_elink_cb(hid_t lapl_id, H5L_elink_traverse_t *func,
                             void **op_data) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_elink_cb, herr_t, (lapl_id, func, op_data));
  return result;
}

herr_t HDF5::H5Pset_copy_object(hid_t plist_id, unsigned int crt_intmd) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_copy_object, herr_t, (plist_id, crt_intmd));
  return result;
}

herr_t HDF5::H5Pget_copy_object(hid_t plist_id, unsigned int *crt_intmd) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_copy_object, herr_t, (plist_id, crt_intmd));
  return result;
}

herr_t HDF5::H5Padd_merge_committed_dtype_path(hid_t plist_id,
                                               const char *path) {
  BRAHMA_UNWRAPPED_FUNC(H5Padd_merge_committed_dtype_path, herr_t,
                        (plist_id, path));
  return result;
}

herr_t HDF5::H5Pfree_merge_committed_dtype_paths(hid_t plist_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Pfree_merge_committed_dtype_paths, herr_t,
                        (plist_id));
  return result;
}

herr_t HDF5::H5Pset_mcdt_search_cb(hid_t plist_id, H5O_mcdt_search_cb_t func,
                                   void *op_data) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_mcdt_search_cb, herr_t,
                        (plist_id, func, op_data));
  return result;
}

herr_t HDF5::H5Pget_mcdt_search_cb(hid_t plist_id, H5O_mcdt_search_cb_t *func,
                                   void **op_data) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_mcdt_search_cb, herr_t,
                        (plist_id, func, op_data));
  return result;
}

herr_t HDF5::H5Pregister1(hid_t cls_id, const char *name, size_t size,
                          void *def_value, H5P_prp_create_func_t prp_create,
                          H5P_prp_set_func_t prp_set,
                          H5P_prp_get_func_t prp_get,
                          H5P_prp_delete_func_t prp_del,
                          H5P_prp_copy_func_t prp_copy,
                          H5P_prp_close_func_t prp_close) {
  BRAHMA_UNWRAPPED_FUNC(H5Pregister1, herr_t,
                        (cls_id, name, size, def_value, prp_create, prp_set,
                         prp_get, prp_del, prp_copy, prp_close));
  return result;
}

herr_t HDF5::H5Pinsert1(hid_t plist_id, const char *name, size_t size,
                        void *value, H5P_prp_set_func_t prp_set,
                        H5P_prp_get_func_t prp_get,
                        H5P_prp_delete_func_t prp_delete,
                        H5P_prp_copy_func_t prp_copy,
                        H5P_prp_close_func_t prp_close) {
  BRAHMA_UNWRAPPED_FUNC(H5Pinsert1, herr_t,
                        (plist_id, name, size, value, prp_set, prp_get,
                         prp_delete, prp_copy, prp_close));
  return result;
}

H5Z_filter_t HDF5::H5Pget_filter1(hid_t plist_id, unsigned int filter,
                                  unsigned int *flags, size_t *cd_nelmts,
                                  unsigned int cd_values[], size_t namelen,
                                  char name[]) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Pget_filter1, H5Z_filter_t,
      (plist_id, filter, flags, cd_nelmts, cd_values, namelen, name));
  return result;
}

herr_t HDF5::H5Pget_filter_by_id1(hid_t plist_id, H5Z_filter_t id,
                                  unsigned int *flags, size_t *cd_nelmts,
                                  unsigned int cd_values[], size_t namelen,
                                  char name[]) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Pget_filter_by_id1, herr_t,
      (plist_id, id, flags, cd_nelmts, cd_values, namelen, name));
  return result;
}

herr_t HDF5::H5Pget_version(hid_t plist_id, unsigned int *boot,
                            unsigned int *freelist, unsigned int *stab,
                            unsigned int *shhdr) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_version, herr_t,
                        (plist_id, boot, freelist, stab, shhdr));
  return result;
}

herr_t HDF5::H5Pset_file_space(hid_t plist_id, H5F_file_space_type_t strategy,
                               hsize_t threshold) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_file_space, herr_t,
                        (plist_id, strategy, threshold));
  return result;
}

herr_t HDF5::H5Pget_file_space(hid_t plist_id, H5F_file_space_type_t *strategy,
                               hsize_t *threshold) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_file_space, herr_t,
                        (plist_id, strategy, threshold));
  return result;
}

herr_t HDF5::H5PLset_loading_state(unsigned int plugin_control_mask) {
  BRAHMA_UNWRAPPED_FUNC(H5PLset_loading_state, herr_t, (plugin_control_mask));
  return result;
}

herr_t HDF5::H5PLget_loading_state(unsigned int *plugin_control_mask) {
  BRAHMA_UNWRAPPED_FUNC(H5PLget_loading_state, herr_t, (plugin_control_mask));
  return result;
}

herr_t HDF5::H5PLappend(const char *search_path) {
  BRAHMA_UNWRAPPED_FUNC(H5PLappend, herr_t, (search_path));
  return result;
}

herr_t HDF5::H5PLprepend(const char *search_path) {
  BRAHMA_UNWRAPPED_FUNC(H5PLprepend, herr_t, (search_path));
  return result;
}

herr_t HDF5::H5PLreplace(const char *search_path, unsigned int index) {
  BRAHMA_UNWRAPPED_FUNC(H5PLreplace, herr_t, (search_path, index));
  return result;
}

herr_t HDF5::H5PLinsert(const char *search_path, unsigned int index) {
  BRAHMA_UNWRAPPED_FUNC(H5PLinsert, herr_t, (search_path, index));
  return result;
}

herr_t HDF5::H5PLremove(unsigned int index) {
  BRAHMA_UNWRAPPED_FUNC(H5PLremove, herr_t, (index));
  return result;
}

ssize_t HDF5::H5PLget(unsigned int index, char *path_buf, size_t buf_size) {
  BRAHMA_UNWRAPPED_FUNC(H5PLget, ssize_t, (index, path_buf, buf_size));
  return result;
}

herr_t HDF5::H5PLsize(unsigned int *num_paths) {
  BRAHMA_UNWRAPPED_FUNC(H5PLsize, herr_t, (num_paths));
  return result;
}

herr_t HDF5::H5Rcreate(void *ref, hid_t loc_id, const char *name,
                       H5R_type_t ref_type, hid_t space_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Rcreate, herr_t,
                        (ref, loc_id, name, ref_type, space_id));
  return result;
}

hid_t HDF5::H5Rdereference2(hid_t obj_id, hid_t oapl_id, H5R_type_t ref_type,
                            const void *ref) {
  BRAHMA_UNWRAPPED_FUNC(H5Rdereference2, hid_t,
                        (obj_id, oapl_id, ref_type, ref));
  return result;
}

hid_t HDF5::H5Rget_region(hid_t dataset, H5R_type_t ref_type, const void *ref) {
  BRAHMA_UNWRAPPED_FUNC(H5Rget_region, hid_t, (dataset, ref_type, ref));
  return result;
}

herr_t HDF5::H5Rget_obj_type2(hid_t id, H5R_type_t ref_type, const void *_ref,
                              H5O_type_t *obj_type) {
  BRAHMA_UNWRAPPED_FUNC(H5Rget_obj_type2, herr_t,
                        (id, ref_type, _ref, obj_type));
  return result;
}

ssize_t HDF5::H5Rget_name(hid_t loc_id, H5R_type_t ref_type, const void *ref,
                          char *name, size_t size) {
  BRAHMA_UNWRAPPED_FUNC(H5Rget_name, ssize_t,
                        (loc_id, ref_type, ref, name, size));
  return result;
}

H5G_obj_t HDF5::H5Rget_obj_type1(hid_t id, H5R_type_t ref_type,
                                 const void *_ref) {
  BRAHMA_UNWRAPPED_FUNC(H5Rget_obj_type1, H5G_obj_t, (id, ref_type, _ref));
  return result;
}

hid_t HDF5::H5Rdereference1(hid_t obj_id, H5R_type_t ref_type,
                            const void *ref) {
  BRAHMA_UNWRAPPED_FUNC(H5Rdereference1, hid_t, (obj_id, ref_type, ref));
  return result;
}

hid_t HDF5::H5Screate(H5S_class_t type) {
  BRAHMA_UNWRAPPED_FUNC(H5Screate, hid_t, (type));
  return result;
}

hid_t HDF5::H5Screate_simple(int rank, const hsize_t dims[],
                             const hsize_t maxdims[]) {
  BRAHMA_UNWRAPPED_FUNC(H5Screate_simple, hid_t, (rank, dims, maxdims));
  return result;
}

herr_t HDF5::H5Sset_extent_simple(hid_t space_id, int rank,
                                  const hsize_t dims[], const hsize_t max[]) {
  BRAHMA_UNWRAPPED_FUNC(H5Sset_extent_simple, herr_t,
                        (space_id, rank, dims, max));
  return result;
}

hid_t HDF5::H5Scopy(hid_t space_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Scopy, hid_t, (space_id));
  return result;
}

herr_t HDF5::H5Sclose(hid_t space_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Sclose, herr_t, (space_id));
  return result;
}

herr_t HDF5::H5Sencode(hid_t obj_id, void *buf, size_t *nalloc) {
  BRAHMA_UNWRAPPED_FUNC(H5Sencode, herr_t, (obj_id, buf, nalloc));
  return result;
}

hid_t HDF5::H5Sdecode(const void *buf) {
  BRAHMA_UNWRAPPED_FUNC(H5Sdecode, hid_t, (buf));
  return result;
}

hssize_t HDF5::H5Sget_simple_extent_npoints(hid_t space_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Sget_simple_extent_npoints, hssize_t, (space_id));
  return result;
}

int HDF5::H5Sget_simple_extent_ndims(hid_t space_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Sget_simple_extent_ndims, int, (space_id));
  return result;
}

int HDF5::H5Sget_simple_extent_dims(hid_t space_id, hsize_t dims[],
                                    hsize_t maxdims[]) {
  BRAHMA_UNWRAPPED_FUNC(H5Sget_simple_extent_dims, int,
                        (space_id, dims, maxdims));
  return result;
}

htri_t HDF5::H5Sis_simple(hid_t space_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Sis_simple, htri_t, (space_id));
  return result;
}

H5S_class_t HDF5::H5Sget_simple_extent_type(hid_t space_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Sget_simple_extent_type, H5S_class_t, (space_id));
  return result;
}

herr_t HDF5::H5Sset_extent_none(hid_t space_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Sset_extent_none, herr_t, (space_id));
  return result;
}

herr_t HDF5::H5Sextent_copy(hid_t dst_id, hid_t src_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Sextent_copy, herr_t, (dst_id, src_id));
  return result;
}

htri_t HDF5::H5Sextent_equal(hid_t sid1, hid_t sid2) {
  BRAHMA_UNWRAPPED_FUNC(H5Sextent_equal, htri_t, (sid1, sid2));
  return result;
}

H5S_sel_type HDF5::H5Sget_select_type(hid_t spaceid) {
  BRAHMA_UNWRAPPED_FUNC(H5Sget_select_type, H5S_sel_type, (spaceid));
  return result;
}

hssize_t HDF5::H5Sget_select_npoints(hid_t spaceid) {
  BRAHMA_UNWRAPPED_FUNC(H5Sget_select_npoints, hssize_t, (spaceid));
  return result;
}

herr_t HDF5::H5Sselect_copy(hid_t dst_id, hid_t src_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Sselect_copy, herr_t, (dst_id, src_id));
  return result;
}

htri_t HDF5::H5Sselect_valid(hid_t spaceid) {
  BRAHMA_UNWRAPPED_FUNC(H5Sselect_valid, htri_t, (spaceid));
  return result;
}

herr_t HDF5::H5Sselect_adjust(hid_t spaceid, const hssize_t *offset) {
  BRAHMA_UNWRAPPED_FUNC(H5Sselect_adjust, herr_t, (spaceid, offset));
  return result;
}

herr_t HDF5::H5Sget_select_bounds(hid_t spaceid, hsize_t start[],
                                  hsize_t end[]) {
  BRAHMA_UNWRAPPED_FUNC(H5Sget_select_bounds, herr_t, (spaceid, start, end));
  return result;
}

htri_t HDF5::H5Sselect_shape_same(hid_t space1_id, hid_t space2_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Sselect_shape_same, htri_t, (space1_id, space2_id));
  return result;
}

htri_t HDF5::H5Sselect_intersect_block(hid_t space_id, const hsize_t *start,
                                       const hsize_t *end) {
  BRAHMA_UNWRAPPED_FUNC(H5Sselect_intersect_block, htri_t,
                        (space_id, start, end));
  return result;
}

herr_t HDF5::H5Soffset_simple(hid_t space_id, const hssize_t *offset) {
  BRAHMA_UNWRAPPED_FUNC(H5Soffset_simple, herr_t, (space_id, offset));
  return result;
}

herr_t HDF5::H5Sselect_all(hid_t spaceid) {
  BRAHMA_UNWRAPPED_FUNC(H5Sselect_all, herr_t, (spaceid));
  return result;
}

herr_t HDF5::H5Sselect_none(hid_t spaceid) {
  BRAHMA_UNWRAPPED_FUNC(H5Sselect_none, herr_t, (spaceid));
  return result;
}

herr_t HDF5::H5Sselect_elements(hid_t space_id, H5S_seloper_t op,
                                size_t num_elem, const hsize_t *coord) {
  BRAHMA_UNWRAPPED_FUNC(H5Sselect_elements, herr_t,
                        (space_id, op, num_elem, coord));
  return result;
}

hssize_t HDF5::H5Sget_select_elem_npoints(hid_t spaceid) {
  BRAHMA_UNWRAPPED_FUNC(H5Sget_select_elem_npoints, hssize_t, (spaceid));
  return result;
}

herr_t HDF5::H5Sget_select_elem_pointlist(hid_t spaceid, hsize_t startpoint,
                                          hsize_t numpoints, hsize_t buf[]) {
  BRAHMA_UNWRAPPED_FUNC(H5Sget_select_elem_pointlist, herr_t,
                        (spaceid, startpoint, numpoints, buf));
  return result;
}

herr_t HDF5::H5Sselect_hyperslab(hid_t space_id, H5S_seloper_t op,
                                 const hsize_t start[], const hsize_t _stride[],
                                 const hsize_t count[],
                                 const hsize_t _block[]) {
  BRAHMA_UNWRAPPED_FUNC(H5Sselect_hyperslab, herr_t,
                        (space_id, op, start, _stride, count, _block));
  return result;
}

hid_t HDF5::H5Scombine_hyperslab(hid_t space_id, H5S_seloper_t op,
                                 const hsize_t start[], const hsize_t _stride[],
                                 const hsize_t count[],
                                 const hsize_t _block[]) {
  BRAHMA_UNWRAPPED_FUNC(H5Scombine_hyperslab, hid_t,
                        (space_id, op, start, _stride, count, _block));
  return result;
}

herr_t HDF5::H5Smodify_select(hid_t space1_id, H5S_seloper_t op,
                              hid_t space2_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Smodify_select, herr_t, (space1_id, op, space2_id));
  return result;
}

hid_t HDF5::H5Scombine_select(hid_t space1_id, H5S_seloper_t op,
                              hid_t space2_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Scombine_select, hid_t, (space1_id, op, space2_id));
  return result;
}

htri_t HDF5::H5Sis_regular_hyperslab(hid_t spaceid) {
  BRAHMA_UNWRAPPED_FUNC(H5Sis_regular_hyperslab, htri_t, (spaceid));
  return result;
}

htri_t HDF5::H5Sget_regular_hyperslab(hid_t spaceid, hsize_t start[],
                                      hsize_t stride[], hsize_t count[],
                                      hsize_t block[]) {
  BRAHMA_UNWRAPPED_FUNC(H5Sget_regular_hyperslab, htri_t,
                        (spaceid, start, stride, count, block));
  return result;
}

hssize_t HDF5::H5Sget_select_hyper_nblocks(hid_t spaceid) {
  BRAHMA_UNWRAPPED_FUNC(H5Sget_select_hyper_nblocks, hssize_t, (spaceid));
  return result;
}

herr_t HDF5::H5Sget_select_hyper_blocklist(hid_t spaceid, hsize_t startblock,
                                           hsize_t numblocks, hsize_t buf[]) {
  BRAHMA_UNWRAPPED_FUNC(H5Sget_select_hyper_blocklist, herr_t,
                        (spaceid, startblock, numblocks, buf));
  return result;
}

hid_t HDF5::H5Sselect_project_intersection(hid_t src_space_id,
                                           hid_t dst_space_id,
                                           hid_t src_intersect_space_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Sselect_project_intersection, hid_t,
                        (src_space_id, dst_space_id, src_intersect_space_id));
  return result;
}

hid_t HDF5::H5FD_core_init() {
  BRAHMA_UNWRAPPED_FUNC(H5FD_core_init, hid_t, ());
  return result;
}

herr_t HDF5::H5Pset_fapl_core(hid_t fapl_id, size_t increment,
                              hbool_t backing_store) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_fapl_core, herr_t,
                        (fapl_id, increment, backing_store));
  return result;
}

herr_t HDF5::H5Pget_fapl_core(hid_t fapl_id, size_t *increment,
                              hbool_t *backing_store) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_fapl_core, herr_t,
                        (fapl_id, increment, backing_store));
  return result;
}

hid_t HDF5::H5FD_family_init() {
  BRAHMA_UNWRAPPED_FUNC(H5FD_family_init, hid_t, ());
  return result;
}

herr_t HDF5::H5Pset_fapl_family(hid_t fapl_id, hsize_t memb_size,
                                hid_t memb_fapl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_fapl_family, herr_t,
                        (fapl_id, memb_size, memb_fapl_id));
  return result;
}

herr_t HDF5::H5Pget_fapl_family(hid_t fapl_id, hsize_t *memb_size,
                                hid_t *memb_fapl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_fapl_family, herr_t,
                        (fapl_id, memb_size, memb_fapl_id));
  return result;
}

hid_t HDF5::H5FD_hdfs_init() {
  BRAHMA_UNWRAPPED_FUNC(H5FD_hdfs_init, hid_t, ());
  return result;
}

herr_t HDF5::H5Pget_fapl_hdfs(hid_t fapl_id, H5FD_hdfs_fapl_t *fa_out) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_fapl_hdfs, herr_t, (fapl_id, fa_out));
  return result;
}

herr_t HDF5::H5Pset_fapl_hdfs(hid_t fapl_id, H5FD_hdfs_fapl_t *fa) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_fapl_hdfs, herr_t, (fapl_id, fa));
  return result;
}

hid_t HDF5::H5FD_log_init() {
  BRAHMA_UNWRAPPED_FUNC(H5FD_log_init, hid_t, ());
  return result;
}

herr_t HDF5::H5Pset_fapl_log(hid_t fapl_id, const char *logfile,
                             unsigned long long flags, size_t buf_size) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_fapl_log, herr_t,
                        (fapl_id, logfile, flags, buf_size));
  return result;
}

hid_t HDF5::H5FD_multi_init() {
  BRAHMA_UNWRAPPED_FUNC(H5FD_multi_init, hid_t, ());
  return result;
}

herr_t HDF5::H5Pset_fapl_multi(hid_t fapl_id, const H5FD_mem_t *memb_map,
                               const hid_t *memb_fapl,
                               const char *const *memb_name,
                               const haddr_t *memb_addr, hbool_t relax) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Pset_fapl_multi, herr_t,
      (fapl_id, memb_map, memb_fapl, memb_name, memb_addr, relax));
  return result;
}

herr_t HDF5::H5Pget_fapl_multi(hid_t fapl_id, H5FD_mem_t *memb_map,
                               hid_t *memb_fapl, char **memb_name,
                               haddr_t *memb_addr, hbool_t *relax) {
  BRAHMA_UNWRAPPED_FUNC(
      H5Pget_fapl_multi, herr_t,
      (fapl_id, memb_map, memb_fapl, memb_name, memb_addr, relax));
  return result;
}

herr_t HDF5::H5Pset_fapl_split(hid_t fapl, const char *meta_ext,
                               hid_t meta_plist_id, const char *raw_ext,
                               hid_t raw_plist_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_fapl_split, herr_t,
                        (fapl, meta_ext, meta_plist_id, raw_ext, raw_plist_id));
  return result;
}

hid_t HDF5::H5FD_ros3_init() {
  BRAHMA_UNWRAPPED_FUNC(H5FD_ros3_init, hid_t, ());
  return result;
}

herr_t HDF5::H5Pget_fapl_ros3(hid_t fapl_id, H5FD_ros3_fapl_t *fa_out) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_fapl_ros3, herr_t, (fapl_id, fa_out));
  return result;
}

herr_t HDF5::H5Pset_fapl_ros3(hid_t fapl_id, H5FD_ros3_fapl_t *fa) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_fapl_ros3, herr_t, (fapl_id, fa));
  return result;
}

hid_t HDF5::H5FD_sec2_init() {
  BRAHMA_UNWRAPPED_FUNC(H5FD_sec2_init, hid_t, ());
  return result;
}

herr_t HDF5::H5Pset_fapl_sec2(hid_t fapl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_fapl_sec2, herr_t, (fapl_id));
  return result;
}

hid_t HDF5::H5FD_splitter_init() {
  BRAHMA_UNWRAPPED_FUNC(H5FD_splitter_init, hid_t, ());
  return result;
}

herr_t HDF5::H5Pset_fapl_splitter(hid_t fapl_id,
                                  H5FD_splitter_vfd_config_t *config_ptr) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_fapl_splitter, herr_t, (fapl_id, config_ptr));
  return result;
}

herr_t HDF5::H5Pget_fapl_splitter(hid_t fapl_id,
                                  H5FD_splitter_vfd_config_t *config_ptr) {
  BRAHMA_UNWRAPPED_FUNC(H5Pget_fapl_splitter, herr_t, (fapl_id, config_ptr));
  return result;
}

hid_t HDF5::H5FD_stdio_init() {
  BRAHMA_UNWRAPPED_FUNC(H5FD_stdio_init, hid_t, ());
  return result;
}

herr_t HDF5::H5Pset_fapl_stdio(hid_t fapl_id) {
  BRAHMA_UNWRAPPED_FUNC(H5Pset_fapl_stdio, herr_t, (fapl_id));
  return result;
}

}  // namespace brahma

#endif  // BRAHMA_ENABLE_HDF5
